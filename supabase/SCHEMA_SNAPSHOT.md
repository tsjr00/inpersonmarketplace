...
|------|-----------|---------|
| 2026-05-12 | 20260512_138_vendor_market_agreement_acceptances + 20260512_139_weekly_booth_rentals | **DRAFTS — NOT YET APPLIED to any environment.** Two new table designs drafted for Phase B / Phase C work, sitting in `supabase/migrations/` (NOT in `applied/`). Migration 138: `vendor_market_agreement_acceptances` (id, vendor_profile_id FK, market_id FK, accepted_at, statements_snapshot JSONB, agreement_version, created_at; UNIQUE NULLS NOT DISTINCT on (vendor_profile_id, market_id, agreement_version); 2 indexes; RLS enabled default-deny). Records vendor's electronic-signature snapshot of manager opt-in agreement statements at acceptance time — survives catalog edits. Migration 139: `weekly_booth_rentals` (id, vendor_profile_id FK RESTRICT, market_id FK CASCADE, week_start_date, inventory_id FK RESTRICT to market_booth_inventory, booth_number, price_cents snapshot, status CHECK ('pending_payment','paid','cancelled','completed'), stripe_* placeholders, agreement_acceptance_id FK to mig 138, booked_at/paid_at/cancelled_at, created/updated_at; UNIQUE on (vendor_profile_id, market_id, week_start_date); 3 indexes; same-market integrity trigger mirroring mig 135; updated_at trigger mirroring mig 134; RLS enabled default-deny). Foundation for Phase B weekly booth rental flow + Phase C Stripe Connect payment. **Schema snapshot's structured Tables/Columns/Indexes sections will NOT be updated until these are applied.** Session 82. |
