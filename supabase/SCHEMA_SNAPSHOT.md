...
**Last Verified:** 2026-04-12 (Migrations 115–118 applied to all 3 envs. 116: `event_ratings` table. 117: `vertical_id` on `error_logs`. 118: 7 event indexes + FK on `organizer_user_id` + CHECK on `event_wave_reservations`. Structured tables updated below.)
