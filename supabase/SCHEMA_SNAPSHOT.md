...
**Last Verified:** 2026-04-04 (Dev + Staging verified from REFRESH_SCHEMA.SQL column output — all tables/columns match migrations 110-112. Note: user_profiles.buyer_tier default differs: Dev='free', Staging='standard' — pre-existing, not from this session.)
...
|------|-----------|---------|
| 2026-04-05 | 20260405_113_hybrid_events_access_code | **New columns on `catering_requests`:** `access_code` (TEXT, UNIQUE partial index where NOT NULL — 8-char uppercase code for company-paid/hybrid event attendee verification), `company_max_per_attendee_cents` (INTEGER, CHECK > 0 when not null — dollar cap per attendee for hybrid events). Applied to Dev & Staging. |
