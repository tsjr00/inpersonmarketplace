...
| 2026-05-12 | 20260512_139_weekly_booth_rentals | **DRAFT — NOT YET APPLIED to any environment.** Foundation for Phase C weekly booth rental + Stripe Connect payment flow. `weekly_booth_rentals` (id, vendor_profile_id FK RESTRICT, market_id FK CASCADE, week_start_date DATE, inventory_id FK RESTRICT to market_booth_inventory, booth_number TEXT, price_cents snapshot, status CHECK ('pending_payment','paid','cancelled','completed'), stripe_checkout_session_id/stripe_payment_intent_id, agreement_acceptance_id FK to mig 138, booked_at/paid_at/cancelled_at, created/updated_at). UNIQUE on (vendor_profile_id, market_id, week_start_date). 3 indexes including (market_id, week_start_date, status). Same-market integrity trigger mirroring mig 135. updated_at trigger mirroring mig 134. RLS enabled default-deny. Will apply alongside Phase C build. Session 82. |
