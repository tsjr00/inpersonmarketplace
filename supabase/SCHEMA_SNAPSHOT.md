...
**Last Verified:** 2026-02-17 (Staging database)
...
|------|-----------|---------|
| 2026-02-18 | 20260218_034_vendor_favorites | New `vendor_favorites` table: `id` (UUID PK), `user_id` (UUID FK→auth.users ON DELETE CASCADE), `vendor_profile_id` (UUID FK→vendor_profiles ON DELETE CASCADE), `created_at` (TIMESTAMPTZ). UNIQUE(user_id, vendor_profile_id). Indexes: `idx_vendor_favorites_user`, `idx_vendor_favorites_vendor`. RLS: users SELECT/INSERT/DELETE own rows. No vertical_id needed — vendor_profile_id scopes to vertical via vendor_profiles.vertical_id. **PENDING: Not yet applied.** |
