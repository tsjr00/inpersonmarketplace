...
| 20260426_127_vendor_payouts_market_box_subscription_constraint.sql | ✅ | 2026-04-26 | ✅ | 2026-04-26 | **CHECK constraint update:** `vendor_payouts_has_reference` extended to accept `market_box_subscription_id IS NOT NULL` as a valid reference (previously only allowed `order_item_id` or `market_box_pickup_id`). The `market_box_subscription_id` column existed but had never been usable for inserts because the constraint pre-dated it. Caused all market box subscription payouts to fail silently with `ERR_PAYOUT_003`. Discovered while backfilling Order #FA-2026-34616411 — vendor was charged $106.65 but no `vendor_payouts` row existed. Constraint logic only — no data backfill required. Applied to Dev + Staging 2026-04-26. **Pending Prod.** |
