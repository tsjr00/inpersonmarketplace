...
| 20260426_127_vendor_payouts_market_box_subscription_constraint.sql | ✅ | 2026-04-26 | ✅ | 2026-04-26 | **CHECK constraint update:** `vendor_payouts_has_reference` extended to accept `market_box_subscription_id IS NOT NULL` as a valid reference (previously only allowed `order_item_id` or `market_box_pickup_id`). The `market_box_subscription_id` column existed but had never been usable for inserts because the constraint pre-dated it. Caused all market box subscription payouts to fail silently with `ERR_PAYOUT_003`. Discovered while backfilling Order #FA-2026-34616411 — vendor was charged $106.65 but no `vendor_payouts` row existed. Constraint logic only — no data backfill required. Applied to all 3 envs (Prod 2026-04-26). |
| 20260430_128_event_setting_column.sql | ✅ | 2026-04-30 | ✅ | 2026-04-30 | **New column on `catering_requests`:** `event_setting` TEXT NULL with CHECK `(event_setting IS NULL OR event_setting = ANY (ARRAY['indoor','outdoor','either']))`. Captures Indoor/Outdoor/Either as structured data, separate from `setup_instructions` (free-text setup notes). Form previously misused `setup_instructions` as the enum store at EventRequestForm.tsx:446-471. After this ships with the form rework, vendor matching at `viability.ts` reads `event_setting` directly for deal-breakers (loud generator at indoor/quiet event) + warnings (weather-sensitive at outdoor). No backfill — legacy rows where `setup_instructions IN ('indoor','outdoor','either')` remain as-is; optional cleanup script in migration comments. **Applied to all 3 envs (Prod 2026-05-05).** Session 75. Structured tables in SCHEMA_SNAPSHOT.md not yet regenerated. |
| 20260509_137_enable_rls_market_manager_tables.sql | ✅ | 2026-05-09 | ✅ | 2026-05-09 | **Enable RLS on Market Manager v1 tables** (closes Supabase advisor warning from migs 134/135/136). `ALTER TABLE ENABLE ROW LEVEL SECURITY` on `market_booth_inventory`, `market_booth_placeholders`, `market_optin_statement_catalog`, `market_optin_selections`. No policies added — default-deny model: service_role (used by manager API routes via createServiceClient) bypasses RLS for full access; anon and authenticated keys are blocked from direct table reads/writes. Matches the design intent stated in migs 134/135/136 comments. Idempotent (re-running ENABLE on already-enabled table is a no-op). Updated table COMMENTs to document the security model. Verified with anon-role count = 0 + service_role count = 15 on prod immediately after apply. Applied to all 3 envs (Prod 2026-05-09). Session 81. |
| 20260509_136_market_optin_statements.sql | ✅ | 2026-05-09 | ✅ | 2026-05-09 | **Two new tables for opt-in vendor agreement system** (Phase A of Market Manager v2 plan): `market_optin_statement_catalog` (id TEXT PK, category TEXT CHECK in product_quality/conduct/insurance/fees/compliance, statement TEXT, placeholders TEXT[] DEFAULT '{}', active BOOLEAN, sort_order INTEGER, created_at) and `market_optin_selections` (id UUID PK, market_id UUID FK→markets ON DELETE CASCADE, statement_id TEXT FK→market_optin_statement_catalog ON DELETE CASCADE, placeholder_values JSONB DEFAULT '{}', selected_at; UNIQUE(market_id, statement_id)). Index `idx_market_optin_selections_market` on market_id. Seed: 15 starter statements via INSERT...ON CONFLICT DO NOTHING (idempotent re-run safe), copied from `apps/web/.claude/market_manager_optin_statements_v1.md`. Vendor acceptance records ship in Phase B alongside co-branded vendor signup. RLS not enabled — route-layer auth via isMarketManager helper. Applied to all 3 envs (Prod 2026-05-09). Session 81. |
| 20260509_135_market_booth_placeholders.sql | ✅ | 2026-05-09 | ✅ | 2026-05-09 | **New table `market_booth_placeholders`** (Phase A of Market Manager v2 plan): tracks off-platform vendor occupancy without capturing vendor identity. Columns: `id UUID PK`, `market_id UUID NOT NULL FK→markets ON DELETE CASCADE`, `inventory_id UUID NULL FK→market_booth_inventory ON DELETE SET NULL`, `booth_number TEXT NOT NULL`, `notes TEXT NULL`, `created_at`, `updated_at`. UNIQUE(market_id, booth_number). Indexes on market_id (unconditional) and inventory_id (partial WHERE NOT NULL). Two triggers: `trg_booth_placeholder_inventory_market` BEFORE INSERT/UPDATE OF (market_id, inventory_id) raises exception if inventory_id belongs to a different market — same-market integrity check that prevents cross-market id spoofing if API auth is bypassed; `trg_market_booth_placeholders_updated_at` BEFORE UPDATE refreshes updated_at. Single-column UUID FK matches codebase pattern (no composite FKs anywhere); ON DELETE SET NULL means placeholder remains if size tier deleted, manager can re-link. RLS not enabled — same model as mig 134. Applied to all 3 envs (Prod 2026-05-09). Session 81. |
| 20260508_134_market_booth_inventory.sql | ✅ | 2026-05-08 | ✅ | 2026-05-08 | **New table `market_booth_inventory`:** per-market booth size tiers backing Phase 2 of the market manager v2 plan. `id UUID PK`, `market_id UUID NOT NULL FK→markets ON DELETE CASCADE`, `size_label TEXT NOT NULL`, `dimensions TEXT NULL`, `count INTEGER CHECK >= 0`, `weekly_price_cents INTEGER CHECK >= 0`, `created_at`, `updated_at`. UNIQUE (market_id, size_label). Index on market_id. BEFORE-UPDATE trigger `trg_market_booth_inventory_updated_at` refreshes updated_at. RLS not enabled — route-layer auth via `isMarketManager()` helper enforces access. Off-platform-vendor placeholder tracking deferred to a later migration alongside manager onboarding UI. Applied to all 3 envs (Prod 2026-05-09). Session 80. |
| 20260508_133_market_manager_v1_schema.sql | ✅ | 2026-05-08 | ✅ | 2026-05-08 | **New columns on `markets`:** `manager_email TEXT`, `manager_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL`, `manager_invited_at TIMESTAMPTZ`, `manager_accepted_at TIMESTAMPTZ`. Two partial indexes: `idx_markets_manager_email ON markets(LOWER(manager_email)) WHERE manager_email IS NOT NULL` + `idx_markets_manager_user_id ON markets(manager_user_id) WHERE manager_user_id IS NOT NULL`. Foundation for Market Manager v1 partnership work — buyer dashboard card + manager dashboard skeleton + future admin assignment UI / weekly booth rentals / vendor-to-booth assignment. Manager assignment is dual-key: admin can set `manager_email` before user signs up; `manager_user_id` is backfilled later. Auth helper at `src/lib/markets/manager-auth.ts` checks both branches. v1 vertical scope is `farmers_market` only (FT park-operator deferred). Note: `market_vendors.booth_number` was originally planned here but already exists from migration 001; no re-assertion. Applied to all 3 envs (Prod 2026-05-09). Session 80. |
| 20260508_132_drop_legacy_analytics_functions.sql | ✅ | 2026-05-08 | ✅ | 2026-05-08 | **Drops 3 legacy SQL functions:** `get_analytics_overview(DATE,DATE,TEXT)`, `get_top_vendors(DATE,DATE,TEXT,INTEGER)`, `get_vendor_revenue_trends(UUID,DATE,DATE,TEXT)`. All three queried the legacy `transactions` table (empty since migration 046 in Feb 2026 noted it was replaced by orders/order_items). Result: admin analytics dashboards showed $0 / 0 orders for ~3 months because the functions returned empty aggregations. Vendor-side trends route already had no caller (bypassed via direct order_items query). Admin overview + top-vendors callers were rewritten in the same commit (e42db025) to query orders/order_items + market_box_subscriptions directly using JS aggregation, matching the working vendor-side pattern. Function-only DROP, no schema changes, no data backfill. Applied to all 3 envs (Prod 2026-05-09). Session 80. |
| 20260501_129_catering_requests_address_optional.sql | ✅ | 2026-05-01 | ✅ | 2026-05-01 | **Schema relaxation:** `ALTER TABLE catering_requests ALTER COLUMN address DROP NOT NULL`. Per Session 75 Stage 1/Stage 2 design, address is OPTIONAL at Stage 1 (form intake) and BECOMES MANDATORY at Stage 2 (admin gate at `admin/events/[id]/route.ts:113-122` blocks status='approved' when address null/empty). Original migration 070 created the column as NOT NULL predating the Stage 1/Stage 2 split. Form was 500'ing on insert with PostgreSQL code 23502 ("null value in column address violates not-null constraint") for any organizer who didn't fill address. Hotfix discovered during staging test 2026-05-01. No data backfill — existing rows already have non-null address values. **Applied to all 3 envs (Prod 2026-05-05).** Session 75. |
| 20260502_130_wave_capacity_no_silent_fallback.sql | ✅ | 2026-05-01 | ✅ | 2026-05-01 | **Function rewrite:** `recalculate_wave_capacity(p_market_id UUID)` — removes silent `COALESCE(mv.event_max_orders_per_wave, 25)` fallback that was added in migration 120. Now `RAISE EXCEPTION` if any accepted vendor at the market lacks declared `event_max_orders_per_wave`. Per Session 76 audit (D1) and the user's "no silent fallbacks" rule — the 25 default could over-promise capacity (buyers reserve waves vendor can't fulfill) or under-promise (turn away orders vendor could handle). Return type preserved as `TABLE(waves_updated INTEGER, new_capacity INTEGER)` to match migration 120's contract — initial draft used plain `INTEGER` and hit Postgres 42P13 on dev. Function-body change only, no schema changes, no data backfill (pre-push verification query returned zero rows on Prod 2026-05-05). **Applied to all 3 envs (Prod 2026-05-05).** Session 76. |
| 20260504_131_pickup_dates_require_active_vms.sql | ⚠️ | 2026-05-04 (DDL applied; runtime broken on Dev due to pre-existing env drift — `markets.event_end_date` missing) | ✅ | 2026-05-04 | **Function rewrite:** `get_available_pickup_dates(p_listing_id)` — tightens WHERE clause in `listing_schedules` CTE so traditional markets in ALL verticals require an active `vendor_market_schedules` row. Previously FM was exempt (`vertical_id != 'food_trucks'`), which let the listing detail page show pickup dates for markets where the vendor had deactivated attendance, while the cart correctly rejected those items at checkout. New logic: traditional → vms required; private_pickup → vms not required (unchanged); FM events → vms not required (unchanged); FT events → vms required (unchanged). Verified on Staging against listing `b6d14489-251a-4db6-a23f-d64a1cf5bbe2` (Sweet Rise Bakery, vendor `60edb3d6-...`): Amarillo + Canyon (inactive vms) now correctly excluded; Anew Perspective Pop-Up (FM event, no vms) still appears. Pre-fix audit query confirmed only 3 listings × 2 markets × 1 vendor affected, all with `inactive_vms_count=1, missing_vms_count=0` — no backfill required. Function body otherwise verbatim from migration 109. **Applied to all 3 envs (Prod 2026-05-05).** Dev runtime failure is independent: Dev is missing migrations 039/040 (event date columns); separate backlog item. Session 78. |
| 20260425_126_unified_market_box_tier_limits.sql | ✅ | 2026-04-25 | ✅ | 2026-04-25 | **Function rewrite:** `enforce_market_box_tier_limit()` (BEFORE INSERT/UPDATE trigger on `market_box_offerings`) now uses unified tier limits matching `vendor-limits.ts` (pro=6, boss=10, all others=3). Migration 064 used pre-unification limits (FM standard=2, premium=4, etc.) which diverged from app layer after migration 089 unified tiers. Vendors saw "2 of 3 used" in UI but got blocked at 2/2 by DB trigger. App and DB now agree. Applied to all 3 envs (Prod 2026-04-26). |
| 20260425_125_market_box_term_duration.sql | ✅ | 2026-04-25 | ✅ | 2026-04-25 | **Function rewrite (Option A duration semantics):** `create_market_box_pickups()` — `original_end_date` now equals `start_date + term_weeks*7` instead of `start_date + (num_pickups-1)*interval`. Effect: subscription's "ends" date matches the named term length (4-week = 28 days, 8-week = 56 days) regardless of cadence. Pickup dates unchanged. Buyer mental model: "1 Month subscription with N pickups." No backfill — staging/prod have no biweekly subs and existing weekly subs continue under old semantic. New subs only get the new end_date. Applied to all 3 envs (Prod 2026-04-26). |
| 20260420_124_market_box_biweekly_frequency.sql | ✅ | 2026-04-24 | ✅ | 2026-04-24 | **New columns:** `vendor_profiles.market_box_frequency` (TEXT NOT NULL DEFAULT 'weekly', CHECK weekly/biweekly), `market_box_subscriptions.pickup_frequency` (TEXT NOT NULL DEFAULT 'weekly', CHECK weekly/biweekly). **Function rewrites:** `create_market_box_pickups()` trigger reads `pickup_frequency` to set interval (7d weekly / 14d biweekly) and pickup count. `check_subscription_completion()` now counts actual pickup rows instead of `term_weeks` — fixes premature completion when extension weeks exist. `vendor_skip_week()` uses frequency-aware interval for extension scheduling. `subscribe_to_market_box_if_capacity()` adds 8th param `p_pickup_frequency TEXT DEFAULT 'weekly'` — note: 7-arg overload remains alongside (CREATE OR REPLACE matches by typed signature, not by defaults), future cleanup needed. Applied to all 3 envs (Prod 2026-04-26). Structured tables in SCHEMA_SNAPSHOT.md not yet regenerated. |
