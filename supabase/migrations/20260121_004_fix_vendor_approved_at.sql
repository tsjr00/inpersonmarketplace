-- ============================================================================
-- Fix Migration: Add approved_at column to vendor_profiles
-- ============================================================================
...
-- Backfill approved_at for existing approved vendors
-- Use updated_at as a reasonable approximation, or created_at if no updates
...
CREATE INDEX IF NOT EXISTS idx_vendor_profiles_approved_at
  ON vendor_profiles(approved_at)
  WHERE status = 'approved';

-- Update the scan function to handle NULL approved_at gracefully
...
BEGIN
  ...
  FOR v_vendor IN
    SELECT
      ...
      vp.last_active_at,
      vp.first_listing_at,
      COALESCE(vp.approved_at, vp.created_at) AS approved_at,
      vp.created_at
  ...
  LOOP
    ...
    -- Days since approval (using COALESCE to handle NULL approved_at)
...
COMMENT ON COLUMN vendor_profiles.approved_at IS
  'Timestamp when vendor was approved. Used for activity monitoring.';
