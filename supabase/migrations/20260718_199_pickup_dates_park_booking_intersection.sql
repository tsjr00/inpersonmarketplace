-- ============================================================================
-- Migration 199: get_available_pickup_dates — paid FT parks sell ONLY on
--                PAID booking dates (T5 / P10 stage (b), relaunch-blocking)
-- ============================================================================
...
-- ============================================================================
-- 1) FM regression: pick any FM listing id — result must be IDENTICAL to
--    pre-migration output (run before + after if possible):
--      SELECT * FROM get_available_pickup_dates('<fm_listing_id>');
--
-- 2) FT paid-park behavior: for a truck with an ACTIVE recurring schedule at a
--    park_mode='paid' market and exactly ONE paid booking date:
--      SELECT pickup_date, is_accepting
--      FROM get_available_pickup_dates('<ft_park_listing_id>');
--    → ONLY the paid booking_date(s) return. A pending_payment occurrence's
--    date must NOT return until it is paid.
--
-- 3) FT free-park behavior: a listing at a park_mode='free' market returns the
--    same dates as pre-migration (schedule-driven, no booking requirement).
--
-- 4) Plan check (optional): EXPLAIN ANALYZE on (1) and (2) — the FM plan must
--    not reference park_spot_bookings at all.
-- ============================================================================
