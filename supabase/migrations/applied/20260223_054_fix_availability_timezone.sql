-- ============================================================================
-- Migration 054: Fix timezone bug in get_available_pickup_dates()
-- ============================================================================
-- CURRENT_DATE returns UTC date. When it's Sunday 6pm CT, the server sees
-- Monday in UTC. The FT same-day filter (ds.potential_date = CURRENT_DATE)
-- then looks for Monday schedules instead of Sunday — returning zero rows.
-- Fix: use market timezone to determine "today" and season date checks.
-- Also affects season_start/end checks and event date filtering.
-- ============================================================================
...
BEGIN
  ...
  matched_dates AS (
    ...
    WHERE EXTRACT(DOW FROM ds.potential_date)::INTEGER = ls.day_of_week
      ...
      -- Food truck parks: today only (same-day ordering) — using market's local today
      -- Food truck events: FM-style advance ordering (7-day window)
      AND (ls.vertical_id != 'food_trucks' OR ls.market_type = 'event' OR ds.potential_date = ls.local_today)
  ),
  with_cutoff AS (
...
COMMENT ON FUNCTION get_available_pickup_dates IS
  ...
  'FT parks: today only (market-local), 0 cutoff (accepts until truck closes), REQUIRES vendor attendance record. '
