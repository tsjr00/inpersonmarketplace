-- ============================================================================
-- Migration 080: 48-hour lead time for catering orders
-- ============================================================================
-- Catering items (advance_order_days > 0) now require a 2-calendar-day
-- minimum lead time. This enforces the 48-hour advance notice rule:
--
--   Regular items (advance_order_days = 0): today only (unchanged)
--   Catering items (advance_order_days >= 2): [local_today + 2, local_today + advance_order_days]
--
-- The advance_order_days dropdown in ListingForm starts at 2 (not 1),
-- so the window always has at least 1 valid date.
-- ============================================================================
...
COMMENT ON FUNCTION get_available_pickup_dates IS
  ...
  'FT parks regular (advance_order_days=0): today only, 0 cutoff (accepts until truck closes), REQUIRES vendor attendance record. '
  'FT parks catering (advance_order_days>0): 2-day minimum lead time (48hr rule), up to advance_order_days ahead, REQUIRES vendor attendance record. '
