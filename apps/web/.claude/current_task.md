# Current Task: A1 + A2 shipped to staging (0cdda987). S3-1 built + gate-green, UNCOMMITTED. ⭐ NEXT SESSION STARTS WITH ADMIN-SCOPE ENFORCEMENT (S4-2 + S5-2) — see priority block below. Then commit S3-1, then combined PROD push = relaunch.

## 🔴🔴 START HERE NEXT SESSION — ADMIN-SCOPE ENFORCEMENT (S4-2 + S5-2) — HIGHER PRIORITY (owner flagged 2026-07-21)
...
## 🟢 S3-1 — Phase-5 vendor double-pay guard (2026-07-21) — DONE, gate-green, UNCOMMITTED
...
**Files:** payout-reconcile.ts (new) · expire-orders/route.ts · market-box-errors.ts · payout-reconcile.test.ts (new) · logic_testing_round_research.md. Suite 66/1722 green, tsc 0, eslint 0. NOT hook-protected (expire-orders not on the list; helper deliberately NOT in payments.ts to avoid the gate). **NEXT: user commit+push approval (separate).**
...
- **local main = origin/staging = `0cdda987`** ("fix(orders,checkout,cart): A2 money-fix batch …") — A1 (`1cd12b93`) + A2 (`0cdda987`) both committed + pushed to staging 2026-07-20 (A2 ref-update 1cd12b93..0cdda987 verified; pre-commit + pre-push build/Playwright green). Staging IS in sync with main.
- **PROD origin/main = `62b686f7`** — unchanged (~80 behind). Combined prod push = RELAUNCH (own approval, 9PM–7AM window), after USER staging test. Migs 184→204 apply IN ORDER before that push. NOTE: the logic-fix batches (A1/A2) are CODE-ONLY — no new migrations; the 184→204 list is unchanged from the day-8 state.
