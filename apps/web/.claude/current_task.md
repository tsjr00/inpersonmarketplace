# Current Task: 🟢 Work ON STAGING (origin/staging `be80810b`); ⚠️ ONE HELD LOCAL COMMIT `7c4c4ff8` (#3 admin activate fix) NOT pushed — user holding for a COMBINED push after testing. prod `62b686f7` unchanged (~110 commits + migs 184→211 behind). ⭐ BIGGEST NEXT = THE RELAUNCH (migs 184→211 to prod). ✅ Migs 206–211 all APPLIED to Dev+Staging (user). 🔨 IN PROGRESS: item #1 (park-required docs for existing/recurring vendors) — design decided, awaiting a privacy sub-decision (see below), NOT built yet.
...
### 🔨 ACTIVE ITEM #1 — park-required docs reaching existing/recurring vendors (DESIGN + one open privacy decision; NOTHING BUILT):
**The gap:** operator adds a required doc (incl. custom "Other", e.g. "Generator safety inspection", mig 206). A vendor already approved with a RECURRING spot (a) isn't notified of the new requirement, and (b) has no clear place to upload it. VERIFIED: vendors CAN upload custom docs only as **"Other" certifications** (label+file) on their profile; standard permits go to the PRIVATE vendor-documents bucket, but **certifications go to a PUBLIC bucket + public URL**, and the cert NAME shows as a badge on the PUBLIC vendor profile (`vendor/[vendorId]/profile/page.tsx:1453-1464`). The operator's vendor-docs view does NOT show certifications today (`vendor-docs/[vendorProfileId]/route.ts:105` selects category_verifications+coi only).
**DECIDED:** use the certification upload as the home for park-required docs. Build = notify affected vendors (active booking/recurring hold at that park) when required_docs changes + surface certifications (incl. Other) in the operator's vendor-docs view + connect the copy.
**⏳ OPEN PRIVACY DECISION (user's 2026-07-26 direction, resolve first):** user is OK with the public knowing a vendor HAS a cert, but NOT with the app revealing the cert's DETAILS/contents. Direction: **REMOVE the public/semi-public display of certification details** from the vendor profile (buyers can dig for actual contents elsewhere — "we don't want to be the point of information distribution"); MAYBE expose only a cert NUMBER if it's already publicly available info elsewhere. NOTE: this ALSO affects EVENT functionality — user wants a CONSISTENT approach across park docs + events. Next session: (1) settle exactly what stays public (badge? number? nothing?) + move cert files to a PRIVATE bucket/signed-URL like permits; (2) then build the #1 notify+operator-view work under that model.
