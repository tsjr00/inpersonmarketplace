# SESSION 2026-09-03 — P2–P6 ALL ON STAGING (`7bde9196`); **P1 HOST MENU PARE-DOWN BUILT, uncommitted; MIG 241 ⏳ applied NOWHERE (paste-and-go)**. Gates after P1: tsc ✓ · vitest 2122/2122 (+7 menu-pare, +4 pair guards) · lint 0 err.
- **P1** (owner: all 5 decisions in decisions.md "Host menu pare-down"; #3 = sequence protection, no consent machinery): mig `20260903_241_event_menu_host_status.sql` (`evl.host_status` approved|declined, default approved, inert) + `lib/events/menu-pare.ts` (validatePare, MIN_KEPT_ITEMS=2, 7 spec tests). Select route: GET += per-vendor `proposed_items` (evl+titles) + `can_pare`/`min_kept_items`; POST accepts `pared_listing_ids` map — first-round only, selected vendors only, validatePare — applies THE PAIR (evl declined + listing_markets delete). Readers filter declined: shop-data · Public Event Page · vendors-with-listings · admin menu display; admin RESTORE rebuilds links from approved rows only. Vendor GET += `event_menu`; Vendor Event Page "Your event menu" block (approved list + struck pared + "N of M approved" line — the informed-consent surface, renders before the pay button). Select page: proposal chips tap-to-remove (floor-aware, read-only verdicts in change mode; catalog fallback for legacy no-proposal rows). 4 flow-integrity pair guards; 14_Events item 9. NOTE: min-2 floor means the shop fallback (no-evl vendors) can never trigger from a pare.
- ⏳ OWNER: paste mig 241 on Dev + Staging before testing P1 (PASTE-AND-GO, inert; post-checks in header). Prod order: 238→239→240→241.
- Owner retest (after mig + push): select page first round → tap items off (floor stops at 2; 2-item menus untappable) → confirm → shop/public page lack pared items → Vendor Event Page shows "N of M approved" with struck items BEFORE paying → change-selections round shows verdicts read-only, no new paring.
- **P6 v2** (owner "build v2 of the schedule if we have all we need" — all inputs verified in snapshot): NEW `lib/vendor/week-strip.ts` (pure `assembleStrip` + loader; queries mirror availability.ts patterns, date helpers imported from it — `shiftDate` now exported) + NEW `api/vendor/week-schedule` (client sends local `start` date, no tz hardcode) + NEW `WeekAtAGlance.tsx` ("Your next two weeks": date list, paid park days outrank weekly projections (dedupe), SELECTED events on real dates, **blackout days struck "Skipped — you're at {event}"**, **manager-cancelled days struck**; renders nothing on load/error). REPLACES the v1 DOW grid on /vendor/markets — v1 wiring removed (attending flag, grid transform, type additions); dashboard rename + PickupScheduleGrid (public profile) untouched. NOT in v2 (noted in module doc): 'special' make-up overrides, standing-occurrence pay-by flags. Map: 11_Vendor_Orders week-strip block + stamp; INDEX row.
- Owner retest: /vendor/markets "Your next two weeks" — committed days only, selected event on its date; blackout day struck with event name (needs the near-dated conflict fixture); cancelled market day struck.
- **P6** Week-at-a-glance (owner "then we will do the next one"): `/vendor/markets` gets a top "📅 Your week at a glance" section reusing the public profile's `PickupScheduleGrid`, fed ONLY real commitments — fixed-market windows with an active vms row (NEW `attending` flag per schedule from `api/vendor/markets`, built from attendanceData schedule_ids), vendor's own active private-pickup windows, and events where stage === 'selected' (shared classifier; awaiting/benched excluded — same accuracy rule as the pill). Dashboard card renamed "Manage Locations" → **"Locations & Schedule"** (plain string, no locale key; no test pinned it). Known v1 limit noted in-code: DOW grid can't show blackout-skipped days; date-strip = v2. Files: api/vendor/markets/route.ts · vendor/markets/page.tsx · markets/types.ts (Schedule.attending, Market.market_schedules) · vendor/dashboard/page.tsx.
- Owner retest: /vendor/markets top grid shows attended market days + private windows + selected event only; dashboard card reads "Locations & Schedule".
- **P2** Admin events Vendor Invitations table (owner "p2"): API `api/admin/events/route.ts` mv select += `is_backup, organizer_selected_at, standby_opted_in_at` (snapshot re-read same turn) + map passthrough; page renders a stage chip beside the response badge via the SHARED classifier — Selected (green) / Bench · standby (blue) / Awaiting selection (grey); non-accepted rows unchanged. Classifier guard now pins 4 surfaces. Map 19_Admin untouched (display-only, page purpose unchanged). Owner retest: admin events → approved event → Vendor Invitations table shows the chips matching the organizer roster.
- **P3** Organizer dashboard per-truck roster (owner "go with full names"): `event-manager/[id]/dashboard/page.tsx` — market_vendors query widened to ALL stages (embed hint `vendor_profiles:vendor_profile_id(profile_data)`; accepted-only counts preserved), roster sorted selected→bench→said-yes→invited→declined→withdrawn, "Your trucks (N invited)" card between Event details and waves, badges from the SHARED `vendor-stage.ts` classifier (+standby annotation), link to Select page. **DECISION logged (decisions.md): full names to organizer at every stage; organizer-identity masking toward vendors UNTOUCHED** (one-way disclosure). Flow-integrity classifier guard now pins 3 surfaces. Protected-path hook fired on decisions.md → verified additive-only → retried per protocol.
- Owner retest (with P4/P5 lines): organizer dashboard shows every invited truck with stage badge matching their real state (their test event: 3 invited / 1 declined / 2 accepted / 1 selected / 1 standby-bench).
