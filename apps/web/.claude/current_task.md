...
- **Item 3 — DONE, UNCOMMITTED, gates green (tsc 0, lint 0, vitest 1501/1501).** Added `describe('Phase E season flow integrity')` to `flow-integrity.test.ts` — 8 static contract tests: webhook→confirm_season_paid; cron Phase 18 confirm+cancel RPCs; Phase 16 excludes group_id (F1); season-booking→book_season_atomic; settlement route writes season_settlement booth_credits; vendor cancel uses managerReceivesCents basis; settlement card→/settlement; cancel button→booth-groups/cancel. (Pre-existing unused-var warning at line 260 left untouched — out of scope, non-blocking.) **Awaiting user review → commit → push staging.**
- **Next:** prod push (Items 1–3, 9 PM–7 AM CT, user call). Item 4 (credit redemption + make-up-dates + upgrade source, money path) = own session.
