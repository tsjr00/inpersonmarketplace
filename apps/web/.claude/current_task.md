...
**REMAINING broad lockdown (Tier 2+, separate):** vendor-lifecycle + money routes (vendors/[id]/{approve,reject,fast-track,verify,verify-coi,verify-category,event-approval}, events/[id]/payments) → verifyAdminScope(target vertical); then events/markets/listings/order-issues/knowledge data routes; then a structural test forcing every api/admin/** mutation through a sanctioned gate. S4-3 regional_admin/regional-manager still separate.
