# Current Task: A1 + A2 + S3-1 ALL shipped to staging (631fb36e). Logic-testing fix arc COMPLETE except admin-scope. ⭐ NEXT SESSION STARTS WITH ADMIN-SCOPE ENFORCEMENT (S4-2 + S5-2) — see priority block below. Then combined PROD push = relaunch.
...
**⭐ ACCESS-SAFETY (owner caveat — additive-first, never lock out tsjr00+Jen):** verified on STAGING BEFORE pushing the strict helper — both accounts `passes_strict_platform_check=TRUE` (role=platform_admin) AND vertical_admins of all 3 verticals (double-covered); no other admin accounts exist. The verification query is in the session log (user_profiles/vertical_admins join) — **RE-RUN IT ON PROD after mig 204 lands, must be TRUE for both, BEFORE the strict helper goes live on prod.**
**SCOPE:** closes S4-2 (6 verifyAdminScope routes + errors route) + S5-2 (fee-override). The other ~42 bare-hasAdminRole routes stay unscoped = separate larger "full lockdown" build (money routes first: events/[id]/payments, settlement, backfill-stripe-fees) + structural test. S4-3 regional_admin/regional-manager still separate.
