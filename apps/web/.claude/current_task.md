...
- ⬜ **Group D scope-by-vertical (add verifyAdminScope):** errors/[id], order-issues, events/route, events/[id]/{route,generate-waves,invite,rematch,repeat}, vendor-activity/{settings,flags/[id]}, knowledge (per-vertical TBD). Each needs the target's vertical source identified.
- ⬜ **Group E platform-only candidates (DECISION):** backfill-stripe-fees (platform-wide Stripe reconciliation), users/[id] (user mgmt — users aren't vertical-scoped). Lean hasPlatformAdminRole (platform-only) for both — confirm before building (locks out vertical admins by design). Exempt: login.
