...
**REMAINING broad lockdown (Tier 3+, separate):** events/markets/listings/order-issues/knowledge/vendor-activity DATA routes (cross-vertical read/write, lower risk than escalation/money) → scope; then a structural test forcing every api/admin/** mutation through a sanctioned gate; then the flagged follow-up (admins POST writes role:'admin' not 'platform_admin'). S4-3 regional_admin/regional-manager still separate.
