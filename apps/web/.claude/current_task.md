...
**REMAINING:** **(E) Settlement** — `booth_credits` ledger migration (166) + season-end settlement panel (rollover-credit + booth-upgrade, O4) + vendor self-cancel (O5: pre-start full credit / post-start penalty+credit) + cancelled-day counter (derive `market_date_overrides ∩ group weeks`; late-buyer cutoff = `purchase_date`). **(F)** flow-integrity tests (season-checkout→webhook→children-paid; settlement). Minor TBC: after-start penalty % (25% default); partial-week picker UI. **Now user-testable** once committed+deployed (manager: dashboard "Season Pre-sales" card → create + open; vendor: market booth-booking page → "Reserve a whole season").
