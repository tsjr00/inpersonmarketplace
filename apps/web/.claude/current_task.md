...
**NEXT SESSION — START HERE (Phase E build IN PROGRESS).** Design complete (O1–O6, plan §8). **✅ BACKEND BOOKING PATH DONE + GREEN** (tsc clean, vitest 1493/1493): migs **164** (foundation: market_seasons + booth_booking_groups + weekly_booth_rentals.group_id + markets.schedule_confirmed_at) + **165** (book_season_atomic, all-or-nothing wrapper) applied Dev+Staging; helpers `lib/markets/season-weeks.ts` + `season-booking.ts`; money path `payments.ts createSeasonBoothCheckoutSession` + `webhooks.ts handleSeasonBoothCheckoutComplete` (critical-path, **user-approved**) + 2 notif types `booth_season_paid_vendor/_manager` (tripwire bumped 80→**82**, user-approved); route `api/vendor/markets/[id]/book-season/route.ts` (gates, O6 presale window, season-vs-partial, late-buyer full-season, Stripe-failure cleanup). **ALL UNCOMMITTED** (+ migs 164/165 in `supabase/migrations/`, Prod pending).

**✅ (D) UI DONE + GREEN (tsc + eslint clean), 2026-06-25 — NOW END-TO-END FUNCTIONAL.** Manager API `api/market-manager/[marketId]/seasons` (GET list; POST create w/ derived market-days + 10% cap + O1 schedule-confirm gate via `markets.schedule_confirmed_at`; PATCH open/close prepay enforcing 60-day lead cap + one-season-ahead via uq index + 14-day grace close + set_cap w/ 15% ceiling) + vendor GET `api/vendor/markets/[id]/seasons` (open seasons + inventory + week counts). Components: `MarketSeasonCard.tsx` (wired into manager dashboard after cancel-date + ManagerJumpNav "Seasons" chip); `SeasonBookingSection.tsx` (vendor picker, wired into `[vertical]/markets/[id]/book/page.tsx` after BookBoothForm; renders only when seasons open; whole-season reserve → Stripe; **partial-week selection is a fast-follow** — backend supports it). **ALL UNCOMMITTED.**

**REMAINING:** **(E) Settlement — BACKEND DONE 2026-06-25 (tsc clean):** mig **166** `booth_credits` ledger (applied Dev+Staging, Prod pending); `lib/markets/cancelled-days.ts` (cancelled-day counter, pure + DB wrapper); `api/vendor/booth-groups/[groupId]/cancel` route (O5 vendor self-cancel → credit: pre-start full / post-start remaining−25%). **STILL TODO (next session):** manager **season-end settlement panel** (offer rollover-credit + booth-upgrade when cancelled days > cap — uses `getGroupCancelledDays` + `market_seasons.refund_cap_days`); **credit redemption** at booking (apply `booth_credits` balance → reduces vendorPays + the manager transfer — touches the money path, **per-file approval**); vendor-facing "cancel my season" button (calls the cancel route); season_settlement credit grant. **(F)** flow-integrity tests (season-checkout→webhook→children-paid; settlement). Minor TBC: after-start penalty % (25% default); partial-week picker UI. **Now user-testable** once committed+deployed (manager: dashboard "Season Pre-sales" card → create + open; vendor: market booth-booking page → "Reserve a whole season").
