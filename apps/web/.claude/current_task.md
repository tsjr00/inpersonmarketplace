...
**Tier-3 worklist (from the ALLOWLIST):** helper-swap (dead vertical_admins fallback, like Group A): listings/[id], markets/route, markets/[id], markets/[id]/manager. Scope-by-vertical (add verifyAdminScope): errors/[id], order-issues, events/route, events/[id]/{route,generate-waves,invite,rematch,repeat}, vendor-activity/{settings,flags/[id]}, knowledge (per-vertical TBD). Platform-only candidates: backfill-stripe-fees, users/[id]. Exempt: login.
