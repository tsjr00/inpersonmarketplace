...
**REMAINING:** **(D) UI** — Manager season UI (create season; derive+confirm market days [O1]; open/close prepay window enforcing MAX_PRESALE_LEAD_DAYS=60 + one-season-ahead; set refund_cap_days=10%/15%-ceiling) + O1 schedule-confirm gate (`markets.schedule_confirmed_at`); vendor season-picker UI that calls `book-season`. **(E) Settlement** — `booth_credits` ledger migration (166) + season-end settlement panel (rollover-credit + booth-upgrade, O4) + vendor self-cancel (O5: pre-start full credit / post-start penalty+credit) + cancelled-day counter (derive `market_date_overrides ∩ group weeks`; late-buyer cutoff = `purchase_date`). **(F)** flow-integrity tests (season-checkout→webhook→children-paid; settlement). **NOT user-testable end-to-end until the UI lands** (no way to create a season or invoke the route yet). Minor TBC: after-start penalty % (25% default).
