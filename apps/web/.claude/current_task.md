...
- **NEXT — Step 3:** make-up-date scheduling — manager API to insert a `'special'` `market_date_overrides` row (count-capped by potential_makeup_days, post-close, isMarketManager-gated, only while status=ended) + `booth_makeup_scheduled_vendor` notification (tripwire 84→85). Then Step 4 (settlement `'made_up'` resolution), Step 5 (tests), Step 6 (UI, deferred).
