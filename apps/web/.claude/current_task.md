...
**PART C COMMITTED `5cf7d64b`. PART D BUILT 2026-09-19 — UNCOMMITTED; commit BLOCKED by guardrail Rule L (6 migrations past the 251 stamp — a structured-tables refresh is due; the stamp may only move after a real rebuild).** Built: mig 257 file (PRE-CHECK FIRST; source CHECK + `related_cancel_date` + 2 partial unique indexes; snapshot changelog + column rows), `lib/markets/booth-cancel-credit.ts` (declared dates, per-day share, cap), `cancel-date-cascade.ts` path B credits one-off weeks per declared day + `market_date_cancelled_vendor` gains the amount (no new type), `weekly-rental/[rentalId]/cancel` route (manager cancels a PAID one-off week → credit remaining declared days, release redeemed credit, notify `booth_week_cancelled_by_manager` — tripwire 132→133 cited), `WeeklyBookingsList` Cancel-week control with reason, unit test `booth-cancel-credit.test.ts`, 2 flow pins, registry TR-091/092 + printable list, MESSAGE_TEMPLATES, map 12, design STATUS. tsc 0 · eslint 0 errors · vitest all green EXCEPT Rule L. **NEXT (owner, Dev): 257 pre-check → paste 257 → post-check → run the scoped delta query I give them (columns of market_vendors + booth_credits, booth_credits indexes + CHECK, signatures of the 5 changed functions) → I rebuild those structured sections + move the stamp to 257 → commit part D → Staging: 257 pre-check + paste → ONE staging push (B+C+D) → owner retests TR-078–092.**

**PART B COMMITTED `09143e29`. PART C BUILT 2026-09-19 — UNCOMMITTED, gates green (tsc 0 · eslint 0 errors · vitest 90 / 2244).** Owner "go on that" after the ANTI-BLOAT pass (7 notification types → 1 `booth_number_changed` + one hold-moved line on the manager's existing paid confirmations; two cuts reversed to avoid confusion: manager must learn a moved hold, frozen field says why inline). Built: `lib/markets/booth-assignment.ts` (payment writes the pin / transfers a yielded soft pin / never moves an assigned one), `webhooks.ts` ⚠ two approved blocks (one-off + season, after the flip, try/catch, `ERR_WEBHOOK_020` new catalog code) + `holdMovedFromName` on the manager payload, `season-notifications.ts` opts, `lib/markets/booth-freeze.ts` + freeze on `vendor-booth`, `vendor-tier`, `weekly-rental` (409 `ERR_BOOTH_ASSIGNED_FROZEN`), `weekly-rental` pre-check + notification, `VendorBoothList` locked field + inline reason + one-line hold explanation, notification tripwire 131→132 (cited), MESSAGE_TEMPLATES, map 10/12, registry TR-088–090 + printable list, design STATUS/BR-8 revised. **NEXT:** owner's word to commit part C → part D (mig 257 PRE-CHECK FIRST + FM per-day credit on cancel-date + manager cancel-paid-week route, BR-9/10 — the existing renter notice gains the amount; one new type for the manager cancel) → ONE staging push → owner retests TR-078–090.

**PART A PUSHED TO STAGING `041fd629..80e66d59` (Playwright 49/1 skip); mig 256 ✅ Dev + Staging (identical fingerprints, snapshot row). PART B BUILT 2026-09-19 — UNCOMMITTED, gates green (tsc 0 · eslint 0 errors · vitest 90 / 2242).** Owner authorized the three pin changes ("have you reviewed… if so proceed"). Built: BR-1 gate at ALL managed markets (`schedules/route.ts` `managedJoinBlocked` — charging test gone, pending-vs-apply copy, middle path + terms path DELETED; selector no longer collects terms), `lib/markets/booking-gates.ts` (BR-1/13/4) wired into `book`, `book-season` and the book page (apply door · `DeclareDaysGate` inline day picker · locked size + pinned number in `BookBoothForm`), Apply size request (`ApplyToMarketButton` + market page tiers + POST stores `requested_inventory_id` tolerantly), approval sets size + booth # + note (`vendor-approval` route + roster form; `vendor_market_approval_granted` message extended), roster GET returns `requested_size_label` (tolerant query). Pins: 3 rewritten + 2 new (part B). Docs: registry TR-083–087 + TR-076/077 superseded, printable list (WHAT'S NEW part B, Group 2 rebuilt, Group 3 +2), codebase map 11/12, design STATUS. **NEXT:** owner's word to commit part B → part C (payment writes the assignment — `webhooks.ts` needs per-file approval; freeze; notifications) → part D (mig 257 + credits) → ONE staging push (or push B now if the owner wants to retest early).
...
## 🧪 TESTING STRUCTURE + BUNDLE TEST ROUND — 2026-09-14 early (after the prod push)
- **New:** `docs/testing/` (README · CHARTERS · TEST_REGISTRY · OBSERVATIONS) — the tracking mechanism; kickoff reads the registry, close updates it (CLAUDE.md item 8, PROCESSES Protocol 1 + 6). Seeded from `apps/web/.claude/testing_status_research.md`.
...
- **Owed by owner:** (1) the A–D staging test document (only section D items 11–17 were pasted; A–C never reached the repo) so tonight's notes can be matched to their original tests; (2) screenshot of the bundle-card overflow.
- **Ruled, build pending:** multi-location declaration gates schedule conflicts on BOTH verticals (decisions.md 2026-09-13). Route (`schedules/route.ts:305-309, :578`) + trigger (mig 247 early return) + a business-rule test. Pull `pg_get_functiondef` on each env before rewriting the trigger.
- **Not yet proven on Prod by behaviour:** a real checkout + ack/fulfill (no Stripe-enabled vendor yet).
- **Suggested order next session:** item-11 build → market-box vendor-visibility defect (backlog, ranked highest) → browse filter-surface design (present options) → admin stuck-orders destination → bundle overflow once the screenshot arrives.
...
- Week strip v2.1 committed LOCAL `53a61664` (owner: "commit local then next item"). NOT pushed.
- **ADMIN PHASE 6 EVENTS BOARD BUILT (uncommitted)** — full 22-capability inventory + design + build log in `.claude/admin_phase6_events_board.md`. EventsAdminPage.tsx (board: stage groups + jump-nav + stage summaries; detail panel moved VERBATIM); route thin wrapper; map 19 phase-6 block; 2 guards repointed (location-only). Gates: tsc ✓ · 2150/2150 ✓ · lint 0 err.
...
# (session working notes below — superseded by the checkpoint above)
...
- A1 PUSHED `3e3cdaa9` (first push attempt died on a network reset AFTER build+Playwright passed; retry re-ran gates + verified ref-update — chain left on staging, recovered per Rule 1).
- **A2 BUILT** (owner approved vendor-limits diff + tripwire 121→122): mig `20260904_242_vendor_vip_customers.sql` (⏳ applied NOWHERE — paste-and-go, inert; prod order 238→…→242) + snapshot row. vendor-limits.ts `vipCustomers` 0/10/25 (protected-file hook fired → verified approved diff → retried). NEW `api/vendor/vip-customers` POST (cap-checked add + `vip_added` notification, dedup key, 23505 = idempotent) / DELETE (silent). Customers report: rows += is_vip, response += vip {used, limit}; Insights card: slot meter chip + ☆/⭐ toggle per row (disabled at cap; free tier sees "Pro feature"). Orders API += customer_is_vip → OrderCard ⭐ VIP chip beside the segment chip. Favorites page: "⭐ You're a VIP" on vendor cards. customer_milestone nudge now links to insights + says "Make them a VIP?". MESSAGE_TEMPLATES + 2 new flow-integrity guards + map A2 block.
- **Mig 242 = Dev + Staging ✅ (owner 2026-09-04); Prod PENDING** (paste order 238→…→242). **Snapshot structured tables rebuilt → stamp = 242** (Rule L fired on 242's CREATE TABLE — first firing since the rule's 2026-08-30 creation; owner ran the scoped live queries on Dev, output verified = DDL exactly; vendor_vip_customers added to Tables/Columns/FKs/Indexes, host_status added to evl columns). Transparent test update: badges.test.ts milestone-nudge actionUrl → /vendor/insights (owner-approved A2 product change, not test-bent-to-code). Suite 2125/2125.
- Owner retest A2: insights star toggle (cap enforced; free vendor sees no VIP column) → buyer gets "You're a VIP" push/in-app → Favorites badge → order card ⭐ VIP.
- A2 committed LOCAL `d09707d3` (not pushed — owner call). **A3 BUILT, uncommitted** (Q4: "8am daily during the season, only if something to see" → 8am local content-gated; tripwire 122→123 owner-approved "yes, up the tripwire"): NEW `lib/notifications/vendor-digest.ts` — ONE send site (flow-integrity-pinned), per-(buyer,vertical) grouping over followed ∪ VIP vendors' new published listings (24h), 8:00–8:59 local window via vendors' home-market tz (Chicago fallback idiom), once-per-day notifications-table dedup, content gate = season gate. Wired as an independent block in the HOURLY surveys cron (NO vercel.json change — guard forbids a digest cron entry). `followed_vendor_digest` type + registry + MESSAGE_TEMPLATES; maps 17_Crons blocks-hosted note + 18_Notifications (123 types) + stamps. v1 limit noted: "new" = created_at (draft-published-later missed). Gates: tsc ✓ · 2126/2126 · lint 0 err.
- **PHASE A COMPLETE** (A1 staged `3e3cdaa9` · A2 local `d09707d3` · A3 local `826b7623`).
- **B1 DISCOUNT PLUMBING BUILT** (owner approved the 5-hunk checkout/session diff — hook verify-retried): mig `20260904_243_vendor_offers_discount_plumbing.sql` (⏳ applied NOWHERE; vendor_offers + order_items.discount_cents/offer_id + orders.discount_cents; INERT — table empty, columns default 0) + NEW `lib/loyalty/offers.ts` (computeSpendThresholdDiscount VIP-gated + parseSpendThreshold bounds Q6-proposed 5–25%/$15–$100 + allocateDiscount floor+remainder conservation; 11 spec tests incl. the owner's $30→10% example). checkout/session: discount map (spend_threshold × VIP only, inert) → pricingItems NET per-item qty-folded (SAFE: pricing.ts rounds on the SUM, :124-136 verified) → orderItems store `subtotal_cents = net`, `unit_price_cents = list`, discount_cents + offer_id → discounted Stripe line consolidated "— VIP deal" (no negative lines) → orders.discount_cents. **Refund paths untouched BY DESIGN** — guard forbids discount references in reject/resolve/cascade (net-storage carries it). Punch card application deferred to its build (needs redemption state). Gates: tsc ✓ · 2137 green + **Rule L red on 243's CREATE TABLE (owner-gated, expected — same flow as 242)** · lint 0 err.
- ⏳ OWNER: paste **mig 243** Dev + Staging (PASTE-AND-GO, inert) → run the scoped rebuild queries (vendor_offers columns/constraints/indexes) → Claude rebuilds structured tables + stamp → suite green → B1 commit.
- NEXT after B1 commit: punch card build (redemption state + vendor config UI + Q5 bounds confirm) → spend-threshold UI (Q6 confirm). Q5/Q6 proposals live in offers.ts constants pending owner confirm.
- Owner A3 test (staging, manual): `curl -H "Authorization: Bearer $CRON_SECRET" <staging>/api/cron/surveys` during the 8am CT hour after a followed vendor posts a new listing → ONE digest notification; second invoke = no duplicate.
...
## Git / env — VERIFY, don't trust
- `origin/staging` = local `main` = **`8df93d4b`** (5 pushes today, Playwright 49✓ each; every commit `aff15069`→`8df93d4b` is this session's event-UX batch). Tracked tree = only this wrap's docs uncommitted. PROD **`e946c2c0`** — 52 commits behind; owes migs **238→239→240→241 IN ORDER** before the code push (window 21:00–07:00 CT). No emergency.
- **Mig 241 = Dev + Staging ✅ (owner 2026-09-03); Prod PENDING** — file stays in `supabase/migrations/` root. Snapshot changelog row flipped. ⛔ 225 still never-paste (rollback target for 234).

## ⚡ WHAT SHIPPED (owner testing thoughts → all 6 built + staged same day; plan/build record: `event_ux_findings_2026-09-03_plan.md`)
1. **The through-line — accepted ≠ selected, ONE classifier** `lib/events/vendor-stage.ts`, flow-integrity-pinned across FOUR surfaces: **P4** "Vendors who said yes: N of M invited" copy (Vendor Event Page) · **P5** locations pill (Said yes — awaiting selection / On the bench / Attending only when selected) · **P3** organizer per-truck roster ("Your trucks (N invited)" card; **DECISION: full names at every stage, one-way — organizer-identity masking toward vendors untouched**) · **P2** admin invitations chips (Selected / Bench · standby / Awaiting selection).
2. **P6 week-at-a-glance, v1→v2 same session**: v2 = date-based next-14-days strip (`lib/vendor/week-strip.ts` pure assembleStrip + loader mirroring availability.ts patterns · `api/vendor/week-schedule`, browser sends local start date — no server tz guess · `WeekAtAGlance.tsx` on /vendor/markets). Paid park days outrank weekly projections; SELECTED events on real dates; **blackout days struck "Skipped — you're at {event}"; manager-cancelled days struck** — entries explained, never dropped (7 spec tests). Dashboard card renamed **"Locations & Schedule"**. v1 DOW-grid wiring removed; PickupScheduleGrid untouched on the public profile. v2.1 backlog: make-up-day overrides, pay-by flags.
3. **P1 host menu pare-down — designed WITH owner (5 decisions in decisions.md) + built (mig 241)**: all organizers · pare locks when the shop publishes (FIRST selection round only) · min 2 kept · activated backups never pared · NO consent machinery — the SEQUENCE protects fee vendors (payment arms after selection; the Vendor Event Page "Your event menu" block with "N of M approved" + struck items renders BEFORE the pay button). ⚠ **THE PAIR** (guarded ×4): evl.host_status = display half, `listing_markets` = sell half (cart validates there) — pare declines + deletes together; every menu reader filters declined; admin restore rebuilds from approved rows only. Select page: tap-to-remove proposal chips (floor-aware; read-only verdicts in change mode). NOTE: select GET's `catering_items` = whole eligible catalog; the real proposal = new `proposed_items`.
Gates at wrap: tsc ✓ · vitest **2122/2122** (2099→2122: +5 classifier, +7 week-strip, +7 menu-pare, +4 pair guards, +2 others) · lint 0 errors. Maps: 14_Events items 8+9, 11_Vendor_Orders week-strip block, stamps + INDEX (also fixed a pre-existing 14_Events stamp↔INDEX drift).

## 🧪 OWNER RETEST (staging `8df93d4b`, after Vercel Ready + HARD refresh)
1. **P1 loop**: select page first round → tap items off a 3+-item truck (floor stops at 2; 2-item menus untappable) → confirm → shop + public page lack pared items → that truck's Vendor Event Page shows "approved N of your M" with struck items BEFORE paying → change-selections round = verdicts read-only.
2. **Week strip**: /vendor/markets "Your next two weeks" — committed days only, "booked" tag on paid park days, selected event on its date. Struck lines need fixtures (blackout: weekly schedule on DOW X + event on a DOW-X date ≥10 days out → accept; cancelled: manager cancels a date).
3. **Stage surfaces**: pills (accepted ≠ Attending) · organizer roster badges · admin chips · "Vendors who said yes" · dashboard card "Locations & Schedule".

## ▶ NEXT SESSION queue (standing items unchanged from 08-31)
1. Owner retest results → fixes. **Parked decision owner is thinking on: money-moves-only-on-activation scope** (self-serve-only vs admin-assisted where the organizer pays a deposit — record is tier-silent; evidence laid out in chat 2026-09-03; deposit decisions s63/s71 never superseded).
2. Event-gate fixes A+B (Open Pre-Orders while held + ready-copy) — proposed 08-31, STILL awaiting go · vendor-docs crash STILL awaits evidence (Vercel log line or the 3 SQLs) · admin phase 6 events pipeline board on go (P2's chips fold into its design) · Protocol v6 C/D/E money tests (the ≤7-day event fixture unlocks these + blackout assertions).
3. Prod push when ready: migs 238→239→240→241 pasted IN ORDER first, then `git push origin main` in window; then bookkeeping batch (move applied files, flip snapshot rows).
4. Housekeeping candidates (owner OK needed): delete stale `current_task.new` (407KB mid-August copy) + root `.claude/current_task.md` (1-line stale) · vault still ~100+ commits stale (C6 discussion) · Google Vision billing STILL OFF (carry-over since 08-16).
---
...
8. ✅ Fulfill hook — owner approved exact diff, then REVISED once (bare after() throws with no request scope → money-authorization harness aborted a fulfill between status write and transfer) → `scheduleBuyerAchievementEvaluation()` wraps after() in try/catch; guards added
9. ✅ COMMITTED `c29d6734` on local main (35 files; bookkeeping + Layer 1 in ONE commit — pre-commit hook pairs any staged migration file incl. moves with a snapshot edit, so the split was impossible without a token edit). **NOT PUSHED — owner holding until ST-1…26 staging tests reconcile.** origin/main = origin/staging(+1 docs) = bfc60dfd.
10. ✅ 236 on Dev+Staging (owner 2026-08-25) — snapshot row flipped (Prod PENDING; file stays in root until prod). ⬜ Then staging push chain (main → staging) + Tier-1 list: Favorites page badge card, vendor order-card chip, fulfill an order → buyer push + vendor in_app milestone at 4th order.
...
## STAGING TEST RESULTS — owner 2026-08-25 (ST-19…26)
PASS: ST-21 (background-check notice + instructions, set after invite/before acceptance) · ST-22 (child-safety clause present) · ST-25 (waves explainer above capacity dropdown) · ST-26 (checkout "Card processing (1.5% share…)" + HIW "How we get paid" + "What selling costs").
NOT TESTED YET: ST-23 (not-eligible badge — needs fresh unapproved applicant) · ST-24 (fee-card reuse button styling).
COULD NOT TEST: ST-19 — owner found no UI mechanism to un-cancel an event (refusal message untestable from UI).
ST-20 PASS w/ FINDINGS (city edit on live event): message + vendor in-app ✓. Owner then had the vendor cancel participation with 1 pre-order placed →
  (a) vendor cancel after an organizer change must NOT count against vendor rating/reliability — verify what the cancel route increments;
  (b) buyer re-confirm notice said "your order still stands" although the vendor had just cancelled (logic may be right, language wrong: must allow for order cancelled / send them back to the event order page to re-check vendors + offerings);
  (c) the confirm page repeats "order stands";
  (d) status copy inconsistency on the same event: organizer dashboard "0 of 3 vendors confirmed, 1 pre-order" · event page "vendors being confirmed" · attendee shop "no vendors confirmed" · select page "no vendors responded" — owner: detail, later.
### Code findings on the ST-19/20 items (2026-08-25, verified)
- ST-19: admin UI shows NO status action for a cancelled event (only the overlay badge, admin/events/page.tsx:795; lifecycle buttons :853-908 + Cancel :916 are live-status only). The route's un-cancel logic (admin/events/[id]/route.ts:126, :213-268) refuses when refunds/emails happened but ALLOWS the misclick case — a recovery path with no door (address-deadlock class). Option: "Restore event" button on cancelled events → makes ST-19 testable + closes the gap.
- ST-20(a): vendor event withdrawal does NOT touch reliability/rating: cancel route never calls increment_vendor_cancelled (only reject/route.ts:264 does); counter feeds matching only (event-actions.ts:272, admin/events/route.ts:140); average_rating = order_ratings only; quality scans don't count cancellations. Flip side: a withdrawal WITHOUT an organizer change also costs nothing beyond the <72h fee forfeit — owner call.
- ST-20(b)(c): reconfirm page state keys ONLY on orders.status / reconfirm_refunded_at (api/orders/reconfirm/[token]/route.ts:59-65) with fixed "still stands" copy (reconfirm/[token]/page.tsx:131-134). Vendor cancel flips items (cancel/route.ts:333-341) and the ORDER only when no live items remain (:404-408, skipped on session-expire failure). Owner saw "still stands" AFTER the withdrawal ⇒ UNVERIFIED whether order stayed un-flipped (multi-vendor / skipOrderCancel) — diagnostic query given in chat. Design fix: derive state from live items (all cancelled → withdrawal copy; some → partial copy + link to event page) + soften pending copy ("stands as of now — check current vendors/offerings").
- ST-20(d): status-copy inconsistency across organizer dashboard / event page / shop / select — backlog (owner: later).

## PROD API-LOG ERRORS (owner pasted from Supabase 2026-08-25; log times are CT) — all pre-existing, none from today's work
Attribution: 00:35/04:58/18:03/19:14/20:09/20:29/20:42 `market_vendors.status` = PUBLIC VENDORS PAGE renders (humans/crawlers) · 03:01 `new_flags` = vendor-activity-scan cron (08:00 UTC) · 10:00:32-34 quartet (22P02 + 42703 ×4) = hourly SURVEYS cron market-day phase (08–12 local window), both branches failing per market.
Sites (verified reads): (1) [vertical]/vendors/page.tsx:105 `.eq('status','approved')` on market_vendors — column never existed (use `approved`) — since 2026-01-23 → vendor cards show no markets, ?market= filter dead. (2) cron/surveys/route.ts:382 + lazy-generate.ts:41 — `day_of_week` on vendor_market_schedules (lives on market_schedules via schedule_id) — cron vendor branch since 2026-05-22, lazy since 07-17 → NO vendor ever surveyed. (3) order_items status 'completed' (enum lacks it; whole query fails): cron/surveys:520 (05-22, no buyer ever surveyed), lazy-generate:146 (07-17), complete-event.ts:42+:194 (07-11, settlement 0/$0 + unfulfilled alerts skipped), top-products/route.ts:66 (02-19, 500). (4) DB fn scan_vendor_activity (mig 049, 02-22) UPDATEs `new_flags`/`auto_resolved`/`summary` — real cols new_flags_created/flags_auto_resolved/flags_by_reason (mig 002 was right) → daily scan rolls back, zero flags ever.
All silent except top-products 500 + cron 500s: callers read `data` only, never `error`. Proposed: 5 TS fixes + mig 237 (function replace) + 2 guard tests; backlog a dropped-PostgREST-error sweep. AWAITING owner go.
### FIX BATCH BUILT 2026-08-25 (owner "yes" — Fix mode + staging push together with Layer 1)
- ✅ vendors/page.tsx `.eq('approved', true)` · lazy-generate.ts (schedule_id → market_schedules lookup, one query; `'fulfilled'`) · cron/surveys (day's schedule ids → vms by schedule_id; `'fulfilled'`) · complete-event.ts (`("fulfilled","cancelled")`; `'fulfilled'`) · top-products (`'fulfilled'`)
- ✅ Mig 237 `20260825_237_scan_vendor_activity_columns.sql` — generated from mig 049, diff = exactly the 3 SET lines (new_flags_created / flags_auto_resolved / flags_by_reason). ⏳ applied NOWHERE — paste-and-go, all 3 envs; snapshot row added. Post-check = manual cron run → 'completed' row.
...
## OWNER TESTING RESULTS — round 3, 2026-08-27 (logged verbatim-ish BEFORE analysis; plan follows)
R3-1 Vendor market-day survey: intro copy talks about helping the market manager / funders — irrelevant to a vendor; vendor survey should be about THEIR experience (cleanliness, easy to navigate, parking/spot access, signage, foot traffic). Also wrong noun in FT: "market" → "park" / "spot" (market only in FM).
R3-2 Vendor dashboard (truck2, established): red/check box "Onboarding complete — your listings can now be made visible" still shows though all listings are published for a long time. Should only show right after onboarding while listings are unpublished/invisible.
R3-3 Vendor dashboard My Locations card: growing list; opening it shows INVITATIONS for each event location the vendor was invited to. Duplication? The vendor already accepted the event invite from the organizer — does the location-side invite need a second accept for some other element to work? Find the duplication / streamline.
R3-4 Vendor events page bottom: a second list of invited events (pills: accepted/declined/invited) separate from the top unresponded list — find which data feeds which. That bottom list shows a SCHEDULE CONFLICT notice — the event workflow doesn't check availability at all (matching invites regardless; no conflict warning to the vendor). Owner: it's the vendor's choice, but the platform must call it out + get acknowledgment ("you're scheduled at both; choosing the event forfeits your spot payment") and check for pre-orders in that window (unlikely — 10-day lead vs pre-order horizon — but verify no dependency).
R3-5 Mobile UI on the orders screen (and maybe locations): the page "floats" — pinch-zoomable, pans around, content wider than the screen, text not wrapping, buttons not right-sized; information can sit off-screen. Functionality fine; visual/viewport inconsistency.

### ROUND-3 FIX PLAN (analysis done 2026-08-27 pre-compaction; NOT built; owner to approve)
R3-1 Survey copy — 4 sites talk to vendors about managers/funders: `components/surveys/PendingSurveysCard.tsx:49`, `components/surveys/SurveyForm.tsx:108`, `[vertical]/vendor/surveys/page.tsx:105-106`, `[vertical]/survey/[token]/page.tsx:112` (buyer token page — check audience before editing). Rewrite vendor-facing copy around the VENDOR's experience (cleanliness, navigation, spot/parking access, signage, foot traffic) and use `term(vertical, …)` nouns (FT: park/spot, FM: market). Also review the vendor survey QUESTIONS themselves for the same audience mismatch (lib/surveys — not read yet).
R3-2 `components/vendor/OnboardingChecklist.tsx:100-116` — the "Onboarding complete — your listings can go live!" box renders whenever `status.canPublishListings` is true, forever. Fix: also require "no published listings yet" (the dashboard already loads listing status) or a dismissal; hide once ≥1 listing is published.
R3-3 Location-invite duplication — matching creates `market_vendors {response_status:'invited', invited_at}` (event-actions.ts:374-379, no `approved`). The locations page's `PendingMarketInvitations` (NEW-8, manager-initiated flow; labels event rows `:187`) shows the SAME rows as a second "invitation". UNVERIFIED: the exact filter feeding `pendingInvitations` in vendor/markets/page.tsx and whether the event accept path sets what that list keys on (PendingMarketInvitations:13 says accept flips `approved`). Fix direction: ONE workflow — exclude `market_type='event'` rows from the manager-invitation list (events have their own invitation page), and make the event accept path set whatever the locations side needs so nothing waits on a second accept.
R3-4 Bottom-of-page event list = `EventMarketsSection` (vendor/markets page) reading response_status + schedule attendance (:231-246). The "schedule conflict" notice most likely comes from the DB trigger `check_vendor_schedule_conflict` firing when accepting creates a vendor_market_schedules row (backlog "Priority 2 — schedule-conflict trigger dead-end") — UNVERIFIED, read the accept path + trigger next. Design to build: availability check at (a) invitation render and (b) acceptance: same-date rows in vendor_market_schedules (weekday overlap) + paid `park_spot_bookings` on the event date → amber warning + required acknowledgment ("scheduled at both; choosing the event does not refund your spot"); (c) pre-orders on that date/market → block or warn (expected none — 10-day lead vs pre-order horizon — verify no dependency); (d) later: matching deprioritizes conflicted vendors. Matching today checks NO availability (event-actions.ts — only the organizer-email conflict :278).
R3-5 Mobile overflow on orders (and locations?) — OrderFilters already wraps (`flexWrap: 'wrap'` :16); Next's default viewport meta applies (no custom one in layouts). The overflowing element is UNVERIFIED — audit `vendor/orders/page.tsx` + OrderCard for fixed widths / `whiteSpace: nowrap` / long unbroken strings (order numbers, emails); reproduce with Chrome device emulation (claude-in-chrome skill) or owner screenshot. Guard: page container `overflowX: hidden` + `minWidth: 0` on flex children; real fix = the specific element.
Order: R3-2 (trivial) → R3-1 copy → R3-3 dedupe → R3-5 audit → R3-4 (design + build, biggest).

### ROUND-3 APPROVAL (owner 2026-08-27): R3-2, R3-1, R3-3, R3-5 APPROVED to build. R3-3 condition: "make sure you really understand the mechanics behind the location issue — locations are deeply connected with other vital systems. read the code before changing anything." R3-4: REVIEW + PROPOSE solutions only (not approved to build). Working notes → `.claude/round3_research.md`.
### GO (owner 2026-08-27): staging load-bearing query = 0 rows. Build R3-2, R3-5, R3-1 (copy + relabel, NO migration), R3-3 parts (1) list filter + (2) respond-route 409 guard. R3-4 items 1–5 HELD for owner call. "go slow … know the result of the changes … foresee conflicts and redirect."

### ROUND-3 BUILD RECORD 2026-08-27 — R3-2, R3-5, R3-3(1+2), R3-1 BUILT, gates green (tsc · eslint 0 err · vitest 2042 + 2 new guards). Details: `.claude/round3_research.md` "BUILT". R3-4 proposal (5 items) awaiting owner call — see chat 2026-08-27. Uncommitted at write time. Flagged, not built: cleanliness/parking own columns (migration); event vendors' approved=false vs check-in eligibility.
### R3-4 DECISIONS logged in decisions.md ("Event ↔ location conflicts — availability rule", 2026-08-27). Design questions round 2 sent; NOT built. Built batch (R3-1/2/3/5) still UNCOMMITTED — owner has not yet answered "commit + push staging?".

### R3-4 BUILT 2026-08-27 (owner: "build as listed", tripwire 118→119 approved, mig 238 file approved) — gates: tsc clean · eslint 0 errors · vitest 2068/2068 (+17 availability, +7 flow-integrity guards). UNCOMMITTED at write time.
- Mig `supabase/migrations/20260827_238_vendor_date_blackouts.sql` — PASTE-AND-GO, inert on arrival. **Dev + Staging 2026-08-27 (owner); Prod PENDING — file stays in root.** Snapshot row added (PENDING). Code is tolerant pre-migration (blackout write/lift log-and-continue).
- New: `lib/events/availability.ts` (pure evaluateConflicts + loader), `lib/events/blackouts.ts` (write/lift), `lib/events/__tests__/availability.test.ts`.
- Wired: accept route (shared check, 4 error codes, blackout write + `park_spot_skipped_for_event`), Vendor Event Page GET (`availability` while invited) + page block/checkbox, lifts in select (bench) / vendor cancel / organizer cancel / admin cancel, event cards lose Set Schedule (link to event page instead), FM profile flag (same key), notification type + tripwire 119 + MESSAGE_TEMPLATES, 14_Events item 7 + INDEX stamp.
- Backlog to add: reverse-direction check (booking/schedule over an accepted event); release-without-credit for a skipped paid spot; manual "skip a day" UI on vendor_date_blackouts; check-in prompt still fires on a skipped paid park day (owner: take the hit).

# STAGING TEST PROTOCOL 2026-08-28 — WORKFLOW EDITION (owner asked for multi-element workflows; reconcile by tag on return)
Combines everything unreported: ST-1…18, ST-23, ST-24 (2026-08-16 protocol); 08-15 list #3 capacity copy, #4 below-claim note, #6 B7 notice; 08-25 Tier-1 #6 order chip + customer_milestone; 08-26 round-2 retest R2-1…5; 08-27 round-3 fixes R3-1/2/3/5 + R3-4 rule (A–I). Already PASSED, not repeated: ST-19(untestable)/20/21/22/25/26, Tier-1 #1–5, #7 First Bite, iPhone submit, fee $1 + reuse flow, Vercel prod Ready.
W1 New truck → approval → first listing: [ST-23] not-eligible badge · [#3] capacity copy on /vendor/edit · [R3-2] onboarding banner until first publish.
W2 Organizer intake → invite → conflicted accept → pay → select: [R2-1] email prefill · [R2-2] capacity box + verdict · [R3-4 A] amber box, Accept gated · [#4] below-claim amber note · [R3-4 B] park date gone after accept · [ST-10] unpaid can't sell → pay → sells · [ST-24] reuse-button styling · [ST-8] risk checklist · [ST-7] bench rec · [R2-3] backups sentence + invite-more link + "You're confirmed" · [R2-5] next-step copy.
W3 Pre-orders + multi-truck: [R3-4 C] open order blocks, fulfill clears · [R3-4 D] flag on → "cover both", no blackout, organizer sees note · [R3-4 E] paid spot → operator notice.
W4 Deselect → bench → standby → refund: [R2-4] ConfirmDialog → roster · [ST-9] benched menu gone from shop · [ST-12] deselection refund · [ST-13] admin fee card + manual refund · [ST-5] standby join/leave · [R3-4 F] benched truck's park date returns · [ST-14] free-event card empty state.
W5 Cancellation money: [ST-3] early auto-refund · [ST-4] status cancelled · [B7] notices · [R3-4 G] withdraw lifts blackout · (move date via admin → [ST-15] reconfirm) · [ST-1] late forfeit · [ST-2] waive → REVERSED transfer · [ST-6] covered-spot chain · [ST-18] cron refund manual GET.
W6 Reconfirm + prep: [ST-15] · [ST-16] GET never confirms, button does · [ST-17] prep split + banner.
W7 Buyer orders + loyalty + surveys: [R3-5] orders page mobile · [T1-6] "New customer" chip · [MILESTONE] Regular @4 chip + vendor in-app · [R3-1] survey copy/questions.
W8 My Locations: [R3-3] no event rows in Pending Market Invitations; event cards link, no Set Schedule; real manager invite still accepts.
W9 FM: [R3-4 H] profile flag present; FM event overlapping market day → same flow.
W10 PROD checks: Tier-2 smoke · API log clean (5 signatures) · scan_log completed rows · Google Vision billing.

## OWNER RESULTS 2026-08-28 — W1 (workflow protocol): [#3] capacity copy present (PASS, copy changes requested) · [ST-23] not-eligible badge PASS · badge gone after approval PASS · [R3-2] banner gone after first publish PASS. Testing continues.
## OWNER "DO SOON" LIST 2026-08-28 (organize, not build yet) → backlog "🔜 DO SOON — owner 2026-08-28"
### DO-SOON items 3–7 BUILT 2026-08-28 (owner "build items 3-7 while i test"): docs blue box moved inside DocumentsCertificationsSection · prep-time last sentence · Pickup Capacity two sentences · readiness "How our events work" (3 paragraphs verbatim, FM variant) · wave-field helper. Gates: tsc clean · eslint 0 errors · vitest 81 files green. UNCOMMITTED — owner mid-test on staging; ask before pushing (redeploy mid-test). Items 1–2 (notification routing, pickup-line acknowledgment + branded signs) still open.
## OWNER RESULTS 2026-08-28 — W2: [R2-1] not tested (phone) · [R2-2] PASS (capacity box + "Comfortable — your 20 per window…") · [#4] PASS · [R3-4 A] PASS functionally, PRESENTATION FAIL (checkbox text runs together → owner wants 4 bullets) · post-accept "next steps" box WRONG INFO (owner supplied 4 bullets) · [R3-4 B] untestable (event > 8 days out) · [ST-10] pay button appeared only after selection (blue box says so — OK) then menu shows after payment PASS; fee-paid notices to vendor + organizer PASS · organizer select page after confirm: "Your Trucks Are Confirmed!" copy says trucks "will connect their catering menus" (stale — it's automatic) · **truck2 NOT notified of selection (event_vendor_selected missing) though pay button appeared** · [ST-7] PASS · [ST-8] instruction too vague · [R2-3] bench rec present; backups sentence/invite-more not reported · [R2-5] FAIL — "Your selected items are now on the event menu…" not shown on Vendor Event Page after selection+payment (items ARE live on /shop) · NEW: cart drawer "Continue shopping" from an event item → /browse instead of the event shop page.
### W2 FIXES BUILT 2026-08-28 (owner "yes"): (1) select route GET+POST derive "selected"/"first confirmation" from organizer_selected_at only (status 'ready' fallback removed — it silenced event_vendor_selected + the organizer kit on self-service events and opened the select page in the confirmed state); guard added · (2) Vendor Event Page Next Steps state-aware (owner's 4 pre-selection bullets; post-selection: menu live + pay fee if unpaid + prep) · (3) conflict checkbox → owner's 4 bullets · (4) organizer "Trucks Are Confirmed" copy: menus already attached · (5) `lib/events/last-event-shop.ts`: shop page remembers itself; cart drawer + checkout empty state/back link return there. Gates: tsc clean · eslint 0 errors · vitest 81 files green. UNCOMMITTED.
### PROTOCOL v2 (2026-08-28, after owner: "rewrite them so I'm not guessing what to do on what page") — same tags as the workflow edition; W2 retest tags added: [W2-FIX-1] selection notifies + select page shows Select buttons · [W2-FIX-2] Next Steps state-aware · [W2-FIX-3] conflict bullets · [W2-FIX-4] organizer confirmed copy · [W2-FIX-5] continue shopping → event shop. Staging = 4e9a6179 (includes copy commit 1a846cfc). Full text in chat.
### VENDOR DELEGATES ("Team members") — design + sweep FINALIZED 2026-08-29 in `.claude/vendor_delegate_design.md` (sections 0–5: decisions, 45-site sweep, RLS 57-policy map, surface classification, build spec incl. mig 239 + resolver + owner-only lists + Team card, tests, 3-session sizing). Owner: "finalize the plan so we can implement it later." Not scheduled.

# ACTIVE 2026-08-29 — "fix found/known issues" batch while owner tests (owner: "proceed with items that help us fix found or known issues — go in the order you determine to be best"; Team members stays on backlog). LOCAL COMMITS ONLY until owner pauses testing. Order: (1) silent-failure sweep (helper + ratchet guard; money files need per-file diff approval) → (2) Restore-event button, reconfirm item-based state/copy, status-copy drift → (3) notification routing (admin-who-is-organizer → organizer link; organizer_user_id set at approval when the account exists) → (4) pickup-line acknowledgment + printable branded pickup signs.
### (1) SILENT-FAILURE SWEEP — BUILT 2026-08-29 (uncommitted at write time): `lib/errors/observe.ts` `observed(query, { table, operation?, route?, logNoRows?, extra? })` — keeps caller shape, logs via logError (schema-class codes forced high), PGRST116 not logged; admin alert email throttled 1/hour per code+route (`logger.ts`). Codemod (scratchpad `codemod_observed.js`) wrapped **706 sites in 227 files** (crons, events, markets, vendor orders/markets, market-manager, admin, buyer, lib) — happy path byte-identical. Remaining 129 = **77 in 9 critical-path files (per-file approval; diff at scratchpad `critical_observed.patch`)** + 52 residue the codemod couldn't parse. Guardrail Rule J ratchet: unwrapped sites ≤ 129. Gates: tsc clean · eslint 0 errors · vitest 81 files green.
### Critical-file wrap APPROVED ("do all of them") + applied 2026-08-29: 9 money files, 77 sites; money-authorization harness + full suite green; ratchet lowered 129 → 58. Committed locally.
### (2) KNOWN-ISSUE FIXES BUILT 2026-08-29 (committed 4c8a42e8; B5 amendment CONFIRMED by owner — decisions.md): **Restore Event** button on cancelled events (admin/events page) → route's guarded misclick repair (refuses with reason when buyers refunded or vendors notified) — ⚠ this REMOVES the unconditional one-way block that decisions.md:172 (B5 option a, 2026-08-15) added; the guarded check enforces the same rationale (nothing that moved money or told anyone is reversible) — FLAGGED to owner, revert if they want the hard block back · **Reconfirm page** derives state from live items: all cancelled → "vendor withdrew / refunded" screen (+ link to the event), some cancelled → partial banner, awaiting copy now "stands as of now … check who's attending" · **Status vocabulary** unified: said yes / selected / attending (organizer dashboard "N of M selected · K said yes", public page "Attending" / "Still Responding", shop "No vendors are attending yet", select "No vendors have said yes yet", ready label).
### (3) NOTIFICATION ROUTING BUILT 2026-08-29: `lib/events/organizer-recipient.ts` — organizer id resolved from the column, else by contact_email → user_profiles (backfilled); admin copy of catering_vendor_responded goes to admins MINUS the organizer (respond + cancel routes); cancel route's admin copy keys fixed (companyName/eventDate → vendorName/marketName, was rendering "undefined"); intake stamps organizer_user_id when the submitter is signed in with the same email. Flow-integrity guard. Gates green.
### (4) SEPARATE PICKUP LINE — BUILT 2026-08-29: `PickupLineAcknowledgment` (explainer + "I agree" → `api/vendor/onboarding/acknowledge-pickup-line` → `profile_data.pickup_line_acknowledged_at`); onboarding status gates SUBMIT-for-approval on it (new vendors) + progress step; approved vendors get a compact reminder card on the dashboard until acknowledged (publishing NOT gated); `/[vertical]/vendor/pickup-signs` = print-ready branded "APP ORDER PICKUP" sign (8.5×11 / 11×17 via @page) + "Pickup Signs" dashboard tile. Guard in flow-integrity. Gates green. All four "fix known issues" items now built + committed locally; NOT pushed (owner testing).
### ⚠ PUSH TO STAGING FAILED ONCE 2026-08-29, FIXED (vendor-limits.ts un-wrapped — client-bundled; client-bundle guard added; local `npm run build` green) — re-pushed. Original failure: (pre-push `npm run build`: client bundle pulled `lib/errors/breadcrumbs.ts` → `async_hooks` via a lib the codemod gave an `@/lib/errors` import; local `staging` = 6092acc9, remote still 4e9a6179) — fixing, then re-push. Intended stack: `6092acc9` (8 commits: copy 1a846cfc · delegates design 44dfd6a8 · sweep 2c2e17cb+f3001ab2 · fixes 4c8a42e8 · routing 510aafb2 · pickup line 5f84d9e4 · B5 docs e113432a · follow-ups 6092acc9). Prod still e946c2c0; migs owed to prod: 238 (+ pending 239? none). Owner retest lines in chat (R4-1…R4-8).
### ✅ PUSHED TO STAGING 2026-08-29 — `4e9a6179..30f7c15c` (9 commits; two failed attempts first: client-bundle async_hooks via vendor-limits [fixed 30f7c15c], then a transient Google Fonts fetch in the hook's build [retried]). Playwright 49✓. origin/staging is 18 commits ahead of prod (e946c2c0); prod owes mig 238 paste before the code push. Retest lines R4-1…R4-9 in chat.
## OWNER RESULTS 2026-08-29 (W2-R + W3 prep): PASS: R2-1 · W2-FIX-3 · W2-FIX-2 (both states) · TRUCK3 accept · W2-FIX-1 select buttons + kit email link · W2-FIX-4 copy · confirmation notification + link · W2-FIX-5 continue shopping (event cart + empty cart). FINDINGS: (a) survey fatigue — vendor asked to rate the park every scheduled day → revise cadence; (b) admin "approve" step unclear + event already showed approved (self-service?) ; (c) PRODUCT: invitations go out before the organizer has entered price/details — owner wants richer details required before invitations (preliminary match ok); (d) wants a clear status line on Vendor Event Page: selected → payment due → confirmed & paid; (e) prep sheet won't activate after orders placed — time/proximity gated?; (f) truck had no park schedule until listings were connected to the park — how do FT park schedules get created?; (g) park is active in admin but park manager never accepted the email — what does the system do with an unaccepted manager?

## FINDINGS DISPOSITION 2026-08-29 (from W2-R/W3 results) — read code, nothing built yet
- (a) survey cadence: `lib/surveys/lazy-generate.ts:84-108` inserts one vendor survey per (vendor, market, market_date) for today/yesterday whenever a schedule matches → daily parks = daily surveys. Buyer side (`:145-160`) same per pickup day. PROPOSED: per (vendor|buyer, market) throttle — first survey after first day, then ≥30 days between surveys (query latest market_surveys row, no migration). Owner to pick interval.
- (b) admin approve: self-service intake auto-approves when an address is present and auto-invites in the same request (`api/event-requests/route.ts:331-341`, `:404-415`). Admin "Approve" only matters for admin-assisted requests. UNVERIFIED whether admin panel still shows an Approve control on an already-approved self-service row → retest N2.
- (c) PRODUCT (owner): invitations fire at intake before fee/details exist (fee can only be set after approval). PROPOSAL: auto-approve + match at intake, hold invitations until organizer completes required details and clicks Send invitations. Needs owner decisions: required field list, fee mandatory (explicit "free event"), same gate for admin-assisted. ~1 session; touches intake route, organizer dashboard, cron self-service rematch (`auto_invite_sent_at`), admin invite.
- (d) status line on Vendor Event Page header: Invited → You said yes (awaiting selection) → Selected · fee due → Confirmed & paid / Confirmed (free or covered) → On the bench / Withdrawn. Small build, needs approval.
- (e) prep sheet: NO link to `/vendor/events/[marketId]/prep` from the Vendor Event Page or dashboard; only Pickup Mode's event panel links it (`vendor/pickup/page.tsx:796`). Route gates only on response_status accepted (`prep/route.ts:62`). FIX: add Prep sheet button on Vendor Event Page (accepted) + dashboard event row. Diagnostic retest N1 = open the URL directly.
- (f) FT park schedules are created by (1) paid park-booking webhook (`lib/stripe/webhooks.ts:1948-1985`) or (2) vendor Set Schedule (`api/vendor/markets/[id]/schedules/route.ts`). Listing↔market connect route touches only listing_markets/listings/markets — it does NOT create schedules. Cause of owner's observation UNVERIFIED → retest N3 (which park, was booking paid, which page showed the schedule).
- (g) unaccepted manager: assignment notify returns without sending when the email has no account (`api/admin/markets/[id]/manager/route.ts:66`) → invited manager gets NOTHING; acceptance = first dashboard load (`[vertical]/dashboard/page.tsx:182`); 8 manager-facing sends skip on null manager_user_id. PROPOSAL: Resend invite email to manager_email at assignment (+ admin "invited, not accepted" badge + Resend invite button). Owner decision.

## PROTOCOL v3 (2026-08-29) — supersedes the W1–W10 index above. PASSED (not repeated): W1 all; W2: R2-1, R2-2, #4, R3-4 A, ST-7, W2-FIX-1…5, TRUCK3 accept, confirmation notice. Not yet reported from W2 (owner to confirm or run): ST-8, ST-10, ST-24, R2-3.
Accounts: ADMIN · ORG (organizer, self-service) · TRUCK1/2/3 · BUYER · MGR (park manager). All URLs are staging, vertical food_trucks unless stated.
N1 prep sheet: TRUCK (accepted, has orders) → type `/food_trucks/vendor/events/<marketId>/prep` (marketId from the Vendor Event Page URL) → report what loads. (Button missing = known; fix pending approval.)
N2 admin approve: ADMIN `/food_trucks/admin/events` → find ORG's self-service event → report exactly what the row shows (Approve button? status?) — expected: already Approved.
N3 park schedule: report which park + whether the truck's booking was paid on staging + which page showed the schedule before/after connecting listings.
N4 manager: MGR email had no account → expect NO email at all (known gap). Confirm.
W3 pre-orders + multi-truck: TRUCK2 with event accepted at `/food_trucks/vendor/events/<id>`: [R3-4 C] BUYER pre-order at TRUCK2's park on event date → TRUCK2 second event same date → Accept refused "open orders" → TRUCK2 `/food_trucks/vendor/orders` fulfil → Accept works. [R3-4 D] TRUCK2 `/food_trucks/vendor/edit` tick "I have more than one truck…" → accept overlapping event → amber box says cover both, no blackout (`/food_trucks/vendor/markets` park date still bookable), ORG `/food_trucks/event-manager/<id>/dashboard` shows the multi-truck note. [R3-4 E] paid park spot skipped for event → MGR `/food_trucks/notifications` operator notice. [R3-4 B] after plain accept: BUYER at TRUCK2's park shop → event date absent from pickup dates.
W4 deselect → bench → standby → refund: ORG `/food_trucks/events/<token>/select`: [R2-4] deselect → ConfirmDialog → roster updates · [ST-9] `/food_trucks/events/<token>/shop` benched truck's menu gone · [ST-12] benched truck's fee refunded (Stripe) · [ST-13] ADMIN `/food_trucks/admin/events` fee card + Manual refund · [ST-5] TRUCK3 standby join/leave on Vendor Event Page · [R3-4 F] benched truck's park date returns · [ST-14] free event → fee card empty state.
W5 cancellation money: TRUCK Vendor Event Page **Cancel My Participation**: [ST-3] early → auto-refund · [ST-4] status cancelled · [B7] notices to ORG + ADMIN · [R3-4 G] blackout lifted (park date back) · ADMIN move date → [ST-15] reconfirm emails · [ST-1] late cancel → forfeit · [ST-2] ORG waive → REVERSED transfer · [ST-6] covered-spot chain · [ST-18] cron refund via manual GET.
W6 reconfirm + prep: [ST-15] BUYER `/food_trucks/reconfirm/<token>` · [ST-16] opening the link never confirms; button does · [ST-17] prep split + banner · [R4-2] truck withdrew → "refunded" screen; one of two withdrew → amber partial banner.
W7 buyer orders + loyalty + surveys: [R3-5] `/food_trucks/vendor/orders` mobile width · [T1-6] "New customer" chip · [MILESTONE] Regular @4 + vendor in-app · [R3-1] `/food_trucks/vendor/surveys` + `/food_trucks/buyer/surveys` copy/questions.
W8 My Locations `/food_trucks/vendor/markets`: [R3-3] no event rows under Pending Market Invitations; event cards link to Vendor Event Page, no Set Schedule; real MGR invite still accepts.
W9 FM `/farmers_market/...`: [R3-4 H] `/farmers_market/vendor/edit` flag "I can staff more than one location…" present; FM event overlapping market day → same accept flow.
R4 (pushed 30f7c15c): [R4-1] Restore Event `/food_trucks/admin/events` · [R4-3] status vocabulary (ORG dashboard "N of M selected · K said yes"; public page Attending/Still Responding) · [R4-4] admin-who-is-organizer gets ONE "said yes" notice → ORG dashboard · [R4-5] new vendor onboarding pickup-line checkbox gates Submit · [R4-6] TRUCK2 dashboard compact card → agree → "Print your pickup signs" · [R4-7] `/food_trucks/vendor/pickup-signs` Print 8.5×11 / 11×17 · [R4-8] event cart empty → back to event shop; normal cart → Browse (PASSED as W2-FIX-5) · [R4-9] after a day: error_logs rows → paste.
W10 PROD (after prod push): Tier-2 smoke · API log clean · scan_log rows · Google Vision billing.
### OWNER ANSWERS 2026-08-29 + corrected reads (nothing built)
- (a) DECIDED: vendors weekly (one page, section per place attended that week); buyers after 1st + 2nd purchase then weekly in purchase weeks, section per place + "other places" free text. Rows stay per (person, market, date) — no migration.
- (c) DECIDED required-before-invite: fee free/charged (`catering_requests.event_vendor_fee_cents`, mig 228); NEW "run before? Y/N" column (migration) + reworded required `estimated_spend_per_attendee_cents`; Event Context all except is_themed/theme_description; background_check_required+details; cancellation_risk_factors. Open: admin-assisted same gate? explicit Send button?
- (e) CORRECTED: Upcoming page 7-day window; empty-state skeleton "Prep List" is a non-link placeholder (`vendor/upcoming/page.tsx:39-41,100-160`). Market prep page works for events (event market has market_schedules, `event-actions.ts:154-160`). Ask owner: was the event >7 days out? Proposed: skeleton not button-like + Prep sheet link on Vendor Event Page.
- (f) EXPLAINED: schedule rows created at payment (`webhooks.ts:1930-1975`); buyer dates need listing_markets (mig 238:148). Proposed nudge for paid park with 0 connected listings.
- (g) CORRECTED: admin assign sends NO email (`manager/route.ts:186-238`); only "expired" screen = confirm-email (Supabase signup link). Proposal: invite email + badge + Resend — awaiting yes/backlog.
- (d) proposal = replace header badge with staged status line — awaiting yes.
- Pickup-line: friendlier copy drafted in chat (both variants) — awaiting yes. Placement already onboarding (checklist) + dashboard compact for pre-existing vendors.
### BUILT 2026-08-29 (owner-approved batch; uncommitted; gates: tsc clean, vitest see below)
- (d) Vendor Event Page header = ONE staged status line (`eventStage()` in `vendor/events/[marketId]/page.tsx`).
- (e) Upcoming empty-state note (7-day window + "Open your prep sheet" pointer) + Prep sheet link on the Vendor Event Page when accepted.
- (f) Dashboard amber nudge: paid park spot(s) with NO listing connected → "Connect a listing to {park}" (query `park_spot_bookings` status paid ≥ today vs `listing_markets`).
- (g) Manager invite email on admin assign (`sendManagerInviteEmail` → `sendExternalEmail`, signup link with email) + `resend_invite` action + "Invited · not yet accepted" badge + "Resend invite email" link (`MarketManagerAssignment.tsx`).
- (c) INVITATION GATE — **mig 239** (Dev+Staging 2026-08-29, Prod PENDING): `lib/events/invitation-gate.ts` + test (8); intake dry-run (`autoMatchAndInvite(..., { dryRun })`), copy on intake response/form/organizer email; cron re-match sweep + refresh-matches skip held; `api/events/[token]/release-invitations` (POST, organizer, atomic); details PATCH stamps `event_context_confirmed_at`/`logistics_confirmed_at` + allows `has_run_before`; vendor-fee PUT stamps `vendor_fee_decided_at`; `InvitationGateCard` on Organizer Event Dashboard (self-service only); details editor: Budget Part 1/2 reword + "needed before invitations go out" label tags. Admin-assisted: NOT gated (owner: later).
- (a) SURVEY CADENCE — **mig 240** (Dev+Staging 2026-08-29, Prod PENDING): `lib/surveys/cadence.ts` (+ test 9) + `weekly.ts` (`generateWeeklySurveys`, `isEarlyBuyer`); cron: per-day vendor branch REMOVED, buyer per-day only for purchases 1–2, weekly batch in the 15:00 UTC daily branch; lazy paths → weekly; `buildWeeklySurveyEmail`; 2 new notification types `survey_weekly_vendor/buyer` (→ NOTIFICATION_REGISTRY 119→121: tripwire `cutoff-and-sort-functional.test.ts:284` — needs owner OK); SurveyForm buyer "other places" textarea → `other_places_request`; list-page + card copy.
- NOT built (owner didn't answer): pickup-line friendlier copy.
## RETEST LINES R5 (pushed with this batch — add to Protocol v3)
- [R5-1] Status line: TRUCK Vendor Event Page badge reads Invited… → You said yes… → Selected · fee due $X → Confirmed & paid (free event: Confirmed · free event; bench: On the bench).
- [R5-2] Upcoming `/food_trucks/vendor/upcoming` with no pickups in 7 days: grey preview + the note under it; accepted TRUCK's Vendor Event Page shows "📋 Open your prep sheet →" → prep sheet loads.
- [R5-3] Paid park + no connected listing → dashboard amber "You're booked at {park} — but nothing is for sale there yet" → Connect link → after connecting, card gone.
- [R5-4] ADMIN `/food_trucks/admin/markets` → assign manager with a NEW email → that inbox gets "You're the manager for {market} — accept your invite"; card shows "Invited · not yet accepted" + "Resend invite email" (resend works; refused once accepted).
...
### 2026-08-30 MORNING — snapshot rebuild + Rule L + logs retired (UNCOMMITTED; gates: tsc 0, 2097/2097)
- **Snapshot structured tables REBUILT** from a Dev information_schema export (3 attempts — Supabase CSV export truncates every value at 64 chars; final query = one row per FIELD): Tables 55→91, Columns 67→91 tables / 1396 cols, FKs 167, Enums, Views. Indexes + Checks sections retained (definitions can't survive the cap; banner marks best-effort). Stamp: `Structured tables rebuilt: 2026-08-30 · current through migration 240`. Changelog row added.
- **Guardrail Rule L**: stamp is machine-read; suite fails on any newer CREATE TABLE migration or >5 migrations past the stamp. migNumber handles both filename eras (date_TIME_num + date_num).
- **MIGRATION_LOG.md retired** (both copies → pointer stubs); migration-workflow.md + verification-discipline.md Rule 3 updated (snapshot Change Log = single record; stamp rule documented).
- Owner still owes: Protocol v4 section 1 retest + R5-8 on staging ff25ac33.
- PHANTOM COLUMNS FIXED 2026-08-30 (owner "do the phantom columns while i test"; gates: tsc 0, 2097/2097): admin/reports listings `quantity` (stock col never existed → all 'Unlimited'); vendor-fee report ledger `amount_cents`/`type` + balance summary now sums all-time debits/credits from the ledger (view only has balance_cents); location-insights buyer density `preferred_latitude/longitude`. Rule K KNOWN_PHANTOM_COLUMNS → [] (may only shrink). Backlog entry can be closed. UNCOMMITTED.

### ADMIN UI REBUILD — PHASE 1 BUILT 2026-08-30 (owner "build phase 1"; UNCOMMITTED)
One admin shell for both tiers: `components/admin/AdminShell.tsx` (client chrome: scope pills = held scopes only, ☰ grouped menu with every page + queue badges + attention dot) · `lib/admin/nav.ts` (single nav definition + NAV_EXEMPT_PAGES) · `lib/admin/queue-badges.ts` (7 parallel HEAD counts; statuses verified per owning page: vendors 'submitted', markets 'pending', catering new/reviewing, issues reported+null/new/in_review via orders join, error_reports pending/acknowledged, flags 'pending', cause paid_at NULL platform-only) · `lib/admin/shell-data.ts` (server resolution). Both layouts rewritten to render it; `<AdminNav` stripped from 18 pages (files AdminNav/AdminSidebar kept, unused). NEW flow-integrity contract: nav ⊇ all top-level admin pages AND every nav link has a page; `<AdminNav` may not return. Access/auth untouched. Map 19_Admin updated. Plan doc: admin_ui_redesign_research.md.
Owner test (after push): any admin page → sticky bar; ☰ lists every page grouped, badges match real queues; scope pills FM|FT|Platform switch correctly; vertical admin sees no Platform pill; old pill-nav gone everywhere.
- Phase-1 smoke (owner): S1–S5 PASS; S6 FAIL → fixed (AdminShell scope-pills nav: minWidth 0 + touch scrolling) → pushed `1391d6bd`. Owner retests S6 on a phone + S7 if a vertical-admin account exists. Staging = 1391d6bd (26 commits ahead of prod). Phase 2 (queues-first hubs) awaits owner go.

### ADMIN UI REBUILD — PHASE 2 BUILT 2026-08-30 (queues-first hubs; UNCOMMITTED)
`lib/admin/hub-data.ts` (queues from queue-badges + watch: stuck orders >24h / stale vendors ≥2d + NEW rolling 24h/7d snapshot (orders, sales, new vendors/users — rolling windows, no tz hardcode) + totals preserving EVERY old hub stat + platform per-vertical command counts) · `components/admin/AdminHubZones.tsx` (zones: Needs you now → command cards (platform) → Activity → Totals → Browse tiles built FROM nav.ts) · both hub pages rewritten thin. Capability inventory + disposition in admin_ui_redesign_research.md (only relocation: platform's inline 5-row pending table → behind the pending tile). Gates: tsc 0, 2099/2099, lint 0.
- ✅ PHASE 2 PUSHED 2026-08-30 — `1391d6bd..0b51aac1` (first push attempt failed on a TRANSIENT pre-push Playwright error — 23 tests didn't run, out-of-test error, likely stale-server + reuseExistingServer; clean repro passed 49/49; retry pushed with hook green. No history rewrite, same commit). Staging = 0b51aac1. Owner smoke: /food_trucks/admin zones + queue-tiles ≙ ☰ badges + totals sanity; /admin command cards + 24h/7d snapshot; two queue tiles land right. Phase 3 (page merges) awaits owner go.
- Phase-2 smoke (owner): 1/2/4/5 PASS with polish asks → built (UNCOMMITTED): Browse group labels underlined + more air between groups; Activity snapshot = dense auto-fit grid (2–3 cols on phones); old-style horizontal banners removed — stale vendors now an attention TILE in the queue grid, stuck orders a warning DashboardCard (one visual language). Gates: tsc 0, lint 0, 2099/2099.

### FINDINGS ROUND (invitation-gate UX + vendor page + signs) — BUILT 2026-08-30, owner "yes, build it" (UNCOMMITTED)
Scorecard logged in chat. Root cause 7/8/10 = no router.refresh() after editor/fee saves (server-rendered card never updated). Built: router.refresh() in OrganizerEventDetails.saveGroup + EventVendorFeeCard.save · Send-invitations card = FULL checklist ✓/○ (invitationGateChecklist(), strike-through resolved) · red * on Budget/Event Context/Logistics section headers · EXPLICIT "None of these apply" checkbox on the risk list ('NONE' UI sentinel → NULL + stamp; re-renders checked from logistics_confirmed_at; GET now returns both stamps) · bottom OrganizerProgress gets invitationsHeld (finding 3) · success screen scrolls to top (finding 1) · captive-audience keyed to accepted_count ≤ 1, else "no outside food options" (owner correction) · fee box bench branch (VERIFIED display gap not workflow conflict: select :348/:369/:482, step-in cancel route :546-553) · pickup signs: dashboard tile removed → PromoteCard renamed "Marketing & Promotions" with signs link; print CSS hides header/nav/footer; sign carries vendor profile QR ("scan to order ahead next time"); flow test updated to the new home (owner-directed move). R5-8 still owed by owner.

## PROTOCOL v5 (2026-08-30 evening) — passed items REMOVED. Removed as passed: gate 2/4/5/6/9, bench cycle, weekly survey, admin S1–S5+S6-fix, hub smoke 1/2/4/5.
G. Invitation gate retest (staging after this push): [G1] submit self-service event → success screen appears AT TOP, "N trucks look like a fit" visible · [G2] ORG dashboard: BOTTOM "What happens next" box says invitations NOT out yet · [G3] Send-invitations card = full checklist ✓/○; answer each section and watch items strike through WITHOUT reloading · [G4] risk list has "None of these apply" checkbox; check it, Save, leave, return → still checked; ticking a real factor unchecks it · [G5] Budget/Event Context/Logistics headers show red * · [G6] all ✓ → Send unlocks → send → trucks invited, receipt, both progress boxes flip · [G7=R5-8] no-address path: submit w/o address → add address → ADMIN approve → nobody invited → complete details → Send → invited.
V. Vendor event page: [V1] 2 vendors confirmed + no outside food → says "No outside food options…", NOT captive audience; sole confirmed vendor → captive · [V2] benched vendor's fee box: "Nothing to pay while you're on the bench…" (no "organizer hasn't selected yet", no pay button).
M. Marketing & Promotions: [M1] dashboard: Pickup Signs tile GONE; Promote card renamed "Marketing & Promotions" with "Print your App Order Pickup signs →" link · [M2] signs page: print = ONLY the sign (no header/nav) · [M3] sign shows profile QR + "scan to order ahead next time"; scan it → vendor profile.
A. Admin polish retest (from 45f2c988, unreported): [A1] FM hub: NO old horizontal banners; "Vendors waiting 2+ days" is an amber tile · [A2] Activity numbers dense on phone (2–3 cols) · [A3] Browse groups underlined w/ clear breaks · [A4-opt] vertical-admin account: no Platform pill.
STILL OWED (unchanged): Protocol v4 sections 2–9 (chat 2026-08-30) · phantom-column checks (admin listings Stock, vendor-fee report, buyer density) · R4-9 error_logs after a day · W10 prod checks after prod push (migs 238→239→240 first, window 21:00–07:00 CT).
- ADMIN PHASE 3 MERGE 1/11 — USERS (owner "go", 2026-08-30; UNCOMMITTED): components/admin/UsersAdminPage.tsx (server, one query, scope null|vertical) + UsersAdminTable.tsx (client superset); both routes thin wrappers, auth unchanged. Superset kept everything from both copies + fixed a latent defect (platform tier filter offered FT tiers against FM data → tier options now per-scope). Retired-but-present (deletion needs owner OK): both old UsersTableClient.tsx + UsersTable.tsx (that one was ALREADY dead — zero importers). Inventory in admin_ui_redesign_research.md. Gates: tsc 0, lint 0, 2099/2099.
- ✅ USERS-MERGE ROUND 2 PUSHED 2026-08-30 — `d7639112..21f7fdd6` (server-side filters · single row-list layout · hub notice style). Staging = 21f7fdd6. Session checkpointed at 91% context: see `.claude/session_checkpoint_2026-08-30_eve.md` + memory `project_session_2026-08-30.md`. Owner retest = Protocol v6 (chat). Next build on go: merge 2/11 LISTINGS.

### ADMIN PHASE 3 MERGE 2/11 — LISTINGS (built 2026-08-30 session 2, owner "continue with admin UI work"; UNCOMMITTED)
components/admin/ListingsAdminPage.tsx + ListingsAdminTable.tsx; both routes thin wrappers (auth unchanged per route). Superset + 3 filter repairs: server-side title-or-vendor search (was current-page-only both copies) · all-scope vertical dropdown fixed UUID→slug (old filter could never match) · status filter + Paused (suspended). Map 19_Admin updated (stamp 2026-08-30). Old ListingsTableClient ×2 dead (zero refs) — DELETION AWAITS OWNER OK (classifier blocked rm). Gates + commit proposal pending.

### FINDINGS ROUND 2 (2026-08-30 session 2) — 4 fixes BUILT (owner "yes to all 3" + tier chips yes; UNCOMMITTED)
1. Bench fee box HIDDEN entirely while is_backup (vendor events page; safe: cancel-route promotion clears is_backup + stamps organizer_selected_at so box+pay return at step-in). 2. Duplicate acceptance banner removed (pill + prep line carry it). 3. Organizer select success screen: fee-conditional "wait before sharing the link" note (select route GET now returns event_vendor_fee_cents ×2 builders; page interface + amber ¶). 4. Listings rows: tier chips removed. CLEARED TODAY (owner): PC1, A1-A3, V1, V2 copy, G-series gate retest, M1/M3, listings L1-L5 (L1+L4 were stale-cache; staging FM=40/FT=28 verified via owner SQL), CSV=page-scoped by design. STILL OPEN: D/C/E money paths, G7=R5-8, B buyer items, O/L/F sweeps, M2 print, users round-2 five-pointer, R4-9, same-day block (needs near-dated event). Backlog+: background-check deadline enforcement. PC2 dropped (external payments backlogged). Prod: 32 commits behind, no emergency (analysis in chat).

### ✅ SESSION 2026-08-30b WRAP — staging = 0bbf5636 (both pushes verified)
Pushed: listings merge 2/11 (`69e8c6c4`) + findings round 2 (`0bbf5636` — bench fee box hidden until step-in · duplicate acceptance banner removed · fee-event share-link heads-up on select success · listings tier chips off). Gates green on both; tree clean.
OWNER RETEST (after Vercel builds 0bbf5636): benched truck event page = NO fee box, pill still "On the bench" · accept invite = pill flips, no green banner · select truck on FEE event = amber heads-up on success (free event: none) · listings rows = no tier chips.
STILL OPEN (Protocol v6): D deselect/refund money · C cancellation money · E reconfirm+prep · G7=R5-8 · B buyer items + buyer weekly survey · O onboarding copy · L manager new-email invite+resend · F FM mirror · M2 print chrome · users round-2 five-pointer · R4-9 error_logs · same-day block ⛔ needs NEAR-DATED test event (create one — unlocks C/E timing too).
...
- MERGE 7/11 ORDER-ISSUES built 2026-08-31 + fix-attempt form WIRED (record_fix_attempt action → recordFixAttempt → error_resolutions, platform-only). Gates → 2 local commits (wire-up, merge 7) → push.
- MERGE 8/11 ANALYTICS ⚠money built 2026-08-31: AnalyticsAdminPage (client, vertical? prop). NO API/figure/label changes — per-mode metric sets verbatim; layout-only unification. Gates → solo local commit. PUSHES HELD until owner finishes event testing (all of merges 7-8 + wire-up are local-only; staging still 65f0b767).
- MERGE 9/11 CAUSE = NO-MERGE (intentional permission split, owner 2026-08-04; recorded in map+research). Next: merge 10 REPORTS ⚠money solo.
- MERGE 10/11 REPORTS ⚠money built 2026-08-31: ReportsAdminPage (vertical? prop; generators untouched; per-mode surfaces exact). ⚠ finding: accounting reports gated client-side only (API would serve them vertical-scoped) — owner decision pending. Gates → solo local commit.
- MERGE 11/11 FEEDBACK = NO PAIR (vertical-only page; verified by find). **PHASE 3 COMPLETE 2026-08-31.** Local stack UNPUSHED (owner holding staging for event tests): wire-up + merges 7/8/10 = 4 commits on main past origin/staging 65f0b767. Next on go: push stack when owner ready to test, then phase 4 VENDORS superset.
- TEST CORRECTION (owner caught 2026-08-31): G7/R5-8 "submit event WITHOUT address" is UNREACHABLE — address required at intake form (2026-08-08), intake API, admin create; repeat copies it. Step RETIRED. Replacement = Event A step 1 revised: submit normally → ADMIN approves → verify NO trucks invited while gate incomplete → complete gate → Send → invited (keeps R5-8's real assertion: approval must not invite).

### ADMIN PHASE 4 — VENDORS SUPERSET built 2026-08-31 (owner "proceed with next phase"; UNCOMMITTED)
VendorsAdminPage/Table (merged list, 3 filter repairs) + VendorDetailAdminPage (merged detail) + VendorStatusActions/VendorEventApproval split (event toggle = stricter FT+FM COI-gated rule) + moved FeeOverride/LocationEditor/VerificationWrapper. 4 thin wrappers. 7 superseded files → git rm at commit (zero refs ✓). ⚠money notes: FeeOverride untouched but now on vertical detail too (API already allowed); reject/suspend keep pre-existing direct-write pattern. Gates → local commit → propose push.

### ADMIN PHASE 5 — MARKETS built 2026-08-31 (owner "go for 5"; UNCOMMITTED)
- MarketsAdminPage (client, vertical prop; the 1,834-line vertical page moved whole; rows/name cells drill in; ?edit=<id> deep-link = the one edit home) · NEW MarketDetailAdminPage (server; absorbs platform detail; manager section now traditional-any-vertical, not FM-only) · MarketDetailActions (approve/reject/suspend/delete on the detail page — closes the 2026-08-30 mobile "rotate to landscape" finding) · MarketScheduleManager/MarketVendorManager/MarketApproveStatusButton relocated to components/admin. NEW route /[vertical]/admin/markets/[id]; platform list keeps cross-vertical browse (links retargeted), platform [id] = thin wrapper.
- PHANTOM-COLUMN DEFECTS FOUND+FIXED (markets has market_type/zip, not type/zip_code — snapshot-gated): platform type filter errored the query (blank list); every platform row labeled "Private Pickup"; platform MarketForm create+edit errored on save (API writes type + zip_code) → MarketForm/new/edit RETIRED (git rm ×6 incl. the three old [id]/ components, zero refs verified); DeleteMarketButton retargeted to the GUARDED admin DELETE (old path had no ADM-3 financial-history guard).
- Form honesty: type locked on edit (admin PUT ignores market_type); private_pickup removed from create (admin POST rejects it).
