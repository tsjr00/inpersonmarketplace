...
- **PENDING:** commit + push AUT-1 → user applies mig 195 Dev+Staging (prod-pending list becomes 184→195) → slices 10 (admin) → 9 (notifications) → consolidated backlog-tail pass (CHK-1 remainder, VOR-8, CRN-3 [fold into next batch], CRN-10, CHK-7+CRN-5; T5 = own session). User: staging test of the whole train → combined prod push (migs in order first).
