...
- **BATCH 5 BUILT, GATES GREEN (tsc 0, vitest 1687/1687), UNCOMMITTED:** CHK-1 remainder — 3-way paid-flip branch in webhooks.ts + checkout/success (both file-approved, hooks verify-retried): pending→guarded flip w/ lost-race re-read; paid/completed→backfill unchanged; cancelled/refunded→payment recorded + `${orderId}-dead-order` shared-key full refund + ERR_WEBHOOK_017/ERR_CHECKOUT_006 (cataloged) + MB skip; success shows buyer "order expired — refunding". Rule A "CHK-1 OPEN" entries removed per rot-check (pre-declared). Ledger CHK-1 = fixed COMPLETE. Rollback: single-commit revert, code-only; only Stripe refunds in the pathological case are non-reversible.
...
- **MIGS 197 + 198 APPLIED to Dev + Staging 2026-07-18 (user-confirmed); snapshot changelog updated. NOTHING pending on Dev/Staging. PROD-PENDING LIST IS NOW 184→198, IN ORDER, at the combined prod push.**
