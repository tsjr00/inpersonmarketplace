...
**⏳ STILL OPEN:** `lib/stripe/payments.ts:524` charge descriptor `Spot rental — {market}` → `Vendor space — {market}` (**protected money file, needs its own per-file approval** — highest tax value of the renames). Then: jurisdiction storage → subscriptions tax → facilitated-sales tax (payout withholding, critical-path). CPA question list in the doc §6.
