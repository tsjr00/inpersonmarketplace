...
**NEXT SESSION — START HERE (Phase E build kickoff).** DESIGN COMPLETE — all decisions **O1–O6 resolved** (plan §8) + pre-build verifications done (plan §10). Just build. Sequence: **(A)** Migrations (plan §9 step 1): `market_seasons` (Option B separate table; presale fields + `MAX_PRESALE_LEAD_DAYS=60`, `PRESALE_GRACE_DAYS=14`, one-season-ahead, manager-flips-open), `booth_booking_groups`, `weekly_booth_rentals.group_id`, `markets.schedule_confirmed_at`, optional `booth_credits` → user applies Dev+Staging. **(B)** Season/partial booking route + group orchestration (reuse `book_weekly_booth_atomic`; season = all weeks; **late buyer days 0–14 = FULL season incl. elapsed weeks at full price**, not pro-rated). **(C)** Money-path pass = `payments.ts` + `webhooks.ts` season fns + credit redemption = critical/protected, per-file approval + exact diffs. **(D)** Manager season/prepay UI + vendor selector + settlement panel. Minor TBC only: O1 disclosure UX copy; after-start penalty % (25% default). Verified: `market_schedules.day_of_week` (int4, indexed); cutoff coupling `mig 109:135,154`; `markets.season_start/end` = availability gate (kept separate from `market_seasons`).
