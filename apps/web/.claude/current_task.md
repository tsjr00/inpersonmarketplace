...
- **NEXT: (1) commit CHK-18 batch (approval pending); (2) push staging (separate approval); (3) user staging-tests the order lifecycle (ready→ack→fulfill on a PAID order must still work; unpaid/cancelled paths must 400); (4) then slices 3–10; (5) combined PROD push (migs 184→189) unchanged.**
