...
- **Steps 3+4+5 = UNCOMMITTED** (Steps 1–2 already on staging as `27d83e78`). Files: NEW `seasons/[seasonId]/makeup-dates/route.ts`, `notifications/types.ts` (+2 types), `settlement/route.ts` (made_up), `cutoff-and-sort-functional.test.ts` (tripwire 86), `flow-integrity.test.ts` (+9), `current_task.md`. **NEXT:** commit 3+4+5 to staging, then Step 6 (UI, deferred — manager screens to end season / schedule make-up dates / settle made_up).
