...
- Side finding: `admin/events/[id]/route.ts` `validStatuses` (`:73-83`) still lists `'cancelled'`, but the DB CHECK (mig 094:74-76) does NOT include it → picking `cancelled` would 500. Small separate bug.
