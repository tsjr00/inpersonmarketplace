...
| **T-43** ⬜ | **Neither the organizer nor the admin sees that a pre-order happened** | After a completed purchase at their event, the event-manager dashboard showed **no change**, and the admin event view showed **no change**. The entire organizer pitch is that attendees pre-order and the organizer can see interest building — and the admin's viability numbers are supposed to be judged against reality. Right now a real order is invisible to both. ✅ **Owner agreed 2026-08-11:** there should be an indication that transactions have been completed, on BOTH the event-manager dashboard and the admin event view. |
