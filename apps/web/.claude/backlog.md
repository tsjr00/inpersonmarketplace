...
- **(g) Continue payment for an UNPAID SEASON purchase** (owner 2026-09-25: backlog). Same dead end the tester hit on one-off weeks (fixed as TR-118): `/[vertical]/vendor/bookings` shows an unpaid season group as "Pending payment" with no button (CancelSeasonButton renders only for PAID groups); release is Phase 18 of expire-orders (group-aware, Stripe-checked) — Prod-only, never on Staging. Design = the one-off pattern on `booth_booking_groups` (resume the SAME open session; expired → release via the existing `cancel_season_group` path). Money path — needs its own design read (book-season route, Phase 18, webhook season branch) before any build.
...
## ✅ FT — spot-booking must respect an active standing hold (F1) · designed 2026-09-19 · BUILT 2026-09-20 (F1-1 route guard `park-hold-guard.ts` · F1-2 sweep notices `park_standing_occurrence_skipped[_manager]`, once per hold+date · F1-3 held days marked/disabled on the booking page) — retest TR-109
...
2. [ ] **"Order Complete" (= `order_fulfilled`) sent from fulfil's ungated success path.** ⚠ PROTECTED `api/vendor/orders/[id]/fulfill/route.ts:483` — three sibling sends were gated 09-06 (`:215, :257, :443`); this fourth one was missed. Fix = the same `if (!orderData?.bundle_id)` guard. Per-file diff + hook gate.
3. [ ] **Vendor-first edge on a bundle item.** When a vendor fulfils BEFORE the manager's Receiving-now tap, the item goes fulfilled-unacked; the manager's later tap can't attach (collect-ack is READY-only by design, 09-06 flow-integrity pin) and the buyer orders LIST page (`buyer/orders/page.tsx:660`, no bundle awareness — the 09-06 gating went on the DETAIL page) shows the 🤝 "please confirm you received it" banner. Money is safe (buyer's bundle-ack sweep pays the vendor). Two parts, owner to choose at build: (a) make the list page bundle-aware (no per-item prompts on bundle orders); (b) DECIDE the edge itself. Owner-confirmed sequence (OB-016): vendor tapped Fulfil one moment before the manager's Receiving-now; the manager's tap was then REFUSED by collect-ack with "Already collected — the vendor marked this item fulfilled" (`collect-ack/route.ts:83-90`) — wording wrong for this case. Options: **(i) RECOMMENDED — on bundle items, vendor Fulfil refuses until the manager has tapped** (matches the training copy "wait for the manager's tap, then Fulfill"; prevents the edge; touches ⚠ fulfill route); (ii) let the stand-in accept fulfilled-unacked items (moves money in that route; loosens the 09-06 no-money pin; needs its own ruling + test). Either way fix the refusal wording.
6. [ ] **ONE acknowledge control on a bundle order.** Owner 2026-09-14 (OB-018): the detail page showed the green per-item acknowledge hero AND the yellow bundle-card acknowledge together — same words, different money behind them (per-item vendor pay vs margin release). On bundle orders suppress the per-item acknowledge everywhere (detail hero, list-page banner — same root as item 3a); the bundle card's button is the only one. `buyer/orders/[id]/page.tsx:555-556, :584-585, :632`.
7. [ ] **Bundle-ready EMAILS (TR-005 fail, OB-020) — TRACE FIRST, no guessing.** (a) the buyer got one ready email per vendor (2) on a bundle order — the 09-06 ready-route gate skips buyer `order_ready` sends on bundle orders, so find which path emailed (vendor ready tap? fulfil? a different type?) by reading `vendor/orders/[id]/ready/route.ts` + the notification type's channel config; (b) NO combined `bundle_ready` email after the manager's notify tap although the in-app notice arrived — read `notify-ready/route.ts` + the `bundle_ready` type's channels (email suppression? template missing?). Fix only after both are read end to end.
5. [ ] **No review prompt at the buyer's bundle acknowledgment** (owner ruling 2026-09-14, OB-016; previously raised as OB-004). Verified 2026-09-14 (OB-020): the bundle-ack path shows NO popup; the popup came from the per-item acknowledge → resolved by item 6. The who/when of bundle reviews stays a design item (test-notes #3).
4. [ ] **Placed-order email copy on bundle orders** ("placed with 2 vendors… confirmed by 2 vendors") — manager-centric wording wanted (OB-012). Small; ride along.
...
2. [ ] **Vendor dashboard "Analytics & insights" card: label the two halves.** Subheading above the divider = Analytics, below = Insights — and RENAME the insights page to drop "Location" ("location is just some of the insights").
3. [ ] **Insights page restructure: two major sections.** "Location insights" + "Customer insights" with smaller sections inside each — VIP/customer material currently reads as inserted into the location data. Owner: intentional break-out so it doesn't interfere with location insight reading.
4. [ ] **"Markets you're missing" Add button — revisit for managed markets.** Designed pre-market-management; self-adding is wrong when the market manages bookings on-platform. Recommendation direction: keep the signal, swap the action per market state (app-managed → "Apply to this market" via the existing application flow; unmanaged → today's add-self). Needs a read of the current button's write path before final design.
5. [ ] **Week strip: show APPROVED standing holds before their occurrence materializes.** Occurrences only exist ≤7 days out (PARK_STANDING_GENERATION_HORIZON_DAYS, park-standing.ts:31) via cron — an approved Saturday hold >7d away is invisible to the vendor. Option: strip synthesizes hold entries from park_standing_reservations directly.
6. [ ] **VIP perks card: "Save changes" button when a perk is enabled** (defect-adjacent UX; fix proposed in the 2026-09-05 triage — the only button today toggles on/off, so edited numbers can't be saved while on).
7. [x] **(b) Reverse event-conflict check — BUILT 2026-09-05** (owner: "we need to do that one"): `lib/events/booking-event-guard.ts` gates book-park-spot / book / book-season; flow-integrity-pinned. **(a) Backfill sweep — SKIPPED per owner** ("only matters for my testing") ⚠ with one carried caveat: **when mig 238 pastes to PROD, check for real acceptances made before the paste** (same gap would exist there) — added to the prod-push checklist below. Original finding: blackout detection is one-shot at accept (blackouts.ts:10-11); truck3's pre-238 acceptances had no rows.
8. [ ] **Buyer copy for day-of-only FT items on off days.** SQL2 (2026-09-05) proved the confusion: FT items with advance_order_days=0 are orderable ONLY on the truck's operating day (get_available_pickup_dates: "FT parks regular: today only"), but buyers just see "closed" with no why — the owner read it as a possible blocking bug. Show "Order on market day — {next date}" (or similar) instead of a bare closed state on buyer surfaces.
...
3. [ ] **Organizer dashboard: per-truck roster with stage.** `event-manager/[id]/dashboard` shows only the EVENT status line (`STATUS_LABELS`, page.tsx:51-61, :205) + counts; no per-truck list of invited/declined/accepted/selected/backup. Owner's live test event: 3 invited, 1 declined, 2 accepted, 1 selected, 1 backup — trackable at 3 trucks, hopeless at 10-15. Roster block on the organizer dashboard (data = market_vendors response_status + organizer_selected_at + is_backup).
4. [ ] **"Vendors confirmed" → "accepted the invitation"** on the Vendor Event Page. `vendor/events/[marketId]/page.tsx:773` renders `Vendors confirmed: {accepted_count} of {vendor_count}` (and :534 "confirmed so far") — but accepted ≠ selected/confirmed. Owner: an undecided vendor reading "2 of 4 confirmed" may think their chance is gone and not accept — engagement killer. Reword to accepted-language (note :146-148 use "Confirmed" correctly for the SELECTED+paid stage; keep those distinct).
5. [ ] **Locations-page event pill says "Attending" for merely-accepted trucks.** `EventMarketsSection.tsx:242`: `status === 'accepted' || market.hasAttendance ? 'Attending'` — an accepted-but-not-selected truck reads "Attending" though the organizer may never select them. T-68 fixed this pill from a schedule fact to the invitation response, but conflated accepted with selected. Vendors planning their week trust the pill; it must be accurate: distinct labels for accepted-awaiting-selection vs selected/attending vs backup.
6. [ ] **Vendor week-at-a-glance schedule on the locations page.** Trucks constantly ask "where am I on what day" but have no self-view — each location must be opened individually; the only schedule grid is on the PUBLIC vendor profile. Add a read-only next-7-14-days schedule at the top of `/vendor/markets` (reuse the public-profile schedule's data/rendering), covering parks + private pickups + events. Rename the dashboard card "Locations & Schedule". Not interactive — at-a-glance only. Owner: "high value."

## 📊 VENDOR "YOUR CUSTOMERS" DISTRIBUTION REPORT (owner, 2026-08-25) — **PLANNED 2026-09-04: Phase A1 of `vip_loyalty_buildout_plan.md`** (VIP unblocked from flash sales; perk-menu model; decisions.md 2026-09-04)
...
### ✅ A — THE ADDRESS DEADLOCK — RESOLVED 2026-08-08 (verified in code 2026-09-20: address required at intake `event-requests/route.ts:84-99` · admin may supply it with approval `admin/events/[id]/route.ts:153-159` · organizer dashboard + editor keyed by id, needs-address banner, id-addressed Cancel `event-manager/[id]/dashboard/page.tsx:249-262, :475-479, :557-570`). Kept for the record; nothing left to decide.
...
### ✅ A-AUDIT part 2 — CANCELLING IS A ONE-WAY DOOR — FIXED (verified 2026-09-20: reactivation sets `markets.active = true`, `admin/events/[id]/route.ts:294`)
...
### ✅ A-AUDIT part 4 — NOTHING NOTIFIES AN ACCEPTED VENDOR OF ANY CHANGE — FIXED (verified 2026-09-20: the details PATCH sends `event_changed_vendor` to every accepted vendor, `events/[token]/details/route.ts:639`)
...
## ⏸️ T-39 PARKED 2026-08-12 — migration 225 WRITTEN, NOT APPLIED ANYWHERE

**Do not apply without running the verification below. Owner, 2026-08-12: *"I don't like just applying things because you say so — I'll never do that."***

`supabase/migrations/20260812_225_fm_events_require_acceptance.sql` is ready and committed. It is mig 223's file with **exactly one line deleted** — the FM blanket exemption `OR (m.market_type = 'event' AND m.vertical_id != 'food_trucks')` — spliced from 223's actual text and diffed to prove it (349 vs 350 lines, one deletion, zero other executable changes). Three comment lines that asserted the exemption are corrected; the rollback block points at 223. **Two guard tests** are already in `flow-integrity.test.ts` and PASSING: the newest definer must keep the acceptance branch, and must grant events no vertical exemption. Both now assert against comment-stripped SQL (the absence test first failed on the migration's own header comment — the known "absence assertions match comments" trap).

⚠ **Risk runs OPPOSITE to mig 223.** 223 added a disjunct (can only ADD rows). 225 removes one (can only REMOVE rows). An FM event listing stops selling unless its vendor accepted.

**Why it's parked:** there are **zero live FM events on Dev, Staging or Prod** — staging's only one (`Anew Perspective Pop-Up Market`, 2026-05-30) is expired and prod returned no rows at all. So the hole is not exploitable today, and a differential would be all-zeros by construction — it would prove nothing.

**The verification, when there is time.** Borrow the expired staging event, which already carries a perfect control pair (2 listings whose vendor never accepted, 4 whose vendor did): (A) record its real dates, (B) `UPDATE` them to `CURRENT_DATE + 3 / + 4`, (C) run the pre-check in mig 225's header — **expect all six > 0, including the two unaccepted; that is the hole made visible.** If the unaccepted two show 0, STOP: the branch reading is wrong. (D) apply 225, (E) re-run — **expect the two unaccepted at 0 and the four accepted unchanged**; anything else moving = roll back to 223. (F) restore the dates from A.
...
**Status: NOT STARTED. Design direction from the owner, captured verbatim in intent. Nothing built.** *(Display-only service-path cards shipped 2026-08-13 — the toggle itself is still not wired.)*

### ➕ EXTENSION (owner, 2026-08-13, post-presentation): TAG EVERY QUESTION BY PATH

Owner: *"let's start separating the questions that are more focused towards admin-involved vs. self-serve and individual-pays vs. company-pays. If we tag questions with which path they are relevant for, we can come up with custom question queues that make for better matching of events & trucks."*

- **Two tag axes per question:** service path (`self_service` / `full_service` / both) and payment model (`attendee_paid` / `company_paid` / both).
- The tags drive **custom question queues** — an organizer only sees questions relevant to their path, and matching gets cleaner inputs because irrelevant answers stop polluting it.
- **This generalizes T-55**: "expected meals purchased" is the first concrete example — owner believes it belongs to company-paid only and should not factor into a self-serve event. Under this scheme it would simply be tagged `company_paid` and drop out of the self-serve queue.
- **Where the questions live today:** the intake form (`EventRequestForm.tsx`) asks a minimal set for everyone; `OrganizerEventDetails.tsx` holds the post-intake FIELD_GROUPS (budget, event context, logistics). Tagging likely starts as a `paths`/`paymentModels` annotation on those field definitions, then the render filters by the event's actual `service_level` + `payment_model`.
- **Sequencing:** depends on the service-level fork being real (currently hardcoded `self_service` — see CRITICAL FINDING below). Tag definitions can be designed before the fork is wired, but queues can't diverge until it is.
...
| ID | Finding |
|---|---|
| **T-60** ✅ | **RESOLVED 2026-08-12 — migration 224, applied Dev + Staging + Prod.** **Cause: migration 211 (2026-07-25), not any 2026-08-09/10/11 work.** 211 deactivated every active FT `vendor_market_schedules` row lacking a paid park booking — one set-based UPDATE, so every row it touched shares one microsecond-identical `updated_at` (Prod `2026-07-31 18:48:48.633378+00`, 9 rows; Staging `2026-07-26 03:23:10.619661+00`, 7 rows; Dev none). **Its premise was false for most of the platform:** trucks sell at parks we do NOT manage, where a paid booking can never exist, so every legitimately declared schedule at every unmanaged location matched its "phantom" test. It also had no `market_type` filter and hit `private_pickup` spots. **Owner-corrected model, 2026-08-12:** `vendor_market_schedules` is a vendor DECLARATION ("the days/times I'll be here"), NOT attendance or check-in — there is no check-in concept in the schema; no-shows are handled by order confirmation + auto-expire + uncaptured payment intent; check-in data is manager value and must never gate search. **Damage:** locations gone from buyer search, trucks gone from where-are-trucks-today, and private-pickup hours silently reverted to location defaults while ordering kept working (which is why it hid for 2 weeks). **Fix:** restore rows at locations with no manager account, bearing a 211 timestamp, whose vendor still has a published listing there. Pre-registered counts matched exactly — **Prod 7, Staging 5, Dev 0** — and `visible_in_search` flipped true for both Prod parks and for Food Truck Exchange + Hub City on Staging, with the managed Sixth Street Food Park correctly still false. **No code changed — the visibility rule was correct as written.** Postmortem headers added to migs 210 and 211; snapshot bookkeeping corrected (211 was falsely marked "Prod PENDING", which is part of why this hid). ⚠ **Follow-ups:** the browse-page open/closed pills were never investigated (owner said they may be a separate UI issue), and mig 210 left approved trucks with no schedule and nothing prompting them to set one. **ORIGINAL REPORT:** Locations not visible, and items showing CLOSED that should be orderable. Owner: *"it's much better to have events visible where they should not be than people not able to find where trucks are — that's a big part of what the whole app does."* Truck cards on FT browse show multiple locations, but opening an item shows everything closed despite the schedule saying otherwise. **Vendor-specific, not global:** Fuego St Tacos = all closed, Bao Down = all closed, Smokestack BBQ = mostly OPEN. Owner suspects the 2026-08-11 event-visibility work. ⚠ **Claude's initial reasoning, NOT verified:** mig 223 only ADDED an `OR` disjunct, and adding a disjunct to an OR cannot close anything — so 223 is unlikely to be the cause, and `market-stats` feeds only the vendor listing form, not buyer availability. The vendor-specific pattern instead points at the **`vendor_market_schedules` requirement** (mig 131: traditional markets in ALL verticals need an active vms row) or a cutoff/park-booking condition. **DO NOT assume — run the diagnostic in the handoff first.** |
| **T-67** ✅ | **FIXED 2026-08-12 — `api/vendor/markets/route.ts`.** The route selects `*` from `markets`, so everything it returned carried address, headcount and the contact/manager email columns; its event list filtered on `market_type`, `event_end_date` and radius **only** — no invitation or acceptance check anywhere. **Fix:** an event now appears if it is **public** (`is_private` not true) **OR** the vendor has a `market_vendors` row for it (any `response_status`). ⚠ **Deliberately NOT acceptance-gated like the T-09 fix in `market-stats`** — that route feeds the listing picker where nothing is possible before accepting, but this section is headed "Upcoming festivals, fairs, and special events" and is how a vendor DISCOVERS an event; acceptance-gating would delete discovery. Declined counts as invited: the leak is about vendors never invited, and someone invited already saw the details legitimately. **Guarded** by 2 tests in `flow-integrity.test.ts` → "Organizer identity protection" — one pins the privacy filter's presence, one pins the exact bug shape (type/date filter feeding straight into the radius filter) as absent. Verified the negative test would have matched the pre-fix code. ⚠ **Expect a "regression" report on testing:** private events WILL disappear for vendors who were seeing them — that is the fix working. **STILL OPEN:** an earlier session's comment claims an event market's NAME is built from the organizer's company at approval; if true, an invited-but-undecided vendor still sees the host's identity in the name alone. **UNVERIFIED — not chased (owner deferred 2026-08-12).** Original report: **Vendor can see events they were NEVER invited to — with organizer name, address, and a "Set schedule" button.** Found at the bottom of the vendor company/business profile area (where event readiness now lives). Three events listed: one they were invited to, plus **two they were not** — showing who the organizer is, the address, and the date. **Same class as T-09, a surface that fix did not cover.** T-09 closed `market-stats`; this is a different page. |
| **T-73** 🟢 | ✅ **OWNER CONFIRMED 2026-08-12: the behaviour is CORRECT and intended** — *"that's what I asked for, just not what I was used to seeing."* A truck sets its own days; approval does not fabricate them. **Do NOT "fix" this by restoring auto-create.** What remains is optional polish only: nothing PROMPTS the truck to set days, so a location can stay dark with no signal. Low priority. Original note: **An approved truck gets NO schedule, and nothing asks it to set one.** Fallout from mig 210 (2026-07-25), which correctly stopped approval from auto-creating schedule rows — but nothing replaced the prompt. A truck can be approved at a location and stay **invisible in buyer search indefinitely** with no signal that a step is missing, because the visibility rule needs an active declared schedule. This will keep producing dark locations until it's closed. Likely fix: prompt on approval (in-app + the approval email), and surface "you haven't set your days here" on the vendor's location card. Found while diagnosing T-60. |
| **T-75** ✅ | **FIXED 2026-08-13 (same day as verification — owner approved masking, not-accepted vendors only).** New helper `lib/events/event-name.ts` → `maskedEventName(city, eventStartDate)` renders "Private event — Amarillo, Aug 20". Applied on BOTH leaking surfaces: the invitation payload (`api/vendor/events/[marketId]` — `market_name` now gated on `hasAccepted`, same rule as the address three lines below it) and the vendor events list (`api/vendor/markets` — private events mask name AND street address until accepted; public events never masked, the organizer chose to be public). Invite NOTIFICATIONS were already masked — verified all four call sites pass 'Private Event'-style values, so no change needed there. Attendee surfaces always see the real name (the shop page identity is the market name — that is why the mask is per-viewer, not a rename at the source). **3 guard tests** added to "Organizer identity protection": both maskings pinned present, and the exact bug shape (unconditional `market_name: market.name`) pinned absent. VERIFICATION RECORD: — the name IS the leak, and it needs a DESIGN DECISION before a fix.** `approveEventRequest` builds it at `lib/events/event-actions.ts:117`: `` `${request.company_name} ${eventSuffix}` `` — every event market is literally named "Acme Corp Vendor Event". So the disclosure policy (organizer identity withheld until acceptance — T-09/T-67) is defeated by the `name` field on every surface an invited-but-undecided vendor sees: the invitation page, the vendor events list, and every notification that interpolates `marketName`. Mig 219's own comment confirms the name is built from `company_name` at approval and never re-synced. **Why not just rename the market:** ATTENDEES are supposed to see the company name — the shop page's identity is the market name — so a generic name harms the buyer side. **The likely fix shape is per-viewer masking:** un-accepted vendors get a masked display name ("Private event — Amarillo, Aug 20") at the API layer, the way the address is already masked pre-acceptance. Surfaces to cover: `api/vendor/events/[marketId]` (invitation), `api/vendor/markets` (events list, T-67's filter), invite/response notifications. **Needs owner sign-off on the masked-name copy and the surface list before building.** |
| **T-74** ⬜ | **Browse-page open/closed pills — never investigated.** Reported alongside T-60 (items showing CLOSED that looked orderable). Owner flagged them as possibly a separate UI problem and the search issue took priority; T-60's fix (mig 224) may or may not have resolved them. **Re-test before spending time on it** — if items still show closed when the schedule says open, this is its own bug. |

### 🟠 Matching — the biggest functional gap

| ID | Finding |
|---|---|
| **T-63** ✅ | **FIXED 2026-08-13 (owner chose daily cron over per-trigger hooks).** Precise gap found first: matching ran on approval, admin edit/rematch, and organizer refresh — but the refresh banner only appears when the ORGANIZER changes matching fields; **no vendor-side change (event-approval granted, 4th item published, readiness completed) ever triggered anything.** New **Phase 15.7** in the daily `expire-orders` cron: for every open event (approved/ready, market exists, event not past) still short of the vendors requested, re-run `autoMatchAndInvite` — it is idempotent (already-invited vendors skipped inside it), so the only effect is NEW invitations to newly-eligible vendors. Capped at 25 events/run, one batched acceptance-count query. ⚠ **Behavior change on deploy:** the morning after this ships, any under-filled open event starts picking up newly-eligible vendors automatically — that is the feature, but expect the invitations. Hooks-in-four-routes rejected as the paired-surface drift pattern. |
| **T-64** ✅/❓ | **The confirmed defect is FIXED; the reported count remains a hypothesis.** Confirmed and fixed: the admin match preview **hardcoded** the scorer inputs (`30/wave, 6hr, no experience`) — a third, diverged copy of the matching rule, so admins saw scores the engine never produced. `api/admin/events` now sends each vendor's real `readiness` (null = never completed) and the panel scores from it; **null readiness renders "no readiness — auto-match skips" instead of a fictional score** (consistent with the T-70 gate; admins can still hand-invite). The 2-vs-4 item count: the counting rule (`listing_data.event_menu_item`, published, non-deleted) is IDENTICAL on all three surfaces and recomputed on every panel load — **Medium-confidence hypothesis: the vendor's 4 included unpublished/draft items and admin counts published only.** Owner does not recall; re-test when it recurs, with `SELECT status, listing_data->>'event_menu_item' FROM listings WHERE vendor_profile_id = …`. |
| **T-70** ✅ | **FIXED 2026-08-13 — the truth was worse than "ignored": emptiness was REWARDED.** Readiness WAS wired into the scorer (capacity, runtime, experience, generator, odors, perishability) — but an empty questionnaire defaulted to **30/wave and 6 hours**, which usually score green, so matching invited vendors the accept flow would then BLOCK (accepting requires capacity). Owner decision: **completed readiness is now a HARD GATE for matching** — skip reason "Event readiness questionnaire not completed"; the `\|\| 30` / `\|\| 6` generosity defaults removed (capacity now provably real; runtime fallback kept as a type guard only). Aligns matching with the accept flow and the T-62 admin warning. ⚠ **Consequence on thin data:** vendors event-approved via the T-62 "Approve Anyway" override (no readiness) will NOT be auto-matched until they complete the questionnaire — including the FT demo trucks approved on prod 2026-08-13 unless their readiness was filled. **4 guard tests** in `flow-integrity.test.ts` → "Matching readiness integrity": the gate present, the `\|\| 30` default absent, the admin hardcode absent, the cron sweep present. |
...
| **T-65** ✅ | **FIXED 2026-08-13 — a money number.** Root cause: the page divided by `accepted_count` alone, which is a straight tally of vendors who have ALREADY accepted and **excludes the vendor reading the page** (`api/vendor/events/[marketId]:150,184`). Truck #2 of a 4-truck event saw "100 ÷ 1" and was told to expect ~100 shoppers. **A vendor buys food off this number.** Neither denominator is right alone, so it is now a RANGE (owner-approved): fewest = those committed **counting you**, most = what the organizer requested; `expected` is floored at `committed` so the range can never invert. Shows a single number when both ends agree. The basis line names why: *"100 guests ÷ 4 trucks if all confirm, ÷ 2 if none else do (1 of 4 confirmed so far)"*. The Revenue Estimate box folds the truck-count uncertainty into the existing conservative/optimistic pair — worst case is the most vendors at the lowest spend, best case the fewest at the highest — keeping it at two numbers instead of four. ⚠ **The math is INLINE in the page component and has NO unit test.** Extracting it to `lib/` + testing the range logic is a worthwhile follow-up given it is a money-adjacent estimate; deliberately not done in this pass to avoid scope creep. ORIGINAL: **Invitation page ignores already-confirmed trucks in its per-truck maths.** Second truck saw "100 total ÷ 1 trucks" while truck #1 had already accepted. Each vendor's view appears to count only itself. |
| **T-58** ✅ | **CLOSED 2026-08-13 — VERIFIED CORRECT by tracing every path.** One source (`catering_requests.event_start_time/end_time`), one function (`calculateWaveCount`, `lib/events/viability.ts:161`), one formula (`ceil(duration/30)`), and all four callers — invitation display, capacity seeding, viability, and actual wave generation (`generateEventWaves`, same `ceil(minutes/30)`) — use the same 30-minute default fed from the same request row. The count a vendor accepts against equals the waves generated. ⚠ **Latent footgun recorded, not fixed:** `waveDurationMinutes` is a real parameter on wave generation, but NO caller passes it and the invitation math hardcodes the default — if anyone ever wires a custom duration, the invitation count silently diverges. If that parameter gets used, the invitation must read `markets.wave_duration_minutes` instead of assuming 30. |

### 🟠 Vendor event-approval process

| ID | Finding |
|---|---|
| **T-61** ✅ | **CLOSED 2026-08-13 — NOT A BUG, verified by reading.** First readiness submission notifies the vendor AND every admin, and the template keys are correct (no T-08-style mismatch). In the owner test the vendor readiness was EMPTY — no submission had happened, so no notification was due. Caveats recorded: fires on FIRST submission only (edits never re-notify), and recipients are all admin/platform_admin users regardless of vertical — that is the pre-existing "admin notifications go to the wrong vertical" backlog item, not new. ORIGINAL: **Unclear whether an admin is notified when a vendor submits event-readiness.** Owner had no memory of being prompted and the vendor's readiness was empty. Confirm the notification exists; if not, add it. |
| **T-62** ✅ | **FIXED 2026-08-13 — two defects.** **(1) Blind dialog:** the approve confirmation was generic and did not know whether the vendor ever applied, on BOTH admin surfaces (vertical-admin `VendorAdminActions.tsx` and platform-admin `VendorActions.tsx` — same rule, two places, fixed in step with mirrored comments). When approving a non-applicant the dialog now reads "Vendor Has Not Applied… their readiness questionnaire is empty, so matching has no capacity or experience data" with danger styling and "Approve Anyway" — the override stays possible, it just cannot happen unknowingly. "Applied" = readiness exists with `application_status` ≠ `not_applied`. **(2) The pending-vs-approved mismatch is a WRITE-ORDER bug:** grant approval before the vendor applies → the grant route skips its status sync (nothing to sync) → vendor later submits readiness → the route stamped `pending_review` over the existing approval. The readiness route now selects `event_approved` and writes `approved` instead, and skips the "has applied, please review" admin notification in that case — there is no application to review for a decision already made. ORIGINAL: **An admin can grant event-approved status to a vendor who never applied**, with no warning. Owner did exactly that by accident. Later the profile showed "application pending" against an already-granted approval. Needs a confirmation warning naming the mismatch. |
| **T-66** ✅ | **FIXED 2026-08-12.** The link existed but pointed at the vendor DASHBOARD, leaving them to hunt for a questionnaire that had moved to the business profile. Now `/{vertical}/vendor/edit#event-readiness`, labelled "Update event readiness →". Anchor `id="event-readiness"` added to the section wrapper in `vendor/edit/page.tsx` with a comment saying it is a link target — renaming it breaks this silently. ORIGINAL: The invitation tells a vendor to update their event readiness but **does not link to it**; it has moved from Locations to the business profile. Link directly. |

### 🟡 Event-manager funnel — the through-line is "the app never tells them what to do next"

| ID | Finding |
|---|---|
| **T-48** ✅ | **FIXED 2026-08-13 — but NOT the shape the report described.** The "Already have an account? Sign in" link **already existed** on the success screen and had since 2026-04-12 (`64110abf`), four months before testing — it read as a footnote, not a door. **The real dead end was one step later:** the signup page detects a duplicate email and renders TEXT telling the user to go log in, **with nothing to click**, and the event context (`ref=event`) died there. **Two fixes:** (1) `signup/page.tsx` now renders a **Log in instead →** button in that error, carrying the typed email + `ref=event` (which login DOES honour — it routes organizers to /event-manager). ⚠ `returnTo` deliberately NOT passed: login ignores it except for vendor-signup market invites (`login/page.tsx:34-45`), so forwarding it would imply behaviour that does not happen. (2) The event success screen now shows **two equal-weight buttons** — Create Your Free Account / I Already Have an Account — because at that point we do not know which they are. Fix (1) helps every signup on the platform, not just organizers. ORIGINAL: **Existing account, new role = dead end.** A user with a market-manager account filling in the event form is offered only "create your account", and is sent to the signup page. Logging in manually works and the dashboard resolves by email — so the system already *recognises* them. Offer sign-in. First-impression friction on the exact path we want smooth. |
| **T-49** ✅ | **FIXED 2026-08-12** — `components/events/EventRequestForm.tsx`. Now reads "Based on what you've told us so far, we've preliminarily matched you with N …" and the sub-copy asks them to complete the profile because it improves matching. ⚠ Kept the pre-existing constraint the file documents: copy must NOT imply we keep searching in the background — the engine runs ONCE per submission; re-running is an action the organizer takes. ORIGINAL: **The match splash overpromises.** Should read *"based on what you've told us so far we've preliminarily matched you with N truck(s) — log in to your dashboard to complete your event profile and we'll finish matching."* Both sides need more than the intake form gives: vendors want budget + logistics, organizers want the event-context questions. |
| **T-52** ✅ | **FIXED 2026-08-12** — an intro line at the top of the organizer dashboard: use Add or edit details, the more you tell us the better we can match, update any time and re-run matching. Uses `term()` so FT reads "trucks". ORIGINAL: Dashboard should open with something like *"add or edit details — the more you provide, the better the match."* The existing amber "your changes affect vendor matching — refresh matches now" is good and should be leaned into. |
| **T-53** ✅ | **FIXED 2026-08-12** — new `components/events/OrganizerProgress.tsx`, `part="next"`, rendered as a "What happens next" card ABOVE "Manage this event" so housekeeping is no longer the last word on the page. ORIGINAL: **Nothing at the bottom of the event dashboard tells the organizer what to do.** They scroll to the end expecting an action and find ratings, "copy event link", "cancel event". The two real actions sit in small red text at the top. Duplicate them at the bottom as prominent buttons, ideally sequenced. |
| **T-54** ✅ | **FIXED 2026-08-12** — not an empty-page problem. Owner: *"they were not blank, just nothing to do on them because the vendors needed to respond first."* The waiting stage now states it: *"Your event is live and invitations are out — that part is done. It's with the vendors now."* **Review responses** only appears once someone has responded. ORIGINAL: Those two links currently lead to pages with nothing on them yet. Until there is something to see, say *"You're done for now — we're matching your event to vendors and you'll be notified as they respond."* |
| **T-71** ✅ | **FIXED 2026-08-12** — 5-step stage strip under Event details, DERIVED from `catering_requests.status` (no second source of truth). `new`/`reviewing` fold into step 1 (admin-side, meaningless to an organizer); completed steps stay visible and ticked (owner decision — seeing the finished part is the reassurance); `cancelled`/`declined` show a statement instead of a frozen bar. ⚠ A new status must be added to `STAGE_FOR_STATUS` or progress silently under-reports. ORIGINAL: **After confirming trucks, the dashboard looks identical.** Still reads "2 of 4 vendors confirmed"; no "your trucks are confirmed, here's what's next". Owner's suggestion: a progress strip under Event Details — outlined boxes that fill in (finish profile → review responses → confirm trucks → pre-orders open). |
| **T-59** ✅ | **FIXED 2026-08-13 — both halves.** **(a) In-app notice:** the respond route (accept/decline) notified ADMINS only; the organizer waited on the results email. The vendor *cancel* route had been doing the organizer notification all along — the same rule living in two routes with one kept current, the pattern behind most of this week findings. Added **`event_vendor_responded_organizer`** rather than reusing `catering_vendor_responded`, which is `audience: admin` and links to /admin/events (that mis-route WAS T-08 half two). Carries the vendor message and links to the organizer own event dashboard. ⚠ **Only reaches organizers with an account** — `organizer_user_id` is null until a logged-in user with the matching email loads /event-manager, so **email stays the guaranteed channel and this is additive.** Registry inventory tripwire bumped 104 → 105 with a recorded reason (user-approved). **(b) The vendor message:** written by the vendor above Decline, stored on `market_vendors.response_notes` since the feature shipped, and rendered on exactly one surface — the ADMIN events page. The organizer never saw the note written for them. The select route now selects and returns it and the organizer select page renders it as "THEIR MESSAGE" above the menu, where they are choosing. **(c) Also shipped here:** copy on the event success screen naming the exact email they entered, because **event claiming is by EMAIL MATCH** — /event-manager sets `organizer_user_id` where `contact_email` = the logged-in user email, automatically on load, nothing to click. The signup link only PREFILLS that address and they can change it; sign up with a different one and the event is silently never linked — they log in, see zero events, get bounced to the shopper dashboard with nothing explaining why. ORIGINAL: **No in-app notification to the organizer when a vendor accepts** (email arrived). Owner: *"notifications are free and we want to use the free resource."* **And the vendor's acceptance message is nowhere to be found** — the text typed above the Decline button. |
| **T-56** ⬜ | The intake form has **no self-serve vs admin-assisted qualifier**, which downstream logic depends on. |
| **T-50** ✅ | **FIXED 2026-08-13.** **The count:** the header read "0 of 2 filled" on a section holding EIGHT questions because `countTotalInGroup` excludes booleans and six of the eight are checkboxes. ⚠ **The exclusion is CORRECT and was left in place** — those booleans are typed non-nullable (`beverages_provided: boolean`, not `boolean | null`) and `isFieldFilled` returns true for any boolean, so counting them would read **"6 of 8 filled" before the organizer touched anything** — wrong in the opposite direction. Owner chose option 2: a group that is mostly checkboxes now states its **SIZE** ("8 questions") instead of a fraction that cannot be true either way. Groups with no boolean fields still show "filled". ⚠ **The honest long-term fix is making those six booleans nullable** so "not answered" is a real state — that needs a migration and form changes, deliberately not done here. **The helper text:** "Other Food at Venue" sat directly under "Other Food Vendors Present?" and read as a sub-question of it; it now prompts *"e.g. attendees bringing dishes, a nearby food court, catering already booked…"*. "Theme Details" got one too. ORIGINAL: Event context: "other food at venue" reads as a subset of "other food vendors present" — needs helper text (*attendees bringing food, nearby restaurants, etc.*). And the section header says **"0 of 2 filled"** while counting only the 2 text inputs, ignoring **6 checkboxes**. If that count feeds anything, it is wrong; either way count all 8. |
...
| **T-57** ✅ | **FIXED 2026-08-12** — Decline moved from above the menu picker to beside Accept at the bottom. ⚠ Deliberately NOT disabled by the accept-side conditions (item count, agreement, capacity): a vendor who CANNOT accept must still be able to decline. ORIGINAL: **The Decline button sits at the top of the invitation, above everything.** Move it next to Accept at the bottom — don't invite a decline before they've read anything. (Introduced by the 2026-08-11 toggle removal.) |
| **T-68** ✅ | **FIXED 2026-08-12.** Root cause: the pill read `hasAttendance`, a SCHEDULE fact answering an INVITATION question. `api/vendor/markets` now returns `responseStatus` per event (from the `market_vendors` rows it already fetched) and the pill reads accepted→Attending, declined→Declined, any other status→**Invited**, null→Not attending. **Null means no invitation exists** — a public event found by browsing — so "Not attending" stays correct there. ORIGINAL: Event pill on the vendor's list reads **"not attending"** for an event they were *invited* to and haven't answered. Should read "invited". |
| **T-69** ✅ | **FIXED 2026-08-12** — `EventReadinessForm.tsx`, now "Can sit 30+ minutes (packaged or wrapped items)". Comment added forbidding hot/prepared examples on this option. ORIGINAL: **Food perishability copy is wrong and reputationally risky.** The "30+ minutes" option lists *tacos and sandwiches* as examples. Owner: we should not be seen endorsing hot plates sitting 30 minutes — health-department optics. Leave packaged items only. |

### 🟠 Checkout

| ID | Finding |
|---|---|
| **T-72** ✅ | **FIXED 2026-08-13.** The success page had `market_id` and `market_type` per item but **not the event token**, and the shop route is `/{vertical}/events/[token]/shop`. **Touched a PROTECTED file** (`api/checkout/success/route.ts`) with owner file-level approval after a risk write-up: one scalar column added to an existing embed, plus a SEPARATE lookup for the token. ⚠ **Deliberately not a nested embed** — `markets` and `catering_requests` have TWO FKs between them (`catering_requests.market_id` and `markets.catering_request_id`), the ambiguous case PostgREST cannot infer; select strings are not typechecked, so a wrong guess would surface only when someone actually paid. **Fully additive:** supabase-js returns query errors in the response object rather than throwing, so a failure leaves the token map empty and the page falls back to `/browse` — today behaviour. Risk assessed at ~1-2% (concentrated entirely in the added column name, verified in the snapshot AND by two production routes that already select it); ~85% that it WORKS first try, with a benign failure mode. `npm run build` run manually before commit because it is a money-path file. ⚠ **Only the session_id path gets the token** — the direct order-id path reads a different API and falls back to browse. **VERIFY ON STAGING:** place an event order and check Continue shopping returns to the event, not the general catalogue. ORIGINAL: ⚠ **NOT FIXED — bigger than the backlog implies. Investigated 2026-08-12.** The link is `checkout/success/page.tsx:576`, hardcoded to `/{vertical}/browse`. The success page knows `item.market_type === 'event'` but **does NOT know the event token**, and the event shop route is `/{vertical}/events/[token]/shop`. Fixing it means resolving order → order_items → market → the event token and plumbing it into the success page — a data change on a checkout-adjacent page, not a copy edit. Needs its own scoped change. ORIGINAL: **"Continue shopping" on an event success screen exits the event.** It returns to general FT browse — all trucks, invited or not. A buyer who forgot dessert would shop the wrong catalogue and could order from a truck that isn't at the event. Must return to the event shop page. |
...
| ID | Finding | Detail |
|---|---|---|
| **T-01** ✅ **fixed 2026-08-10** | **Attendee shop page 404s — root cause FOUND** | `app/[vertical]/events/[token]/shop/page.tsx:29` guards with `/^[a-z0-9-]+$/`, written **2026-03-31** (`754f820b`) when tokens were `Date.now().toString(36)` — lowercase. On **2026-06-05** (`12ee9069`) the suffix became 18 base64url chars from `randomBytes(15)` for entropy (the token is an organizer's only credential). base64url contains **uppercase and `_`**, so the guard rejects every token minted since. **Every event approved after 2026-06-05 has had a dead attendee shopping page — 2+ months.** One-line fix. |
| **T-02** ✅ **fixed 2026-08-10** | **Clicking any menu item 404s** | Same cause as T-01: `[token]/page.tsx:310` links every item to `/{vertical}/events/{token}/shop`. Fixed by the same one-line change. **Fix:** guard widened to `/^[A-Za-z0-9_-]+$/` with the history written in place. **Guarded by** `flow-integrity.test.ts` → "Event token format", which EXTRACTS the live regex from the source and runs real shapes through it rather than asserting the regex text (which would just re-encode today's answer). **Both edges are pinned:** acceptance of real base64url tokens, AND rejection of path traversal, whitespace, percent-encoding, quotes, angle brackets, null bytes and the empty string. Verified to fail in both directions — the old `[a-z0-9-]` class fails the acceptance case, and a permissive `/^.+$/` fails the rejection case. Without the second half, a future failure could have been "fixed" by widening the guard to nothing while the test stayed green. ⏳ **Owner must re-test the shop page and item ordering on staging: this opens an area nobody has been able to reach since 2026-06-05.** |
| **T-03 + T-04** ✅ **fixed 2026-08-11 — ⏳ owner re-test owed** | **ROOT CAUSE: a pre-checked radio never fires `onChange`.** "Use my profile default" is checked on load (`useProfileWaveCapacity` initialises `true`), and the *only* line that set `maxOrdersPerWave` was that radio's handler — which therefore never ran. The render falls back to the profile value for DISPLAY, so the vendor saw "20 per wave / 8 waves × 20 = 160" while the form state held `''`. The client check refused with "please confirm your per-wave capacity", pointing at a field the screen already showed as filled in; it never reached the server. **T-04 was the same bug** — the accept never succeeded, so nothing persisted. The underlying enabler was DUPLICATION: the wave count was computed inline in the JSX where the submit handler could not see it. **Fix:** one `waveCountFor()` helper at module scope, both capacity fields seeded from the profile default where details load (an async callback, so no `react-hooks/set-state-in-effect` issue), and the JSX now calls the shared helper. **Guarded by** `flow-integrity` → "Vendor event capacity seeding": the loader must seed both fields, `Math.ceil(durationMin / 30)` must appear **exactly once**, and the payload must still carry a per-wave value for FT (the server hard-requires it). Original report follows. |
| ~~T-03 (original)~~ | **Vendor cannot accept an invitation using the default event capacity** | Leaving the prefilled default (8 waves × 20 = 160) and accepting errors "please confirm your per wave customer capacity" with nowhere to confirm it. Workaround is clicking *customize for this event* and retyping the same number. Most vendors will hit the error and stop. Possibly also the cause of T-04. |
...
| **T-05** ✅ **fixed 2026-08-11 — ⏳ owner re-test owed** | **Cause:** in `checkout/success` the four values `marketName` / `marketAddress` / `pickupDate` / `pickupTime` were each assigned under `if (!x)` — first-wins — so every pickup after the first was silently discarded. Vendors and item titles were already handled correctly (a `Set` collapsing to "2 vendors", "2 items"); only the pickup was singular. **Not a regression from the multi-market fix** — the template never handled more than one pickup; that fix simply made it reachable. **Fix:** a deduped `pickups` map keyed by market+date+time; `pickups` is passed ONLY when the order spans more than one, and `NOTIFICATION_REGISTRY.order_placed.message` branches on its presence to a new `notif.order_placed_multi_msg` listing every location. **Single-pickup orders render byte-identical to before.** Wording composed in the registry, not the route, so it stays translated (EN + ES added). **Guarded by** `lib/notifications/__tests__/order-placed-message.test.ts` — 5 behavioural tests incl. the single-element boundary ("more than one", not "present") and Spanish. Original report follows. |
| ~~T-05 (original)~~ | **Multi-market order confirmation email lists only ONE market and ONE item** | Order `FA-2026-69424470`, $17.17, one item from each of two vendors at two markets. Buyer has no record of half their order and no pickup location for it. Not caused by the multi-market fix — the template was never built for it; our fix is the first thing to route a real multi-market order through it. |

### 🔴 P1 continued — found 2026-08-10 after T-01 unblocked the shop page

| ID | Finding | Detail |
|---|---|---|
| **T-36** ✅ **fixed 2026-08-10 — mig 223, verified on staging; BROWSER-CONFIRMED 2026-08-11** (Prod pending) · ⚠ **leaves an FM hole open, see T-39** | **No attendee can order at an FT event — CONFIRMED on staging** | Adding any item errors `ERR_CART_003` "This pickup date is no longer accepting orders." **Root cause:** `validate_cart_item_schedule` (mig 002) delegates entirely to `get_available_pickup_dates`, which (mig 131, lines 116-125) keeps a listing's dates only if it is `private_pickup`, OR an `event` **in a vertical other than food_trucks**, OR the vendor has an active `vendor_market_schedules` row. FT events get no exemption, so they survive only via a vms row — and **`api/vendor/events/[marketId]/respond` never creates one.** It writes `market_vendors`, `event_vendor_listings` and upserts `listing_markets`; there is no vms write anywhere in it. **Confirmed by query on staging** (market `cc81fa25-997d-456b-9797-e50f8f4cdc30`): vendor accepted, `market_type=event`, `vertical_id=food_trucks`, `day_of_week=4` correctly matching the Thursday event date, `schedule_active=true`, **`vms_id=NULL`**. Not per-user — the owner hit the identical error as attendee AND as event manager. **FM events are exempt and should be unaffected** (untested). **Two fixes considered:** (A) create the vms row on acceptance — no migration, but makes "is this vendor attending" answerable from two places that can drift; (B) exempt FT events from the vms requirement so `market_vendors.response_status` stays the single source of truth, matching FM — a migration to a function that gates ordering everywhere. Claude recommends **B**; ⏳ awaiting owner's call. |
| **T-37** ⬜ | **Prices render as $0.00 to logged-out shoppers** | `shop/page.tsx` deliberately gates `price_cents` on auth, but rendering the gated value as **$0.00 reads as FREE**, not as "sign in to see prices". Wrong message on a money surface. |
| **T-39** ⬜ | **FM events sell without ANY acceptance check — found while fixing T-36** | `get_available_pickup_dates` exempts events via `(m.market_type = 'event' AND m.vertical_id != 'food_trucks')`. That branch checks **nothing** about whether the vendor accepted. So on farmers markets, a listing attached to an event market is orderable whether the vendor accepted, declined, or was never invited. Staging holds a live example of the shape: `f4000000-0202` is attached to an event whose vendor was never invited — on FT that now correctly yields no dates (mig 223's acceptance branch), but the equivalent on FM would sell. **Deliberately NOT fixed with mig 223:** tightening FM could switch off events that work today, and that needs its own before/after differential exactly like T-36 got. **The fix is already written** — mig 223's acceptance branch is deliberately scoped to `market_type='event'` in BOTH verticals, so closing this is just deleting the `AND m.vertical_id != 'food_trucks'` exemption above it and re-running the differential. Do not do it blind. |
| **T-38** ⬜ | **Add-to-cart fails silently when logged out** | The button gave no visible response; the "Error: Unauthorized" box rendered above the fold, so the owner only found it by scrolling up. Should prompt sign-in, and errors must surface where the action happened. |

### 💰 From the first complete FT event transaction — owner, 2026-08-11 (order `FO-2026-55913356`, $15.21)

**The lifecycle PASSED end to end** and should not be re-tested: buy → vendor confirm → mark ready → shopper acknowledge receipt → vendor fulfil → rating, with both notification chains, both emails, and consistent shopper/vendor views throughout. These are the defects found inside that otherwise-working run.

| ID | Finding | Detail |
|---|---|---|
| **T-40** ⬜ 💰 | **Success/receipt screen shows the right total but does not itemise everything** | **The CHECKOUT screen is correct and adds up** — sandwich $12.78 + service fee $0.15 + 10% tip $1.28 + Community Chip In $1.00 = **$15.21**. The success/receipt and order-detail screens show the same correct **$15.21 total** but omit lines and amounts: no Chip In line, so the visible itemisation reaches only $14.21. A buyer cannot account for $1.00 of their own money. Owner (2026-08-11): *"we can add the full itemization so it matches the checkout amount."* **The fix is display-only — do NOT touch any total or calculation.** The page predates Chip In (mig 213). |
| **T-41** ✅ **CLOSED 2026-08-11 — NOT A BUG. Do not "fix" this.** | **The vendor's order card correctly shows their own menu price.** Owner, 2026-08-11: *"it is the price the vendor entered when they were asked how much they charge for a sandwich. It's probably the same they charge walk-up customers. We have to ask them for a number/price they have context with. That is where it comes from and is how they think about their pricing."* So `$12.00` on a vendor order card is **their price, in their terms** — not a stale or wrong figure. It is also the same principle as T-18: the mental math holds (1 × $12.00 = $12.00), and adding a take-home figure would re-surface the platform fee we have agreed not to keep putting in front of vendors. ⚠ **Claude's original reading was wrong** and is recorded so nobody repeats it: the argument was that $12.00 is "neither what the buyer paid ($12.78) nor what the vendor receives ($11.07), so it appears nowhere in the transaction." That is arithmetically true and beside the point — it is the vendor's own list price, which is exactly the number they have context for. **The genuine defect in this area is T-06** (the ORGANIZER sees base prices while attendees see base + fee) — a different audience, deciding with a number their guests will never see. Original report follows. |
| ~~T-41 (original)~~ | **Vendor order card shows $12.00 where the buyer paid $12.78** | Vendor card reads `Qty: 1 x $12.00 = $12.00`. $12.78 ÷ 1.065 = $12.00 exactly, so $12.00 is the **base** and $12.78 is base + the 6.5% buyer fee. **OWNER'S RULE (2026-08-11), which governs this:** a vendor enters their **base price** when setting up a listing — we never make them add 6.5% themselves — but **everywhere else, all users see base + platform fee.** Owner: *"if you find a different situation please let me know, I may have forgotten about something."* ⚠ So the question is not "base vs take-home" (Claude's first reading, wrong) but whether the vendor order card is one of the "everywhere else" surfaces that should show $12.78, or a deliberate exception. **UNVERIFIED — read `lib/pricing.ts` and the vendor order card before proposing anything, and report any surface found doing something different.** Distinct from T-18, which is about never re-surfacing the fee as a *line item*. |
| **T-42** ✅ **fixed 2026-08-11** | **Unsubstituted `{vendorName}` placeholder in a live email** | Order-placed email ended *"confirmed by {vendorName}"* — the literal token, in a real inbox. **Cause:** `lib/locale/messages/index.ts:21` used `msg.replace('{k}', v)`, and `String.replace` with a **string** pattern substitutes only the **FIRST** occurrence. **Wider than one email — 5 messages repeat a token and rendered half-filled:** `notif.order_placed_msg` (`{vendorName}`), `about.mission_p2` (`{vendors}`), `sub_success.ft_desc` (`{tier}`), and in Spanish `markets.found` + `markets.found_within` (`{s}` — the **plural suffix**, so Spanish market counts have always been half-pluralised). **Fix:** `split(...).join(v)`. ⚠ **Deliberately NOT `replaceAll`** — with a string replacement it still interprets `$&`, `$'`, `` $` `` and `$1` in the REPLACEMENT, and these values are user data; demonstrated: a vendor named `Bob's $& Tacos` renders as `Bob's {v} Tacos` under `replaceAll` and correctly under `split/join`. **Guarded by** the new `lib/locale/__tests__/interpolation.test.ts` (6 behavioural tests against the real catalogue, incl. the repeated-token case and the `$` case). |
| **T-43** ✅ (organizer dashboard shows the pre-order count, `event-manager/[id]/dashboard/page.tsx:337`; admin side UNVERIFIED) | **Neither the organizer nor the admin sees that a pre-order happened** | After a completed purchase at their event, the event-manager dashboard showed **no change**, and the admin event view showed **no change**. The entire organizer pitch is that attendees pre-order and the organizer can see interest building — and the admin's viability numbers are supposed to be judged against reality. Right now a real order is invisible to both. ✅ **Owner agreed 2026-08-11:** there should be an indication that transactions have been completed, on BOTH the event-manager dashboard and the admin event view. |
| **T-44** ⬜ | **A vendor can mark an order "ready" a month before the event** | Owner marked ready on 2026-08-11 for a 2026-09-10 pickup and nothing objected; the buyer was notified their food was ready. Wants a warning when the pickup date is far out — the likely real cause is a vendor acting on the wrong order. |
| **T-45** ⬜ | **Social-share prompt fires during the handoff** | The "share your experience" popup appears as the shopper taps confirm at pickup. Owner: *"a good thing at the wrong time — we don't want any distractions during that 30-second handoff window."* Move it to after BOTH parties have confirmed fulfilment. |
| **T-47** ⬜ | **Pickup dates in notifications are formatted `en-US` regardless of the recipient's locale** | `checkout/success` formats the pickup date with `toLocaleDateString('en-US', …)` before handing it to the notification layer — which then carefully renders the surrounding sentence in the buyer's language. A Spanish-speaking buyer gets Spanish prose wrapped around "Thursday, September 10, 2026". Pre-existing; spotted while fixing T-05 and deliberately NOT bundled in, to keep that change to one thing. Likely wants the raw date passed through and formatted in the template layer where the locale is known. |
| **T-46** ⬜ | **New review appears on the vendor's Customer Reviews tile but not on their profile** | May be an aggregate that refreshes on a schedule rather than a bug. Verify which before touching it. |

### 🟠 P2 — wrong information shown to someone making a decision

| ID | Finding | Detail |
|---|---|---|
| **T-06 + T-07** ✅ **fixed 2026-08-11 — ⏳ owner re-test owed** | **T-06:** `events/[token]/select/page.tsx` hand-rolled `price_cents / 100` at two sites (per-item `:306`, average `:284`) and showed the vendor's **base** price. The organizer is choosing trucks for their guests, so every price there must be what those guests pay. Now uses `formatDisplayPrice` / `calculateItemDisplayPrice` from `pricing.ts` — the existing single source of base + 6.5%. **T-07:** `lib/events/viability.ts` computed `estimatedOrders = expectedMealCount \|\| headcount × rate` but ALWAYS printed a headcount-derived range beside it, so an organizer-supplied figure produced "~50 orders/vendor estimated (125 visitors × 10-30% = 13-38 orders)" — a sentence explaining a calculation that never ran. **No number changed**; the explanation now names the input actually used, on both scorers (`scoreCapacityAttendeePaid` and `scoreCapacityCrowd`), and shows the ÷ vendors step. **Guarded by** `lib/events/__tests__/viability-explanation.test.ts` (6 behavioural tests). ⚠ **Known gap deliberately left:** the yellow and red capacity branches carry no basis at all — an admin sees "~8 orders/vendor" with nothing explaining where 8 came from. Pre-existing on both scorers and a different problem from T-07 (which was a contradiction, not an absence); pinned by a test so it stays visible. Original report follows. |
| ~~T-06 (original)~~ | **Organizer's select-vendors page shows BASE prices, not fee-inclusive** | `/events/[token]/select` shows the vendor's base price; the public event page shows the correct attendee-facing price. The organizer chooses trucks against numbers no attendee will ever see. |
...
| **T-08** ✅ | **FIXED 2026-08-13 — cause found, and it was not a missing field.** The `catering_vendor_responded` template reads `d.vendorName` / `d.marketName` (`notifications/types.ts:1608`); **every caller passed `companyName` / `eventDate`** — respond route and cancel route both. Nothing was unpopulated; the KEYS did not match. Both call sites corrected, and the template now carries a comment naming the required keys plus fallbacks so a future mismatch degrades to "A vendor" instead of "undefined". **The second half of T-08 — "clicking it routed to the vertical admin panel" — is also explained:** that type is `audience: admin` with `actionUrl → /{vertical}/admin/events`. It was never an organizer notification. See T-59 for the organizer-facing type that replaces it on that path. ORIGINAL: **Notification text is broken** | Reads *"undefined accepted the event invitation for undefined"*. Template fields unpopulated. Also unclear whether it was delivered to the admin or the event manager — clicking it routed to the vertical admin panel events list. |
| **T-09 + T-10 + T-14** ✅ **fixed 2026-08-11 — ⏳ owner re-test owed** | **Both findings were the same investigation, and both premises were wrong in the same direction.** **T-10: there was never a second acceptance.** `handleRespond('accepted')` called a local toggle and returned — no server call, nothing persisted. The top "Accept" button only revealed the response form, which is why there was "no difference" between the two and why nothing was remembered on return. **T-09 is therefore NOT gated on that click at all — it was ungated entirely.** `api/vendor/market-stats` returned **every active event market in the vertical**, with `name` and `address`, to **any vendor with a profile** — invited or not. An event market's `name` is built from the organizer's company at approval, so the name IS the host's identity. Verified by unfiltered read of `:30-119`: the only filters are vertical, `status='active'`, and market_type; **no `market_vendors` join exists in that route.** **The rule already existed** — `api/vendor/events/[marketId]:156-175` implements and documents it ("Organizer identity protection… Full address is only revealed after the vendor has accepted"). market-stats predated events and never learned it. **Owner's rule, restated 2026-08-11:** only ACCEPTED vendors see org name + address; INVITED vendors see other event details so they can decide, but nothing they could use to approach the organizer directly. **Fixes:** market-stats now resolves the vendor's accepted `market_vendors` set and admits an event only if it is in it (traditional + private-pickup filtering untouched); and the reveal toggle is **removed entirely** — owner: *"why not show them all the info?"* — since nothing behind it was protected and the terms of the deal were appearing *after* the button that said you accepted them. That also dissolves **T-14** and most of **T-15**. **Guarded by** `flow-integrity` → "Organizer identity protection" (3 tests incl. an explicit check that the bare `\|\| m.market_type === 'event' \|\|` shape cannot return) and the toggle guard in "Vendor event capacity seeding". Original report follows. |
| ~~T-09 (original)~~ | **Address + organizer name visible on the vendor's listing page** | Owner saw the street address and the organizing company's name in the events section of a listing while that vendor had completed only the FIRST acceptance. Everything designed 2026-08-08 treats the address as earned by committing. If it is readable before a real commitment, the staged flow protects nothing. **Investigate together with T-10.** |
...
## ✅ RATE LIMITER DOES NOT FAIL OPEN — closed 2026-08-13

**The suspicion was wrong.** On a Redis error `checkRateLimit` catches and falls through to the in-memory limiter (`lib/rate-limit.ts:181-204`), which **still enforces the same limit**. A Redis outage degrades the limiter from *shared across instances* to *per-instance* — each warm Vercel lambda keeps its own Map, so the effective ceiling becomes up to (instances × limit) instead of `limit` overall. **Weakened, not disabled**, and Vercel routes arbitrarily so an attacker cannot deliberately spread across instances.

**What WAS wrong: it was silent.** The catch block was empty, so Redis could be down for days and the only symptom would be a rate limit that quietly stopped being global. Now logs `[rate-limit] Redis unavailable — degraded to per-instance limiting`.

⚠ **Still worth the owner checking:** whether `UPSTASH_REDIS_REST_URL` is actually set on Staging and Prod. If it is not, the in-memory path is all there has ever been — the same per-instance weakening, permanently, while the file header describes that path as local-dev-only.

**This removes one of the three candidate causes of the flaky rate-limit test below.**
...
- [x] **H2: Duplicate organizer confirmation emails** (FIXED — `selection_email_sent_at` gate `admin/events/[id]/route.ts:540` + `select/route.ts:813`, verified 2026-09-20) — `admin/events/[id]/route.ts:204` (status→ready) and `events/[token]/select/route.ts:329` (vendor selection) both send nearly identical "X vendors ready" emails. Fix: add `selection_email_sent_at` column to gate duplicates.
...
- [x] **M2: CLOSED 2026-08-13 — original claim was STALE, the real remainder fixed.** The organizer-cancel route (`events/[token]/cancel`) already used `event_cancelled_vendor` with correct keys (reworked under EVT-10) — the `:104` claim no longer matched the code. What remained was the mirror: the VENDOR-cancel route (`vendor/events/[marketId]/cancel`) telling the ORGANIZER "X cancelled their commitment to *the event invitation for* Y" through the admin accept/decline template. Fixed by adding a `cancelled` branch to `event_vendor_responded_organizer` (a cancellation is a response-lifecycle event — no new type, no tripwire bump) — organizer-appropriate copy, the vendor's reason attributed, linking to the organizer's own dashboard instead of the admin panel. |
...
## 💰 ORGANIZER-FUNDED RETAINED STANDBY SPOTS — follow-on design session (owner + Claude, 2026-08-16)

The zero-obligation bench (shipped: mig 232 + Phase 3) is a call list, not insurance — standby vendors give up nothing and may all be booked at activation time. Owner: "it doesn't add much assurance to the event going smoothly." The insurance lever, already contemplated in decisions.md "Backup vendors — model decided" #3 ("organizer-funded EXTRA bench spots... priced through the fee machinery"): an organizer pays $X per standby vendor to HOLD the date — real commitment (don't book elsewhere, must accept if activated), paid whether or not activation happens (the payment buys the held calendar day); if activated they also get the covered spot. New money direction (organizer → vendors), needs its own careful design: pricing, refund-on-no-activation question (none — the hold was delivered), vendor-side accept/decline of retainer offers, Connect payout to vendors. Do NOT bolt onto Phase 3 machinery without design.

## 🎪 EVENTS — owner testing 2026-08-26 (round 2), items NOT built yet
- [ ] **Vendor events index page** (`/vendor/events`): invitations · today · upcoming · on standby · past (completed drop to history) + make the dashboard "My Vendor Events" card title the link. Owner: HOLD — the organizer's multi-event page (`/event-manager`) is the pattern to mirror; "build bigger later". (Card was built as a card because no index page exists — vendor/dashboard/page.tsx:666-671.)
- [ ] **Un-decline** for vendors (low priority, owner: "doesn't need to be high priority"): a declined vendor can't re-respond; scenario = organizer short on backups asks a declined truck to stand by. UNVERIFIED whether respond/route.ts blocks declined→accepted today.
- [ ] Organizer-assigned backups stay OFF — bench is volunteer-only by design; the paid **retained standby spot** (already backlogged) is the lever for small towns with 3–4 trucks.

## Event ↔ location conflicts — follow-ons (2026-08-27, from the R3-4 build; rule in decisions.md)
- [ ] **Reverse direction (owner: "backlog it").** Booking a park spot / booth week or activating a weekly schedule on a date where the vendor has an ACCEPTED event runs no availability check. Apply the same rule (`lib/events/availability.ts`) from the booking + schedules routes.
- [ ] **Release a skipped paid spot without credit.** `park_spot_skipped_for_event` only NOTIFIES the operator; the booking stays paid and the slot stays held (uniqueness index). Today the only release is the operator's cancel-date cascade, which would CREDIT the truck who chose to leave. Design a "release, no credit" path so the operator can re-let the day.
- [ ] **Manual "skip this day" UI** on `vendor_date_blackouts` (source_event_market_id NULL). The table + the pickup-date predicate already support it; needs a vendor surface and a lift control.
- [ ] Check-in prompt still fires for a truck on a skipped paid park day (owner: no bail-out on the no-show record — the vendor chose). Revisit only if the prompt itself becomes a nuisance.

## 🔜 DO SOON — owner findings 2026-08-28 (W1 pass + misc; owner: "organize these thoughts & changes for things to do soon")
1. ✅ BUILT 2026-08-29 — **Notification routing — vendor-responded goes to the admin panel.** A user holding BOTH admin and event-manager roles clicked "taco truck accepted the event invite for xyz event" and landed on `/admin/events` (`catering_vendor_responded`, audience admin, actionUrl `/{vertical}/admin/events` — `notifications/types.ts:~1695-1703`; sent to every admin/platform_admin by `vendor/events/[marketId]/respond/route.ts:471-479`). A non-admin organizer never receives that type — they get `event_vendor_responded_organizer` (`:515`) whose link is the Organizer Event Dashboard, but ONLY once `catering_requests.organizer_user_id` is set (null until they load /event-manager logged in). Decide: (a) when the admin recipient is ALSO the organizer of that event, send the organizer version instead (or link the admin one to the organizer dashboard for that event); (b) confirm organizer_user_id backfill so external organizers reliably get theirs.
2. ✅ BUILT 2026-08-29 (ack step + printable signs; copy reinforcement done in items 4–5) — **Onboarding: drive home the separate in-app pickup line.** Nowhere in vendor onboarding do we make the vendor confirm the model: in-app buyers order ahead, pick a pickup time, and DO NOT stand in the walk-up line; the vendor must run a SEPARATE pickup line for app orders, marked with a branded sign. If vendors make app customers queue anyway, buyers order less. Build: (a) an explicit acknowledgment step in onboarding (checkbox + short explainer, both verticals); (b) **standardized branded pickup signs** vendors download and print at 8.5×11 and 11×17 (FM + FT brands) — a "Pickup signs" download on the dashboard/edit page; (c) reinforce the model in the Pickup Capacity and prep-time copy (items 4–5).
3. ✅ BUILT 2026-08-28 (uncommitted) — **Documents blue box is outside the section it describes.** `vendor/edit/page.tsx:~194` renders "These documents are shared with the markets and parks you book with…" ABOVE the `Documents & Certifications` section; move it INSIDE, at the top of the section that holds Documents & Certifications / Required Documents / Optional Certifications (`components/vendor/DocumentsCertificationsSection.tsx`).
4. ✅ BUILT 2026-08-28 — **Pickup prep time copy** (`components/vendor/PickupLeadTimeForm.tsx:~73`): replace the last sentence "The actual window may be longer — this is just the minimum." with "Based on the pick-up time the customer selects when they place their order, you may have much more prep time, 15 or 30 minutes is just the minimum."
5. ✅ BUILT 2026-08-28 — **Pickup Capacity copy** (`components/vendor/PickupCapacityForm.tsx:~113-114` + the "see Full" sentence nearby): last sentence of the walk-up paragraph → "Most times this isn't an issue, but around busy times, like a lunch or dinner rush, this limit will help smooth out your orders so you can manage your walk-up customers, who wait in line, alongside your in-app customers who skip the line when they arrive at their pre-selected pickup time." And "Set it too low and you turn away the app customers you do have — they'll see Full and order somewhere else." → "Set it too low and you turn away the app customers who want to buy from you — they'll see that you are Full and may order somewhere else."
6. ✅ BUILT 2026-08-28 — **Private Events Readiness — Capacity & Experience copy** (`vendor/edit/EventReadinessForm.tsx:~596`): replace with the owner's text verbatim (three paragraphs: "How our events work: Attendees pre-order through the app before the event begins, and every order is assigned to a 30-minute pickup wave. Customers know when their food will be ready, so they arrive at your truck in evenly spaced batches instead of one long line. Your per-wave capacity is how many customers you can realistically serve in one of those 30-minute windows (waves)." / "The system uses algorithms that factor in your capacity as part of the process of matching you with events that are a good fit for you. It also caps the wave slots that are available for event attendees to choose from so you're less likely to be overwhelmed on event day with unexpected rush orders that you may not be ready for." / "Your event capacity is separate from your Pickup Capacity setting above (which paces your everyday, non-event, orders). We know that your capacity may change with your menu and the amount of help you have, so when you accept a specific event invitation, you'll be able to change and confirm your capacity for that event. Your event capacity is important for keeping events running smoothly, be as accurate as you can be."). FM wording: swap "truck" for the FM noun.
7. ✅ BUILT 2026-08-28 — **"Max Headcount Per 30-Minute Wave *" helper** (`EventReadinessForm.tsx:~601`): → "How many people can you serve in a 30-minute window with most orders coming from the app".
