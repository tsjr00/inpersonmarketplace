...
- [ ] **Schema snapshot is wrong about 4 columns on `orders` table — phantom columns may be referenced in code** — Live staging `orders` table is MISSING these columns that `supabase/SCHEMA_SNAPSHOT.md` lines 740-743 claim exist:
  - `vendor_payout_cents`
  - `buyer_fee_cents`
  - `service_fee_cents`
  - `market_id`

  These columns DO exist on `order_items` but NOT on `orders`. The 2026-04-05 snapshot rebuild added them to the `orders` section incorrectly (per the changelog entry, REFRESH_SCHEMA.sql output was used — possibly a parsing/copy error or those columns were dropped post-rebuild without snapshot update). **Risk:** any app code that references `orders.vendor_payout_cents`, `orders.buyer_fee_cents`, `orders.service_fee_cents`, or `orders.market_id` will throw at runtime when that path executes. **Action items:** (1) Grep the codebase for these phantom column references — `grep -r "orders.*vendor_payout_cents\|\.vendor_payout_cents.*orders" apps/web/src` and similar for the other 3 columns. (2) If found, either remove the references OR add the columns via a new migration. (3) Regardless, regenerate `SCHEMA_SNAPSHOT.md` from REFRESH_SCHEMA.sql to clear the lie. Discovered Session 74 when SQL `SELECT o.vendor_payout_cents FROM orders o` errored with "column does not exist" on staging.

- [ ] **Audit other webhook handlers for the `if (!existingPayment)` anti-pattern** — The bug we fixed in `webhooks.ts handleCheckoutComplete` was: success path handles A+B, webhook handles A only, AND the webhook's processing for B is gated behind `if (!existingPayment)` so resends/retries don't backfill. The same shape may exist in other webhook handlers — `handleSubscriptionCheckoutComplete`, `handleMarketBoxCheckoutComplete`, refund webhooks, payment-failed webhooks, anywhere with "concurrent success route may have inserted first" logic. **Action:** grep `webhooks.ts` for `existingPayment`, `existingPayout`, `existingRecord` patterns; for each, trace what gets gated behind it; verify that if the webhook IS the only successful path, all downstream side-effects still happen. Estimated 30 min audit. Found Session 74.
