...
> - **🟣V = Vitest covered.** An active, passing vitest test validates this rule. Test runs automatically on every commit via pre-commit hook.
> - **📋T = Vitest placeholder.** A `.todo()` test exists (structure defined, needs implementation — typically requires DB, API, or cron environment).
>
...
| ID | Rule | Workflow | What to Test |
|----|------|----------|-------------|
| ✅ 🟣V MP-R9 | Per-item rounding: `calculateItemDisplayPrice(900)` = round(900×1.065) = 959 per item. Display subtotal = sum of per-item prices (not bulk calculation) | MP-W1 | 2 items at $9.00 each: display subtotal = $19.18 (not $19.17) |
| ✅ 🟣V MP-R10 | External payment orders charge 3.5% vendor fee (not 6.5%) because no Stripe processing cost | MP-W2, MP-W6 | Ledger entry = subtotal × 0.035 |
...
| ✅ 🟣V OL-R20 | **Cron Phase 4.5: vendor stale reminder, no status change**. Matches: `order_items.status = 'confirmed'` AND `order_items.pickup_date < TODAY` with 3-day lookback. Action: escalating vendor notifications. `order_items.status` stays `'confirmed'` — does NOT change. **USER CONFIRMED Session 52**: Yes, this is correct behavior. | Item `order_items.status = 'confirmed'`, `pickup_date = yesterday` → after cron: `order_items.status` still `'confirmed'`, vendor notification sent. |
...
| ID | Question | Status |
|----|----------|--------|
| ✅ OL-Q1 | Is there a check preventing vendor from confirming AFTER the cutoff time has passed? | CODE VERIFIED — **NO**, but **USER SAYS IRRELEVANT (Session 52)**: Expired orders get moved to fulfilled status by cron, so confirming may not even be possible because the status is no longer 'pending'. The cron handles this — no additional route-level check needed. |
...
| ✅ 🟣V OL-Q5 | Chef Box & Market Box corollary for OL-R16: Do subscription-based box orders follow the same external payment cancellation rules as regular listings? Or do they have separate timing? | **USER DECIDED Session 52**: Yes, boxes follow the same process as regular listings, with caveats: (1) **FT inventory is NOT restored on cancellation** — once a meal is cooked it can't be resold, unlike FM items. (2) **External payment cancellation** — there is no payment cancellation mechanism; buyer and seller have to work it out directly. |
| ✅ 🟣V OL-Q6 | Chef Box & Market Box corollary for OL-R17: Do subscription-based box orders get the same vendor reminder timing (FT=15min, FM=12hr)? Or different? | **USER DECIDED Session 52**: FM boxes = same timing as regular FM. **FT boxes = 3 hours after order** — gives trucks enough time to finish a busy rush and still confirm a box item for the next day or later that week without making the buyer wait too long. |
...
| ID | Rule | Workflow | What to Test |
|----|------|----------|-------------|
| ✅ 🟣V VI-R6 | Email FROM address for notifications: FM=`updates@mail.farmersmarketing.app`, FT=`updates@mail.foodtruckn.app`. FW falls back to FM address. These are notification-only addresses. Each vertical will also have separate email addresses for other purposes (support, marketing, etc.) | VI-W4 | FT order notification sent from foodtruckn.app domain |
| ✅ 🟣V VI-R7 | CSS primary color: FM=`#8BC34A` (green), FT=`#ff5757` (red) | VI-W4 | No green on FT pages; no red on FM pages (except semantic danger color) |
| ✅ 🟣V VI-R8 | `term()` returns vertical-specific strings: FM "Market" vs FT "Location", FM "Market Box" vs FT "Chef Box" | VI-W3 | `term('food_trucks', 'market')` returns "Location" not "Market" |
| ✅ 🟣V VI-R9 | Buyer premium is FM-only. `isBuyerPremiumEnabled('food_trucks')` returns false. FT never shows premium upgrade UI | VI-W5 | FT settings page has no tier upgrade section |
...
| 🟣V NI-R19 | `order_ready` urgency: FT=immediate (food is hot, buyer waiting NOW), FM=standard (pickup could be days away) | NI-W8 | FT order_ready → push+in_app. FM order_ready → email+in_app |
| 🟣V NI-R20 | `order_cancelled_by_vendor` urgency: FT=immediate (push), FM=urgent (SMS). FM buyers en route get SMS fallback for vendor cancellations | NI-W8 | FT cancellation → push+in_app. FM cancellation → sms+in_app |
| 🟣V NI-R21 | `order_cancelled_by_buyer` urgency: FT=immediate (push), FM=urgent (SMS). FM vendors get SMS for buyer cancellations | NI-W8 | FT buyer cancel → push+in_app. FM buyer cancel → sms+in_app |
| 🟣V NI-R22 | `new_paid_order` urgency: FT=immediate (food truck needs to start cooking NOW), FM=standard (order placed days in advance) | NI-W8 | FT new order → push+in_app. FM new order → email+in_app |
| 🟣V NI-R23 | `new_external_order` urgency: FT=immediate (food truck needs to prep NOW), FM=standard (advance order) | NI-W8 | FT external order → push+in_app. FM external order → email+in_app |
| 🟣V NI-R24 | `stale_confirmed_vendor` urgency: FT=immediate (time-critical, food perishable), FM=standard (more lead time to resolve) | NI-W8 | FT stale confirmed → push+in_app. FM stale confirmed → email+in_app |
| 🟣V NI-R25 | `external_payment_reminder` urgency: FT=immediate (15-min reminder window), FM=standard (12-hr reminder window) | NI-W8 | FT ext payment reminder → push+in_app. FM ext payment reminder → email+in_app |
| 🟣V NI-R26 | `pickup_missed` urgency: FT=immediate (food truck may leave, perishable), FM=urgent (still important, SMS fallback) | NI-W8 | FT pickup missed → push+in_app. FM pickup missed → sms+in_app |
| 🟣V NI-R27 | `market_box_pickup_missed` urgency: FT=immediate (perishable), FM=standard (less time-critical) | NI-W8 | FT market box missed → push+in_app. FM market box missed → email+in_app |
| 🟣V NI-R28 | `stale_confirmed_vendor_final` urgency: standard for BOTH verticals (changed from immediate). Second notice, vendor already got first alert | NI-W8 | FT+FM stale final → email+in_app |
| 🟣V NI-R29 | `trial_reminder_3d` urgency: standard for BOTH verticals (changed from immediate). 3-day advance notice, not time-critical | NI-W8 | FT+FM trial 3d reminder → email+in_app |
| 🟣V NI-R30 | `trial_expired` urgency: standard for BOTH verticals (changed from immediate). Account already downgraded, informational | NI-W8 | FT+FM trial expired → email+in_app |
| 🟣V NI-R31 | `order_refunded` urgency: urgent for BOTH verticals (changed from standard). Buyer needs to know money is coming back, vendor needs SMS record | NI-W8 | FT+FM refund → sms+in_app |
| 🟣V NI-R32 | `trial_grace_expired` urgency: immediate for BOTH verticals (confirmed correct, no change). Listings going to draft, service disruption | NI-W8 | FT+FM grace expired → push+in_app |
| 🟣V NI-R33 | `pickup_issue_reported` urgency: urgent for BOTH verticals (confirmed correct, no change). Vendor needs SMS alert for buyer-reported problems | NI-W8 | FT+FM issue reported → sms+in_app |
...
| ✅ 🟣V NI-Q5 | **FM channel gating**: FM vendors now have the same tier-based channel ladder as FT (free=in_app, standard=+email, premium=+push, featured=+sms). This is new — FM vendors previously had no channel restrictions. Confirm this is desired? | **CONFIRMED** — FM vendors have same channel ladder as FT. |
...
|-----------|-------|-----------------|
| `pricing.test.ts` | ~20 active | MP-R1, R2, R5, R9, R13 |
| `order-pricing-e2e.test.ts` | ~18 active | MP-R1, R2, R5, R9 + money conservation |
...
- **DB tests**: Require live Supabase connection (triggers, constraints, RLS policies)
- **API tests**: Require running Next.js server (route handlers, middleware)
- **Cron tests**: Require cron execution context (Phase 1-9 logic)
- **Component tests**: Require React testing library (UI-specific features)

Converting these requires either integration test infrastructure (e.g., test database) or mocking layers. They're ready to implement once the user decides on testing strategy.
