...
/**
 * Events pipeline board (admin phase 6, design locked 2026-08-30 in
 * admin_ui_redesign_research.md:95-99; built 2026-09-04).
 *
 * The former /[vertical]/admin/events flat list + status-filter page,
 * REORGANIZED into stage groups with jump-nav: New requests → Inviting →
 * Selecting → Upcoming → Wrap-up (+ collapsed Closed for declined/cancelled —
 * Restore lives there). One card per event with its stage's summary; clicking
 * opens the SAME detail panel as before (moved verbatim — the panel was
 * already stage-conditional, so every action/section of the old page
 * survives: full inventory in .claude/admin_phase6_events_board.md).
 * Settlement stays its own drill-in page. Events admin is vertical-only —
 * no platform pair exists.
 */
...
interface CateringRequest {
  ...
  /** Invitation gate (mig 239): with service_level this feeds the ONE
   *  invitationsHeld() definition — rides the list GET's select('*'). */
...
// ── Pipeline stages (the board's spine) ────────────────────────────────────
// Derived from status + invitation data — the same fields the old page used.
// 'approved' splits on whether any invitations exist yet: none = the admin's
// next move is inviting; some = selection is in motion.
type PipelineStage = 'new_requests' | 'inviting' | 'selecting' | 'upcoming' | 'wrapup' | 'closed'

const STAGE_ORDER: PipelineStage[] = ['new_requests', 'inviting', 'selecting', 'upcoming', 'wrapup', 'closed']

const STAGE_META: Record<PipelineStage, { label: string; hint: string }> = {
  new_requests: { label: 'New requests', hint: 'Review & approve' },
  inviting: { label: 'Inviting', hint: 'No invitations out yet' },
  selecting: { label: 'Selecting', hint: 'Invites out — organizer picking' },
  upcoming: { label: 'Upcoming / Event day', hint: 'Pre-orders open or running' },
  wrapup: { label: 'Wrap-up', hint: 'Feedback & settlement' },
  closed: { label: 'Closed', hint: 'Declined & cancelled' },
}

function stageOf(req: CateringRequest, invitedCount: number): PipelineStage {
  if (req.status === 'new' || req.status === 'reviewing') return 'new_requests'
  if (req.status === 'approved') return invitedCount === 0 ? 'inviting' : 'selecting'
  if (req.status === 'ready' || req.status === 'active') return 'upcoming'
  if (req.status === 'review' || req.status === 'completed') return 'wrapup'
  return 'closed' // declined | cancelled
}

export default function EventsAdminPage({ vertical }: { vertical: string }) {
  ...
  // Board grouping — every request lands in exactly one stage.
  const grouped = new Map<PipelineStage, CateringRequest[]>(STAGE_ORDER.map(st => [st, []]))
  for (const r of requests) {
    const invitedCount = r.market_id ? (mvMap[r.market_id] || []).length : 0
    grouped.get(stageOf(r, invitedCount))!.push(r)
  }
  for (const [st, list] of grouped) {
    // Soonest event first where there's work to do; newest first in Closed.
    list.sort((a, b) => st === 'closed'
      ? (a.created_at > b.created_at ? -1 : 1)
      : (a.event_date < b.event_date ? -1 : 1))
  }

  // The card's stage-specific second line — the "what's its state" summary
  // the old flat list couldn't show.
  const stageSummary = (req: CateringRequest, st: PipelineStage): string => {
    const mvs = req.market_id ? (mvMap[req.market_id] || []) : []
    if (st === 'new_requests') {
      const days = Math.floor((Date.now() - new Date(req.created_at).getTime()) / 86400000)
      return req.status === 'reviewing'
        ? `in review · ${days === 0 ? 'submitted today' : `waiting ${days}d`}`
        : days === 0 ? 'submitted today' : `waiting ${days}d`
    }
    if (st === 'inviting') {
      // Gate fix A companion: say WHY nothing is out — a held self-serve event
      // is the organizer's move, not the admin's.
      return invitationsHeld({ service_level: req.service_level, invitations_released_at: req.invitations_released_at ?? null })
        ? 'invitations HELD — waiting on the organizer to send'
        : 'approved — no invitations out yet'
    }
    if (st === 'selecting') {
      // Same classifier as every other stage surface (owner 2026-09-03).
      let sel = 0, bench = 0, awaiting = 0
      for (const mv of mvs) {
        const s = classifyVendorEventStage({
        ...
        else if (s === 'accepted_awaiting') awaiting++
      }
      return `${sel} selected of ${req.vendor_count} wanted · ${bench} bench · ${awaiting} awaiting selection · ${mvs.length} invited`
    }
    if (st === 'upcoming') return req.status === 'active' ? '🔴 event day' : 'pre-orders open'
    if (st === 'wrapup') return req.status === 'review' ? 'collecting feedback' : 'settled'
    return req.status === 'declined' ? 'declined' : 'cancelled'
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px' }}>
      ...
      {/* Stage jump-nav — replaces the status-filter chips (S-1 carried:
          stages derive from status + data in ONE place, stageOf, so a
          hand-typed list can't drift and no lifecycle state is unreachable). */}
      <div
        style={{
          display: 'flex',
          gap: spacing['2xs'],
      ...
      >
        {STAGE_ORDER.map((st) => {
          const count = grouped.get(st)!.length
          return (
            <button
              key={st}
              onClick={() => {
                if (st === 'closed' && !showClosed) setShowClosed(true)
                // queueMicrotask: the Closed section may need to render first.
                queueMicrotask(() => document.getElementById(`stage-${st}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }))
              }}
              disabled={count === 0}
              style={{
                ...sizing.control,
                border: `1px solid ${count > 0 ? statusColors.neutral300 : statusColors.neutral200}`,
                backgroundColor: 'white',
                color: count > 0 ? statusColors.neutral700 : statusColors.neutral400,
                cursor: count > 0 ? 'pointer' : 'default',
                fontWeight: count > 0 ? typography.weights.semibold : typography.weights.normal,
              }}
            >
              {STAGE_META[st].label} ({count})
            </button>
          )
        })}
      </div>
      ...
      {loading ? (
        ...
        <div className={`admin-detail-split${selectedId ? ' has-detail' : ''}`} style={{ gap: spacing.md }}>
          {/* The board: one section per stage, one card per event. */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
            {STAGE_ORDER.map((st) => {
              ...
              return (
                <div key={st} id={`stage-${st}`} style={{ scrollMarginTop: 12 }}>
                  <div
                    style={{ display: 'flex', alignItems: 'baseline', gap: spacing.xs, marginBottom: spacing['2xs'], cursor: isClosed ? 'pointer' : 'default' }}
                    onClick={isClosed ? () => setShowClosed(v => !v) : undefined}
                  >
                    <h2 style={{ margin: 0, fontSize: typography.sizes.base, fontWeight: typography.weights.bold, color: statusColors.neutral800 }}>
                      {STAGE_META[st].label} ({list.length}){isClosed ? (showClosed ? ' ▾' : ' ▸') : ''}
                    </h2>
                    <span style={{ fontSize: typography.sizes.xs, color: statusColors.neutral400 }}>
                      {STAGE_META[st].hint}
                    </span>
                  </div>
                  {(!isClosed || showClosed) && (
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: radius.md,
                              backgroundColor: req.id === selectedId ? statusColors.selectionBg : 'white',
                              cursor: 'pointer',
                              textAlign: 'left',
                              width: '100%',
                            }}
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: spacing['3xs'] }}>
                              <strong style={{ fontSize: typography.sizes.sm, color: statusColors.neutral800 }}>
                                {req.company_name}
                              </strong>
                              <span
                                style={{
                            ...
                            {/* Stage summary — the board's reason to exist */}
                            <div style={{ fontSize: typography.sizes.xs, color: st === 'upcoming' && req.status === 'active' ? statusColors.dangerDark : statusColors.neutral600, fontWeight: typography.weights.semibold, marginTop: 2 }}>
                              {stageSummary(req, st)}
                            </div>
                          </button>
                        )
            ...
            >
              ...
              >
                ...
                <div style={{ display: 'flex', gap: spacing['2xs'] }}>
                  ...
                  {/* Gate fix A (owner go 2026-09-04, found 2026-08-31): while
                      the organizer's invitation hold is on, NO vendor has been
                      invited — opening pre-orders would sell an event with
                      nobody to cook. Same invitationsHeld() every invite path
                      uses (flow-integrity-pinned). Disabled + explained, not
                      hidden — the admin should see WHY the step is blocked. */}
                  {selected.status === 'approved' && (
                    invitationsHeld({ service_level: selected.service_level, invitations_released_at: selected.invitations_released_at ?? null }) ? (
                      <button
                        disabled
                        title="The organizer hasn't sent invitations yet — no vendors are invited, so there'd be nothing to pre-order."
                        style={{ ...sizing.control, backgroundColor: statusColors.neutral100, color: statusColors.neutral500, border: `1px solid ${statusColors.neutral300}`, cursor: 'not-allowed' }}
                      >
                        Open Pre-Orders — invitations held
                      </button>
                    ) : (
                      <button
