...
/**
 * OrderIssuesAdminPage — the ONE implementation behind /admin/order-issues
 * and /[vertical]/admin/order-issues (admin UI rebuild phase 3, merge 7/11,
 * owner 2026-08-31). Both copies already shared GET/PATCH
 * /api/admin/order-issues (S4-2: verifyAdminScope forces vertical admins to
 * their own vertical; both tiers may update — that was true before and is
 * unchanged). Client-only merge; superset kept from each:
 *   from the vertical copy — the Closed tab and the issue-age chips
 *   (Nd-ago, red at 3+ days);
 *   from the platform copy — the FT/FM vertical chip on rows (all-scope
 *   view only).
 * Cards were already single-column at every width. Vendor drill-in links go
 * to the issue's own vertical.
 *
 * `vertical === undefined` = platform view (all verticals).
 */
...
export default function OrderIssuesAdminPage({ vertical }: { vertical?: string }) {
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: spacing.sm }}>
          {issues.map(issue => {
            ...
            const issueVertical = issue.vertical_id || vertical || 'farmers_market'

            // Issue age (from the vertical copy — useful on both tiers).
              ...
              <div key={issue.id} style={{
                ...
                <div style={{ fontSize: typography.sizes.xs, color: colors.textMuted, marginBottom: spacing['2xs'], display: 'flex', flexWrap: 'wrap', gap: spacing['2xs'], alignItems: 'center' }}>
                  ...
                  {issue.vendor_profile_id ? (
                    <Link
                      href={`/${issueVertical}/admin/vendors/${issue.vendor_profile_id}`}
                ...
                {isEditing ? (
                  <div style={{ marginTop: spacing['2xs'], display: 'flex', flexDirection: 'column', gap: spacing['2xs'] }}>
                    <select
                      ...
                      style={{ padding: spacing['2xs'], border: `1px solid ${colors.border}`, borderRadius: radius.sm, fontSize: typography.sizes.sm }}
                    >
                    ...
                    <textarea
                      ...
                      rows={2}
                      style={{ padding: spacing['2xs'], border: `1px solid ${colors.border}`, borderRadius: radius.sm, fontSize: typography.sizes.sm, resize: 'vertical' }}
                    />
                    <div style={{ display: 'flex', gap: spacing['2xs'] }}>
                      <button
                        onClick={() => handleSave(issue.id)}
                        disabled={saving}
                        style={{ padding: `${spacing['2xs']} ${spacing.sm}`, backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: radius.sm, fontSize: typography.sizes.sm, fontWeight: typography.weights.semibold, cursor: saving ? 'not-allowed' : 'pointer' }}
                      >
                        {saving ? 'Saving...' : 'Save'}
                      </button>
                      <button
                        onClick={() => setEditingId(null)}
                        style={{ padding: `${spacing['2xs']} ${spacing.sm}`, backgroundColor: 'white', color: colors.textMuted, border: `1px solid ${colors.border}`, borderRadius: radius.sm, fontSize: typography.sizes.sm, cursor: 'pointer' }}
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    style={{ marginTop: spacing['3xs'], padding: `${spacing['3xs']} ${spacing.xs}`, backgroundColor: 'white', color: '#3b82f6', border: '1px solid #3b82f6', borderRadius: radius.sm, fontSize: typography.sizes.xs, fontWeight: typography.weights.semibold, cursor: 'pointer' }}
                  >
                    Review / Update
                  </button>
                )}
