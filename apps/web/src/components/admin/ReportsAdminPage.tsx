...
const AVAILABLE_REPORTS: ReportOption[] = [
  ...
  { id: 'tax_list_supplement', name: 'Texas List Supplement (Form 01-116)', description: 'Sales tax per Texas jurisdiction code for the period, NET of refunds: sales from the per-item snapshots frozen at checkout, minus the reversal ledger rows recorded in the period (at the original rate) — the table the monthly return asks for, with the gross sides shown for audit. A refund in a later month than its sale shows as a negative line. Empty until tax goes live.', category: 'accounting' },
