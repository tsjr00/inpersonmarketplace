...
export default function MarketsAdminPage({ vertical }: { vertical: string }) {
  ...
  // Deep-link: ?edit=<market-id> opens the edit form once the list loads.
  // Used by the market detail page's Edit button and the platform list's
  // Edit links (admin UI rebuild phase 5) so field editing has exactly one
  // home — this form. Consumed once so Cancel doesn't re-open it.
  const editParam = searchParams.get('edit')
  const [editConsumed, setEditConsumed] = useState(false)
  useEffect(() => {
    if (!editParam || editConsumed || loading) return
    const target = markets.find(m => m.id === editParam)
    if (target) {
      // queueMicrotask: avoids the set-state-in-effect lint rule (async init)
      queueMicrotask(() => {
        handleEdit(target)
  ...
  }, [editParam, editConsumed, loading, markets])
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: spacing.md }}>
              ...
              <div>
                ...
                >
                  ...
                  {editingMarket?.market_type === 'private_pickup' && (
                    <option value="private_pickup">🏠 Private Pickup</option>
                  )}
                  <option value="event">🎪 Event (Festival, Fair, Concert)</option>
                </select>
                {editingMarket && (
                  <p style={{ fontSize: typography.sizes.xs, color: colors.textSecondary, margin: `${spacing['3xs']} 0 0 0` }}>
                    Type is set when the market is created and cannot be changed here.
                  </p>
                )}
              </div>
              ...
              {loading ? (
                    ...
                    {filteredMarkets.map((market) => (
                      <tr key={market.id} style={{ borderBottom: `1px solid ${colors.border}` }}>
                        <td style={{ padding: spacing.sm }}>
                          <Link href={`/${vertical}/admin/markets/${market.id}`} style={{ textDecoration: 'none' }}>
                            <div style={{ fontWeight: typography.weights.semibold, color: colors.primary }}>
                              {market.name}
                            </div>
                          </Link>
                        </td>
                ...
                <div className="admin-list-mobile">
                  {filteredMarkets.map((market) => {
                    ...
                    // Mobile rows drill into the detail page. AdminMobileRow's
                    // href and rightAction are mutually exclusive — drill-in
                    // wins; Edit lives on the detail page + desktop rows.
                    return (
                      <AdminMobileRow
                        key={market.id}
                        href={`/${vertical}/admin/markets/${market.id}`}
                        title={market.name}
                  ...
                  <div style={{ padding: '12px 14px', fontSize: typography.sizes.xs, color: colors.textSecondary, fontStyle: 'italic', borderTop: '1px solid #e5e7eb' }}>
                    Tap a market to open its detail page (schedules, vendors, documents, approve / suspend / delete). Editing opens from the detail page.
                  </div>
                </div>
                </>
              )}
