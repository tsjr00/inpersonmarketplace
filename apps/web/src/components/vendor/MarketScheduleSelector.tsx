...
export default function MarketScheduleSelector({
  ...
  // terms any more; an unapproved vendor gets ERR_MARKET_APPLY_REQUIRED from
  // the server and a blocking message pointing at the market's page.
  ...
  const handleToggleSchedule = async (scheduleId: string, currentlyAttending: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.code === 'ERR_SCHEDULE_HAS_ORDERS' || data.code === 'ERR_SCHEDULE_CONFLICT' || data.code === 'ERR_MARKET_APPLY_REQUIRED') {
