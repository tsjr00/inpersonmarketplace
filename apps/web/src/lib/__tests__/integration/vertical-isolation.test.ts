...
describe('VI-R16: Notification Vertical Scoping', () => {
  ...
  it('VI-R16a: notification service stores vertical_id', () => {
    const service = fs.readFileSync(path.join(webRoot, 'src/lib/notifications/service.ts'), 'utf-8')
    expect(service).toContain('vertical_id')
  })

  it('VI-R16b: notification types file has vertical references', () => {
    const types = fs.readFileSync(path.join(webRoot, 'src/lib/notifications/types.ts'), 'utf-8')
    expect(types).toContain('vertical')
  })

  it('VI-R16c: notification service handles vertical scoping', () => {
    const service = fs.readFileSync(path.join(webRoot, 'src/lib/notifications/service.ts'), 'utf-8')
    expect(service).toContain('vertical')
  })

  it('VI-R16d: notification bell component exists', () => {
    expect(fs.existsSync(path.join(webRoot, 'src/components/notifications/NotificationBell.tsx'))).toBe(true)
  })
})

// ── VI-R17: Login Vertical Membership ────────────────────────────────
describe('VI-R17: Login Vertical Membership Check', () => {
  it('VI-R17a: login page exists under vertical route', () => {
    expect(fs.existsSync(path.join(webRoot, 'src/app/[vertical]/login/page.tsx'))).toBe(true)
  })

  it('VI-R17b: login page references vertical membership', () => {
    const login = fs.readFileSync(path.join(webRoot, 'src/app/[vertical]/login/page.tsx'), 'utf-8')
    expect(login).toContain('vertical')
  })

  it('VI-R17c: admin auth checks platform_admin role', () => {
    const admin = fs.readFileSync(path.join(webRoot, 'src/lib/auth/admin.ts'), 'utf-8')
    expect(admin).toContain('platform_admin')
  })
})

// ── VI-R18: Page-Level Vertical Access Gate ──────────────────────────
describe('VI-R18: Protected Page Vertical Gate', () => {
  it('VI-R18a: enforceVerticalAccess is exported and callable', () => {
    expect(typeof enforceVerticalAccess).toBe('function')
  })

  it('VI-R18b: vertical-gate handles redirect logic', () => {
    const gate = fs.readFileSync(path.join(webRoot, 'src/lib/auth/vertical-gate.ts'), 'utf-8')
    expect(gate).toContain('redirect')
  })

  it('VI-R18c: vertical-gate has admin role bypass', () => {
    const gate = fs.readFileSync(path.join(webRoot, 'src/lib/auth/vertical-gate.ts'), 'utf-8')
    expect(gate).toContain('hasAdminRole')
  })

  it('VI-R18d: vertical-gate checks user verticals array', () => {
    const gate = fs.readFileSync(path.join(webRoot, 'src/lib/auth/vertical-gate.ts'), 'utf-8')
    expect(gate).toContain('verticals')
  })

  it('VI-R18e: vertical-gate has vendor_profiles fallback', () => {
    const gate = fs.readFileSync(path.join(webRoot, 'src/lib/auth/vertical-gate.ts'), 'utf-8')
    expect(gate).toContain('vendor_profiles')
  })
})

// ── VI-R19: sendNotification() Vertical Threading ────────────────────
describe('VI-R19: Notification Vertical Storage', () => {
  it('VI-R19a: sendInApp stores vertical_id', () => {
    const service = fs.readFileSync(path.join(webRoot, 'src/lib/notifications/service.ts'), 'utf-8')
    expect(service).toContain('vertical_id')
  })

  it('VI-R19b: webhook sendNotification includes vertical', () => {
    const webhooks = fs.readFileSync(path.join(webRoot, 'src/lib/stripe/webhooks.ts'), 'utf-8')
    expect(webhooks).toContain('vertical')
  })

  it('VI-R19c: fulfill route passes vertical to notifications', () => {
    const fulfill = fs.readFileSync(
      path.join(webRoot, 'src/app/api/vendor/orders/[id]/fulfill/route.ts'), 'utf-8'
    )
    expect(fulfill).toContain('vertical')
  })

  it('VI-R19d: notification service handles null vertical_id', () => {
    const service = fs.readFileSync(path.join(webRoot, 'src/lib/notifications/service.ts'), 'utf-8')
    expect(service).toContain('vertical')
  })
})
