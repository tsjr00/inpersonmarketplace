...
export default function VerticalErrorLogsDashboard() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setLoading(true)
      try {
        const params = new URLSearchParams({ days: String(days), vertical })
    ...
    <div style={{ maxWidth: containers.xl, margin: '0 auto' }}>
      ...
      <div className="admin-filter-bar" style={{ display: 'flex', flexWrap: 'wrap', gap: spacing.xs, marginBottom: spacing.md }}>
        <select
          ...
          style={{ padding: `${spacing['2xs']} ${spacing.xs}`, border: `1px solid ${colors.border}`, borderRadius: radius.sm, fontSize: typography.sizes.sm, backgroundColor: colors.inputBg }}
        >
        ...
        <>
          <div className="admin-grid-4" style={{ display: 'grid', gap: spacing.sm, marginBottom: spacing.md }}>
            ...
            <div style={cardStyle}>
              ...
              <p style={{ fontSize: typography.sizes.xs, color: colors.textMuted, margin: 0 }}>Unique groups</p>
            </div>
            <div style={cardStyle}>
              <p style={{ fontSize: typography.sizes['2xl'], fontWeight: 700, color: '#991b1b', margin: 0 }}>{(data.by_severity.critical || 0) + (data.by_severity.high || 0)}</p>
              <p style={{ fontSize: typography.sizes.xs, color: colors.textMuted, margin: 0 }}>Critical + High</p>
            </div>
            <div style={cardStyle}>
              <p style={{ fontSize: typography.sizes['2xl'], fontWeight: 700, color: '#92400e', margin: 0 }}>{(data.by_severity.medium || 0) + (data.by_severity.low || 0)}</p>
          ...
          <div className={`admin-detail-split ${selectedGroup ? 'has-detail' : ''}`}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: typography.sizes.sm }}>
                ...
                <tbody>
                  {data.groups.map((g, i) => {
                    ...
                    return (
                      <tr key={i} onClick={() => setSelectedGroup(isSelected ? null : g)}
                        style={{ borderBottom: `1px solid ${colors.border}`, cursor: 'pointer', backgroundColor: isSelected ? '#f0f9ff' : 'transparent' }}>
                        <td style={{ padding: spacing['2xs'], fontFamily: 'monospace', fontWeight: 600 }}>{g.error_code || '(none)'}</td>
                        <td style={{ padding: spacing['2xs'], maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{g.route || '—'}</td>
                        <td style={{ padding: spacing['2xs'], textAlign: 'center' }}>
                          <span style={{ display: 'inline-block', padding: `2px ${spacing['2xs']}`, borderRadius: radius.sm, fontSize: typography.sizes.xs, fontWeight: 600, backgroundColor: sevColor.bg, color: sevColor.text }}>{g.severity}</span>
                        </td>
                        <td style={{ padding: spacing['2xs'], textAlign: 'right', fontWeight: 700 }}>{g.count}</td>
                        <td style={{ padding: spacing['2xs'], textAlign: 'right', color: colors.textMuted }}>{timeAgo(g.last_seen)}</td>
                      </tr>
                    )
                  })}
                </tbody>
            ...
            {selectedGroup && (
              <div style={{ padding: spacing.md, backgroundColor: colors.surfaceElevated, borderRadius: radius.md, boxShadow: shadows.sm, position: 'sticky' as const, top: spacing.md }}>
                <h3 style={{ fontSize: typography.sizes.lg, fontWeight: 700, margin: `0 0 ${spacing.sm}`, color: colors.textPrimary }}>{selectedGroup.error_code || '(no code)'}</h3>
                <div style={{ display: 'grid', gap: spacing.xs, fontSize: typography.sizes.sm }}>
                  <div><span style={{ color: colors.textMuted }}>Route: </span><span style={{ fontFamily: 'monospace' }}>{selectedGroup.route || '—'}</span></div>
                  <div><span style={{ color: colors.textMuted }}>Severity: </span>
                    <span style={{ padding: `2px ${spacing['2xs']}`, borderRadius: radius.sm, fontSize: typography.sizes.xs, fontWeight: 600,
                      ...
                      color: (severityColors[selectedGroup.severity] || severityColors.low).text }}>{selectedGroup.severity}</span>
                  </div>
                ...
                <div style={{ marginTop: spacing.md, paddingTop: spacing.sm, borderTop: `1px solid ${colors.border}` }}>
                  <p style={{ fontSize: typography.sizes.xs, color: colors.textMuted, margin: 0 }}>
                    Check the <Link href={`/${vertical}/admin/errors`} style={{ color: colors.primary }}>Error Reports</Link> page for user-submitted reports with this code.
                  </p>
                </div>
              </div>
            )}
