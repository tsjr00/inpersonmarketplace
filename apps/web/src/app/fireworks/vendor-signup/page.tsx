...
export default function FireworksVendorSignup() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch("/api/vertical/fireworks");
  ...
  return (
    <main style={{ maxWidth: 720, margin: "0 auto", padding: 24 }}>
      <h1 style={{ fontSize: 26, fontWeight: 800 }}>Fireworks — Seller Signup</h1>
      <p style={{ marginTop: 10, opacity: 0.8 }}>
        This form is generated from <code>config/verticals/fireworks.json</code>.
      </p>
      ...
      <form onSubmit={handleSubmit} style={{ display: "grid", gap: 14, marginTop: 20 }}>
        {fields.map((f) => {
          ...
          // file (Phase 0: capture name only)
          if (f.type === "file") {
            return (
              <div key={f.key}>
                <label style={{ fontWeight: 600 }}>
