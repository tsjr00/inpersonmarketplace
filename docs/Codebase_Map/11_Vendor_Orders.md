...
<!-- map-stamp: domain=vendor-orders; verified=2026-09-19; commit=booth-round-B -->
<!-- map-claims
src/app/api/vendor/**
src/lib/orders/**
src/lib/vendor/**
...
**VIP Perks config (2026-09-04, punch build — mig 243 `vendor_offers`):** `api/vendor/offers` GET/PUT — the vendor's perk menu, upserted one row per kind (`punch_card` / `spend_threshold`, UNIQUE per vendor). PUT validates through the SAME parsers checkout trusts (`lib/loyalty/offers.ts` — out-of-bounds config can never be saved; flow-integrity-guarded). UI: `components/vendor/VipPerksCard.tsx`, mounted in the "VIP Perks" block on `[vertical]/vendor/insights` (gated to tiers with `vipCustomers > 0`). The perks themselves apply at checkout via `lib/loyalty/offers-checkout.ts` — see 10_Checkout_Payments item 8; the earn ping (`vip_reward_ready`) fires from `lib/loyalty/evaluate.ts`.

**Week strip (2026-09-03, P6 v2 · v2.1 2026-09-04 · v2.2 2026-09-05 — ACTIVE standing holds render as `standing_hold` on their weekday BEFORE the occurrence materializes, since occurrences only generate ≤7 days out; a materialized occurrence replaces the hold entry · v2.3 2026-09-18 — at a MANAGED, FEE-CHARGING FM market the weekly projection renders as `payment_due` ("No paid booth week") unless a PAID booth week covers the date; `lib/markets/managed-fee-gate.ts`, owner ruling D3):** `lib/vendor/week-strip.ts` (pure `assembleStrip` + loader — the vendor's next-14-DATES commitments: weekly schedules, own private-pickup windows, paid park days/booth weeks, SELECTED events; blackout-skipped and manager-cancelled days render STRUCK with the reason, never silently dropped; v2.1 adds unpaid standing occurrences as `payment_due` "Pay by {date}" — cutoff from park-standing.ts `prepayCutoffISO`, the one definition — plus 'special' make-up-day overrides shown informationally and reschedule_date named on cancelled days) → served by `api/vendor/week-schedule` (client sends its local `start` date — no server timezone guess) → rendered by `components/vendor/markets/WeekAtAGlance.tsx` at the top of `/vendor/markets`. Commitment queries mirror `lib/events/availability.ts` patterns (its doc comment explains the split); date helpers imported from it. Unit spec: `lib/vendor/__tests__/week-strip.test.ts`.
...
`markets/route.ts` (list; 2026-09-25 OB-030 (f): each traditional market also carries the card-sequence inputs `isManaged` (yes/no) · `rosterStatus` · `bookable` · `bookDone` (FM paid current/upcoming week, one parallel service-client read) → `lib/vendor/market-steps.ts` + `components/vendor/markets/MarketStepButtons.tsx` render Apply → Set Schedule → Book → Manage Listings → Prep Sheet with numbers/✓/NEXT; home market no longer auto-expanded; create private-pickup market; `pendingInvitations` = manager-initiated `market_vendors` invites ONLY — event-matching rows are excluded since 2026-08-27, events are answered on the Vendor Event Page) · `markets/suggest/route.ts` · `markets/[id]/route.ts` (PUT/DELETE) · `markets/[id]/book/route.ts` ⚠ (weekly booth → `pending_payment`; hard gate on agreement acceptance) · `markets/[id]/book-season/route.ts` ⚠ (season prepay, presale-window gated) · `markets/[id]/book-park-spot/route.ts` ⚠ (FT park spot, single destination charge, atomic all-or-nothing via `book_park_spot_atomic`) · `markets/[id]/standing-reservation/route.ts` (recurring day-of-week hold, operator approves) · `markets/[id]/seasons/route.ts` · `markets/[id]/join/route.ts` · `markets/[id]/respond/route.ts` (accept/decline a manager invitation — flips `approved`; refuses `market_type='event'` rows with 409 `ERR_EVENT_INVITATION` since 2026-08-27, because the event accept path owns menu/caps/agreement and never sets `approved`) · `markets/[id]/schedules/route.ts` (read / bulk update / single toggle; managed PAYING markets gate on roster approval — 2026-09-05 rule; since 2026-09-18 a FREE managed market auto-creates an APPROVED roster row on first activation, requires the market's terms on that first join (`needs_terms` in GET, `ERR_MARKET_TERMS_REQUIRED` from both writers) and records the acceptance + optional doc-sharing consent) · `markets/[id]/agreement-status/route.ts` · `markets/[id]/prep/route.ts` · `booth-groups/[groupId]/cancel/route.ts` ⚠ (self-cancel a paid season booth — **credit-first, no Stripe**: full credit before season start, remaining weeks minus a **25% penalty** after).
...
`checkins/route.ts` · `checkins/log/route.ts` · `location-insights/route.ts` (tier-gated) · `market-stats/route.ts` (2026-09-24: + `taxReadiness` per market from `lib/tax/readiness.ts` — the listing form shows a seller advisory when a TAXABLE item is attached to a not-ready location, owner Q3; 2026-09-25: + `isManaged` (yes/no only, never the manager id) — the listing form no longer declares every day at a MANAGED market on save, BR-1 + BR-13, OB-030 D6; unmanaged markets keep the auto-declare) · `schedule-overlaps/route.ts` (2026-09-25 OB-031, read-only: the vendor's picked days that overlap at two markets — the profile shows them while the multi-location box is off; booking-gates step 4 refuses booking at such a market) · `home-market/route.ts` (cooldown via `canChangeHomeMarket`) · `favorites/route.ts` · `referrals/route.ts` · `reviews/route.ts` · `feedback/route.ts` · `quality-findings/route.ts`.
...
| `lib/vendor/event-readiness-labels.ts` | The ONE per-vertical map of questionnaire labels + option/answer wording (2026-09-19). `EventReadinessForm` renders its selects/radios from it and the admin vendor detail renders `eventReadinessRows()` from it, so an FM vendor's "Tent / Booth" can no longer read back as "Food Trailer". Storage keys are shared across verticals (`vehicle_type` = FM setup type, `utensils_required` = FM samples, `seating_recommended` = FM "needs covered space"; `max_headcount_per_wave` is per HOUR for FM) |
