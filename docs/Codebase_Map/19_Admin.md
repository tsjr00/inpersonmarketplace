...
| Route | Purpose |
|---|---|
| `reports/route.ts` ⚠ | The revenue/report engine — highest money density of any admin route. 2026-09-24: + `tax_list_supplement` (Texas Form 01-116 NET of refunds — `buildNetListSupplement` over the per-item tax snapshots sold in the period minus the `order_item_tax_reversals` ledger rows created in the period; gross sides shown; negatives preserved; 2026-09-25: month cut at CENTRAL midnight (`lib/tax/filing-period.ts` — owner: we file monthly in Texas time; the other reports keep UTC days) and old-rate sales re-stated from `tax_rate_corrections` (mig 261) into a "Rate Correction" column, refunds re-stated too; empty until `TAX_STREAM1_ENABLED`) |
...
**Page merges (phase 3, 2026-08-30, in progress):** duplicated platform/vertical page-pairs collapse into one implementation each under `components/admin/`, both routes becoming thin wrappers (auth gates unchanged per route). Done: **users** — `UsersAdminPage.tsx` (server, scope = vertical | null-for-all) + `UsersAdminTable.tsx` (client superset: vertical column/filter on all-scope, drill-in links, buyer-tier expiry, per-scope tier options — the old platform copy offered FT tiers against FM data). The three superseded per-route clients were DELETED with owner approval 2026-08-30 (grep-verified zero importers; one of them had none even before the merge). **listings** — `ListingsAdminPage.tsx` (server) + `ListingsAdminTable.tsx` (client): one row-list at all widths, suspend/unsuspend kept; three filter repairs — search is server-side now (the old copies searched only the fetched page), the all-scope vertical filter uses the text slug (the old UUID option values could never match `listings.vertical_id`), and the status filter gained Paused (suspended). Both superseded `ListingsTableClient` copies deleted (grep-verified zero importers). **admins** — `AdminsManager.tsx` (client, mode = platform | vertical): merges the UI shape ONLY — the two modes manage different permission systems (platform admins on user_profiles via `/api/admin/admins`; vertical admins as vertical_admins rows via `/api/admin/verticals/[id]/admins`), and every endpoint + S4-2 permission gate is kept verbatim per mode; one row-list at all widths (the old platform copy had no mobile view). **error-logs** — `ErrorLogsAdminPage.tsx` (client, optional vertical prop): both copies already shared GET /api/admin/error-logs (verifyAdminScope), so the merge is client-only; superset = 90-day window + Active-window line (platform) + row-list and Error Reports cross-link (vertical). **errors** — `ErrorReportsAdminPage.tsx` (client, optional vertical prop): shared APIs already (/api/admin/errors[+/id], RLS + in-route platform gates untouched); superset = Level filter/Copy-Context/breadcrumbs/similar-reports (platform) ∪ full action set + card previews (vertical); vertical's fixed drawer replaced by the inline sticky split panel. the Record Fix Attempt form is WIRED (2026-08-31, owner-approved): action record_fix_attempt on the [id] PATCH → `recordFixAttempt()` into error_resolutions, platform-admin only (was a success-banner stub that saved nothing). **order-issues** — `OrderIssuesAdminPage.tsx` (client, optional vertical prop): shared API already (both tiers may update; S4-2 scoping untouched); superset = Closed tab + age chips (vertical) ∪ FT/FM chip on all-scope rows (platform). **analytics ⚠ money** — `AnalyticsAdminPage.tsx` (client, optional vertical prop): both copies were the same client over the same three APIs; the merge changed NO API code, figures, or money labels — each mode keeps its exact pre-merge metric set (revenue semantic stays gross product subtotal excl. fees, per the route docs). **cause — DELIBERATELY NOT MERGED** (2026-08-31): the pair is an intentional permission split, not duplication — platform page = full money management (beneficiaries/Stripe onboarding/remittances/campaigns); vertical page = the owner-decided read-only reduced list (2026-08-04 "read plus attach"; reduction enforced server-side). One component would blur that money boundary for zero dedup gain. **reports ⚠ money** — `ReportsAdminPage.tsx` (client, optional vertical prop): generators untouched (one shared POST route; ADM-1 scopes vertical admins server-side); platform keeps the vertical selector + the six accounting reports; vertical keeps the CSV|Quality tabs. ⚠ pre-existing, owner-flagged: the accounting category is curated CLIENT-side only — the API serves those reportIds (vertical-scoped) to any admin who posts them directly.

**Phase 4 (vendors, 2026-08-31):** `VendorsAdminPage/Table` (merged list — the vertical copy's rich rows for both tiers: markets, listing count, Stripe flag, inline approve/reject, expandable VendorVerificationPanel; repairs: server-side JSONB search, slug vertical filter, suspended option; **2026-09-19:** tier filter/display = unified Free/Pro/Boss via `getVendorTierLabel` — "Free" also matches legacy names + NULL; listing count = PUBLISHED and not deleted, active market boxes counted separately) + `VendorDetailAdminPage` (merged detail — full profile dump, **Markets card** (roster status · booth # + size · declared days, 2026-09-19), Event Readiness rows from the shared per-vertical map `lib/vendor/event-readiness-labels.ts`, certifications w/ signed doc links, full verification panel, LocationEditor, ⚠ VendorFeeOverride, complete status actions via `VendorStatusActions`, event toggle via `VendorEventApproval` = the stricter FT+FM COI-gated rule; the platform FT-only ungated toggle retired). All four routes are thin wrappers; `/admin/vendors/pending` unchanged. Superseded per-route clients deleted. **event-ratings** — `EventRatingsAdminPage.tsx` (client, optional vertical prop): shared API already; the real split is kept — moderation buttons render only in platform mode, matching the PATCH route's platform-admin-only gate (ADM-4); vertical stays read-only. Tab counts scoped per vertical (was a global-counts defect).

**Phase 5 (markets, 2026-08-31):** `MarketsAdminPage.tsx` (client, vertical prop — the former vertical 1,834-liner moved whole: search + status/approval/type filters, both pending-alert banners, the full inline create/edit form incl. schedules editor, event fields, ⚠ operator keep rate, manager assignment, tax card, **tax-readiness filter + row chip** (2026-09-24: "Tax codes" dropdown = the III.7 needs-codes queue — no codes / address changed / stale quarter / ready, same three guardrails as `compute-cart-tax.ts`; a not-ready row wears a chip), intake-review block w/ duplicates + documents; rows and name cells now drill in, and `?edit=<id>` deep-links the form — the one edit home) + NEW `MarketDetailAdminPage.tsx` (server; marketId + backHref + vertical? that 404s outside the vertical) absorbing the platform detail (duplicates, documents, info incl. description/contact, tax, manager + history, schedules display/manager, vendor approvals, survey) + `MarketDetailActions.tsx` (approve/reject suggestion, suspend private-pickup, guarded delete — the list's row actions on the detail page, so phones no longer need landscape). Manager section gates on market_type='traditional' (vertical-agnostic — FT park operators too; was FM-only). Relocated: `MarketScheduleManager` / `MarketVendorManager` / `MarketApproveStatusButton`. Form honesty: type locked on edit (the admin PUT ignores market_type), private_pickup removed from create (the admin POST rejects it). Phantom-column repairs on the platform list (`markets` has `market_type`, not `type`): the type filter errored the whole query (blank list) and every row labeled "Private Pickup" — fixed, + 🎪 Event filter option. `DeleteMarketButton` retargeted to the guarded admin DELETE (listings + ADM-3 financial-history blocks; the old `/api/markets/[id]` DELETE has no guards). RETIRED: platform MarketForm + new/edit pages — `/api/markets` POST/PATCH write phantom columns (`type`, `zip_code`) and errored on save; creation lives on the vertical list. ⚠ open: `/api/markets` POST/PATCH/DELETE now have zero first-party callers — removal proposed, owner decision pending.

**Phase 6 (events pipeline board, 2026-09-04):** `EventsAdminPage.tsx` (client, vertical prop — vertical-only, no platform pair exists) — the former 2,038-line flat list REORGANIZED into stage groups with jump-nav: New requests (new|reviewing) → Inviting (approved, no invites) → Selecting (approved, invites out — card shows selected/bench/awaiting counts via the shared `classifyVendorEventStage`) → Upcoming/Event day (ready|active) → Wrap-up (review|completed) → collapsed Closed (declined|cancelled; Restore lives there). ONE `stageOf()` derivation (S-1 carried — nothing hand-typed to drift). The detail panel moved VERBATIM (already stage-conditional): all 22 inventoried capabilities survive — status ladder + cancel/restore, chip-in, email/address repair, viability + match scoring, invite/rematch panel (T-64/70/79 rules), waves, repeat, settlement/attendee links, fee payments, change-requests card, pending-applications banner (full inventory: `apps/web/.claude/admin_phase6_events_board.md`). Route is a thin wrapper; settlement stays its own drill-in.
...
- **`app/admin/**`** (19 pages — phase 5 retired markets new/edit) — the global console with its **own login and MFA flow** (`/admin/login`, `/admin/mfa/setup`, `/admin/mfa/verify`), separate from vertical user auth.
- **`app/[vertical]/admin/**`** — the vertical-scoped mirror: `admins`, `analytics`, `users`, `vendors` (+ `[vendorId]`), `vendor-activity`, `listings`, `markets` (+ `[id]`), `events` (+ `[id]/settlement`), `errors`, `error-logs`, `feedback`, `event-ratings`, `order-issues`, `knowledge`, `reports`, `stripe-reconcile`.
