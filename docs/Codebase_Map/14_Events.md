...
<!-- map-stamp: domain=events; verified=2026-09-03; commit=f23effb4 -->
<!-- map-claims
src/app/api/events/**
...
7. `lib/events/availability.ts` + `lib/events/blackouts.ts` (2026-08-27, R3-4) — **event ↔ location conflicts.** A vendor cannot do an event AND another scheduled location at the same time (same date + overlapping hours) unless `profile_data.multiple_trucks` says they can cover both (FT "multiple trucks", FM "can staff more than one location" — one key, `EditProfileForm`). `loadVendorAvailability` gathers active weekly schedules, paid park days, paid booth weeks, other accepted events, and own private pickups (only when they hold open orders), plus open order_items / market-box pickups on the dates; pure `evaluateConflicts` decides: flagged → must confirm both (organizer told via the `[MULTI-TRUCK]` note); not flagged → open orders or another event = refused, else must acknowledge the skip → `writeEventBlackouts` pauses the location for the WHOLE day (`vendor_date_blackouts`, mig 238; `get_available_pickup_dates` drops the date, so cart/listing/calendar all stop offering it) and a paid park spot's operator gets `park_spot_skipped_for_event` (notify only — booking stays paid, no no-show suppression). The check runs on the Vendor Event Page GET while invited AND in the accept route; a promoted backup re-enters through accept. `liftEventBlackouts` is called from every exit — organizer bench (`select`), vendor withdraw (`vendor/events/[marketId]/cancel`), organizer cancel, admin cancel — guarded in flow-integrity. Event cards on My Locations no longer offer Set Schedule (attendance is the acceptance row). **Reverse direction BUILT 2026-09-05** (`lib/events/booking-event-guard.ts`, truck3 staging finding): accept-time blackouts only cover commitments existing at acceptance, so `vendorEventConflictsOnDates` (same commitment definition: accepted, not benched, not revoked, multiple_trucks exempt) now gates the three booking routes — park spot, booth week, booth season (booth weeks expand to the market's operating dates) — rejecting with a withdraw-first message; flow-integrity-pinned. Known remaining hole: acceptances made BEFORE mig 238 landed have no blackouts (owner: skip the backfill for staging test data; RE-CHECK when 238 pastes to PROD — any real pre-paste acceptances there would need the one-time sweep).
8. `lib/events/vendor-stage.ts` (2026-09-03) — the ONE ladder for "where is this vendor in the event flow" (declined / withdrawn / invited / bench / accepted_awaiting / selected / none). Consumed by the Vendor Event Page header (which layers fee-detail labels on 'selected') and the My Locations event pill (`EventMarketsSection`) — before this, the pill said "Attending" for any merely-accepted truck. Also the Public Event Page roster (2026-09-17: self-service events list 'selected' only). Guarded in flow-integrity ("Vendor event stage — shared classifier").
9. `lib/events/menu-pare.ts` + `event_vendor_listings.host_status` (mig 241, 2026-09-03) — **host menu pare-down.** The organizer may trim a truck's proposed menu ONCE, at that truck's FIRST selection (ruling B, owner 2026-09-17 — the lock moved from the event's first confirmation to each vendor, so late responders can still be selected and trimmed; an already-selected truck is locked, min 2 kept items, activated backups never pared; safe because unselected vendors cannot take event pre-orders, mig 254). The select GET sends `can_pare` per vendor. ⚠ THE PAIR: evl.host_status is the DISPLAY half, the `listing_markets` link is the SELL half (cart validates there) — the select route declines + deletes together, every menu reader filters declined (shop-data, Public Event Page, vendors-with-listings, admin menu display), and admin event-restore rebuilds links from APPROVED rows only. The fee-vendor protection is the SEQUENCE (decisions.md): payment arms after selection, so the truck sees its final menu on the Vendor Event Page ("Your event menu") before paying. Guarded in flow-integrity ("Host menu pare-down — the pair holds everywhere").
10. **Treat `api/events/[token]/order/route.ts` and the wave stack as quarantined** — see [§ company-paid](#company-paid--deferred-and-non-executable). Do not reason about live money there.
...
| `api/events/[token]/refresh-matches/route.ts` | Re-runs `autoMatchAndInvite` after matching-affecting edits; 409 while a self-service event's invitations are held (mig 239) | No |
| `api/events/[token]/release-invitations/route.ts` | **Invitation gate (mig 239, 2026-08-29).** Organizer's "Send invitations" click for self-service events: organizer auth, approved, every `lib/events/invitation-gate.ts` detail answered (else 409 + list), atomic `invitations_released_at` stamp, then the REAL `autoMatchAndInvite` + `auto_invite_sent_at`. Idempotent after the stamp. | No |
...
| `components/events/AdminEventFeePayments.tsx` | Admin fee-payments card (refund-matrix 2026-08-16): lists every evfp row for an event + Refund button (ConfirmDialog) → `api/admin/events/[id]/fee-payments` POST | Money-adjacent |
...
| File | Purpose |
|---|---|
| `lib/events/event-actions.ts` | `generateAccessCode()`, `approveEventRequest()` (token + market + schedule, `cutoff_hours` clamped 12–168 at `:132`), `autoMatchAndInvite(…, { dryRun })` — dry run scores + counts, writes nothing (intake's preliminary match since mig 239) |
| `lib/events/invitation-gate.ts` | **Invitation gate (mig 239):** `invitationsHeld(row)` (self-service + no `invitations_released_at`) and `missingInvitationDetails(row)` — the owner's required list (fee decided · run-before · spend per purchaser · Event Context saved · background checks · Logistics saved) with the column each maps to. Consumers: intake (dry run), release route, cron re-match sweep, refresh-matches, dashboard card, details editor labels. |
...
| `lib/events/shop-data.ts` | Single source for the attendee payload. Accepts status `approved\|ready\|active` (`:130`), **zeroes `price_cents` for anonymous callers**, loads waves only when wave ordering is on. **Mig-234 mirror (2026-08-16, @paired-rule event-sells-on-acceptance): shows ATTENDING vendors only — accepted + not benched + fee paid/covered at fee events** — matching the SQL cart gate; 3 flow-integrity guards pin both surfaces. **Mig-254 mirror (2026-09-17, owner ruling): on SELF-SERVICE events the vendor must also be organizer-SELECTED (`organizer_selected_at`) — the event turns 'ready' before any selection and late responders arrive un-benched, so on free events they used to sell; admin-managed events keep the acceptance rule. 2 more guards pin the SQL conjunct and this mirror** |
...
| `components/events/OrganizerEventDetails.tsx` | Stage-2 detail editor + refresh-matches banner (~770 lines); labels tag the invitation-gate fields "needed before invitations go out" (mig 239) |
| `components/events/InvitationGateCard.tsx` | Organizer dashboard "Send invitations" card (mig 239): missing-detail list grouped by section, button unlocks when empty, receipt after release |
...
| `app/[vertical]/events/[token]/page.tsx` | Public event landing (`force-dynamic`). Roster = ATTENDING vendors: on self-service events only the organizer-SELECTED (`vendor-stage.ts` 'selected'; 2026-09-17, TR-022), on admin-managed events accepted + not benched. "Pre-Orders Open" shows only when at least one vendor is listed |
