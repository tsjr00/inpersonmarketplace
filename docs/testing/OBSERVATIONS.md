...
### OB-031 · 2026-09-25 · tester (via owner) · manager (FM, Westgate Mall) + vendor (FM) · printable v3 W1 step 2 + W5 · staging `ab1cd64b`
Report (tester's words, verbatim as pasted by the owner):

Part 1 — W1 step 2:
Tester step: W1 step 2
  Market: Amarillo
  What it checks: Right under "Action Items" there is either the amber "Your market isn't visible to buyers  yet" (and Amarillo is not in the public list), or nothing, with the green "✓ Your market is visible to   buyers" at the bottom of Setup (and Amarillo is listed). A mismatch between the card and the public list is a finding.

Results:

Market = Westgate ++ yes, under ‘Action Items’  >  4 With published listings /  9 With active schedules / 0 With all three (needed: 1+).

 at top of Setup section it says: “✓ Setup complete — all 5 required steps configured “  - but at bottom of setup looks like it needs documents but just says ‘required’ next to a box i cant check. ++ 
Action items said 1 vendor pending  -  I approved and system assigned booth 01 based on size tier - action items responded correctly after approving
Westgate is not visible on public market list

Part 2 — W5 (owner: "here is a much more robust testing responce"):
As vendor ( farmersmarketingapp+cottagevendor1 ) the listing page had 2 markets checked but the helpr text at the top of the market list already said i was using 3 of 3 markets - one traditional market was available to select and i did - i saved with no error.  I went back into the item and it showed all 3 markets checked but still said 3 or 3 markets and no error. - the helper text does not list the markets already used. - either the helper text was wrong (count) or it is right and it a brother bug -  

From the next 2 weeks list i cl;ickd on amarillo market to book – it took me to booking page, i agreed and then to payment - i used back button to leave payment and i saw the resume payment button but did not use it.  I went back to the next 2 weeks listing and repeated the process and was given this error: “Your assigned booth is already booked for that week at Amarillo Community Market. Please contact the market manager to resolve it. “ and was blocked from continuing to payment > while in payment the first time i select cash app pay but forgot the vendors email so i backed out and never completed payment - If booth number is being assigned before payment is completed that is a problem.  >>  i then went back to next 2 weeks list and tried to book a booth at river rd market and it lets me - showed success screen and “payment received”  >> i clicked on view my bookings and it showed me the river rd booking i just completed + the amarillo market booking shown as pending >>  I clicked complete payment and it took me to payment screen and let me pay  >> SUCCESS SCREEN  >> Back to the my booth bookings page and both markets show paid for same day  > I went back to vendor profile and the vendor is not shown as being able to do more than 1 market.  (that box may have been checed earlier and i unchecked it & saved before the test) i don;’t recall 100% , that is worth exploring.  >>  back to listings paghe & next 2 weeks list now it shows i am booked at amarillo & river rd markets for Oct. 3rd at same time - i was never shown a box confirming a conflict.  - make sure we didn’t lose that functionality in recent changes (maybe we just never tried this path before.

Triage (code read 2026-09-25):
PASS — W1 step 2 at Westgate Mall (capital M): the not-visible card sat under Action Items with the counts; Westgate absent from the public list — consistent (TR-121). Action Items "1 vendor pending" cleared after approving (TR-098 part). Continue payment from /vendor/bookings worked (TR-118 part).
NOT A DEFECT — "booth 01 on approval": approval never assigns a number (`vendor-approval/route.ts:77-79, :158`; the dropdown starts at "— no booth # —", `BoothNumberPicker.tsx:125`); owner: the vendor likely already held 01.
DEFECTS / GAPS, all fixed 2026-09-25 (`3f71e947`):
 1. Documents card mixed message: "Setup complete" counts setup steps; documents are advisory, not among them, but each line read "· Required" beside a ⬜ that looked like a checkbox and said "before approving your market" on an approved market (`VerificationDocumentsCard.tsx:253-267`). Fixed: status marks, "Requested by the platform · not uploaded yet", no "before approving" once active (TR-124).
 2. "Your assigned booth is already booked" on retrying one's OWN unpaid week: the booking function's held-booth check (`…_258_…sql:434-442`) runs before its duplicate check (`:546-547`) and does not exclude the vendor's own row, so a vendor with a held number never reached Continue payment. Fixed in the route: BOOTH_TAKEN on one's own live row = duplicate → Continue payment; own-row lookup skips released rows (TR-123). On "booth assigned before payment": an unpaid booking holds the booth by design until paid or its Stripe page expires (24h) — owner may shorten later (payments.ts, protected).
