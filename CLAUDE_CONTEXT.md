...
**As of 2026-08-16: migrations 228–233 are on Dev + Staging ONLY — paste ALL SIX on Prod at the next prod push (all paste-and-go): 228/229 Event Vendor Fees, 230 attendee re-confirmation, 231 event background checks, 232 backup bench phase 1, 233 backup bench money (permissive-direction: widened CHECKs/index + RPC replacement; also fixes the market_vendors 'cancelled' CHECK bug).** As of 2026-08-13 (late): migrations 001–224 PLUS 226–227 are applied to Dev, Staging, AND Production** (213–223 reached Prod 2026-08-13 during the client-demo deploy; 224 on 2026-08-12; **226/227 — RLS-only security tightenings closing anonymous reads of private-event data — applied same-day 2026-08-13 with exact-match pre/post anon-role counts**). **Only migration 225 exists nowhere** — written, deliberately parked, verification recipe in its header. ⚠ Do NOT trust this file or `SCHEMA_SNAPSHOT.md` changelog rows for per-environment status — both were proven wrong four separate times on 2026-08-13 (migs 210/211/212/215); **query the live environment** (`information_schema` / `pg_proc` / `to_regclass`).
